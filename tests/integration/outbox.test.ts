...
describe("Transactional Outbox", () => {
  ...
  async function countRows(where: string): Promise<number> {
    ...
    const res = await pool.query(
      `SELECT COUNT(*)::int AS n FROM outbox_events WHERE ${where}`,
    );
    return res.rows[0].n;
  }
  ...
  describe("outbox writer", () => {
    it("commits outbox rows written inside a transaction", async () => {
      ...
      await runInTransaction(async () => {
        await writer.write("post.published", {
          postId,
          title: "Hello",
          slug: "hello",
        });
    ...
    it("persists rows outside a transaction as pending (worker-down safety)", async () => {
      ...
      await writer.write("post.unpublished", {
        postId: "pending-target",
        slug: "pending",
      });
      expect(await countRows(`event_type = 'post.unpublished'`)).toBe(
        before + 1,
      );

      const rows = await repository.claimReady({ limit: 10 });
      const pendingUnpublish = rows.find(
        (r) => r.eventType === "post.unpublished",
      );
    ...
    it("propagates the active OpenTelemetry traceId into the outbox envelope", async () => {
      ...
      const row = rows.find((r) => {
        ...
        return (
          ...
          (env.payload as { postId?: string }).postId === postId
        );
      });
  ...
  describe("outbox repository claims", () => {
    it("claims pending rows exactly once under contention (SKIP LOCKED)", async () => {
      const id = await writeDirect("post.published", {
        postId: "contend-1",
        title: "t",
        slug: "s",
      });
      ...
      try {
        ...
        await client.query(
          `SELECT id FROM outbox_events WHERE id = '${id}' FOR UPDATE`,
        );
    ...
    it("reclaims stale delivering claims so they are retried", async () => {
      ...
      const reclaimed = await repository.reclaimStaleClaims({
        staleAfterMs: 60_000,
      });
    ...
    it("does not claim rows whose availableAfter window has not passed", async () => {
      ...
      await repository.markFailed(id, "simulated relay outage", {
        maxAttempts: 5,
      });

      const now = new Date();
      const withinWindow = await repository.claimReady({
        limit: 10,
        now: new Date(now.getTime() + 1_000),
      });
      expect(withinWindow.map((r) => r.id)).not.toContain(id);

      const afterWindow = await repository.claimReady({
        limit: 10,
    ...
    it("marks a row failed permanently after max attempts", async () => {
      const id = await writeDirect("post.unpublished", {
        postId: "failmax-1",
        slug: "x",
      });
      ...
      await repository.markFailed(id, "nope", { maxAttempts: 3 });
      expect(
        await countRows(`id = '${id}' AND status = 'failed' AND attempts = 3`),
      ).toBe(1);
    });
  });
  ...
  describe("envelope validation", () => {
    it("round-trips typed envelopes", () => {
      const envelope = buildEventEnvelope("post.published", {
        postId: "p1",
        title: "T",
        slug: "t",
      });
  ...
  describe("outbox dispatcher", () => {
    ...
    it("keeps claim order across a batch", async () => {
      ...
      await writeDirect(
        "post.published",
        { postId: "order-1", title: "a", slug: "a" },
        { createdAt: new Date(t0.getTime()) },
      );
      await writeDirect(
        "post.published",
        { postId: "order-2", title: "b", slug: "b" },
    ...
    it("retries failed deliveries after backoff and tolerates re-delivery idempotently", async () => {
      ...
      expect(
        await countRows(`id = '${id}' AND status = 'pending' AND attempts = 1`),
      ).toBe(1);
      ...
      const pool = getDbPool();
      await pool.query(
        `UPDATE outbox_events SET available_after = now() - interval '1 second' WHERE id = '${id}'`,
      );
      await dispatcher.runDispatchCycle();
