...
describe("Transaction Infrastructure", () => {
  ...
  function makePostsService(
    ...
    const originalCreateRevision =
      failingRevisionRepo.createRevision.bind(failingRevisionRepo);
    failingRevisionRepo.createRevision = async (data) => {
      if (overrides.failRevision)
        throw new Error("injected: revision insert failure");
    ...
    const originalReplaceRefs =
      failingMediaRepo.replaceResourceReferences.bind(failingMediaRepo);
    failingMediaRepo.replaceResourceReferences = async (
      resourceType: string,
      resourceId: string,
      mediaIdsWithPaths: Array<{ mediaId: string; fieldPath?: string }>,
    ) => {
      if (overrides.failMedia)
        throw new Error("injected: media reference failure");
      return originalReplaceRefs(resourceType, resourceId, mediaIdsWithPaths);
    };

    const failingMediaService = new MediaService(
      failingMediaRepo,
  ...
  it("commits when work succeeds", async () => {
    ...
    expect(await countRows("users", `email = 'tx.commit@vibress.local'`)).toBe(
      1,
    );
  });
  ...
  it("rolls back when work throws", async () => {
    ...
    expect(
      await countRows("users", `email = 'tx.rollback@vibress.local'`),
    ).toBe(0);
  });
  ...
  describe("post creation failure injection", () => {
    it("audit insert failure rolls back post, authors, tags, and revision", async () => {
      ...
      expect(
        await countRows("audit_events", `metadata->>'title' = 'Tx Audit Fail'`),
      ).toBe(0);
    });
    ...
    it("revision insert failure rolls back post and author assignments", async () => {
      ...
      await expect(
        service.createPost(
          { title: "Tx Revision Fail", primaryAuthorId: testUser.id },
          testUser.id,
        ),
      ...
      expect(
        await countRows(
          "post_authors",
          `post_id IN (SELECT id FROM posts WHERE title = 'Tx Revision Fail')`,
        ),
      ).toBe(0);
    });
    ...
    it("tag insert failure rolls back post and authors", async () => {
      ...
      expect(
        await countRows(
          "post_authors",
          `post_id IN (SELECT id FROM posts WHERE title = 'Tx Tag Fail')`,
        ),
      ).toBe(0);
      expect(
        await countRows("audit_events", `metadata->>'title' = 'Tx Tag Fail'`),
      ).toBe(0);
    });
    ...
    it("successful create leaves consistent state", async () => {
      ...
      expect(await countRows("revisions", `resource_id = '${post.id}'`)).toBe(
        1,
      );
      expect(await countRows("audit_events", `target_id = '${post.id}'`)).toBe(
        1,
      );
    });
  });
    ...
    it("media reference failure rolls back title and author changes", async () => {
      ...
      await expect(
        service.updatePost(
          post.id,
          {
            title: "Tx Update Changed",
            content: { version: 1, text: "changed" },
          },
          testUser.id,
        ),
      ...
      const pool = getDbPool();
      const res = await pool.query(`SELECT title FROM posts WHERE id = $1`, [
        post.id,
      ]);
      ...
      expect(
        await countRows(
          "audit_events",
          `metadata->>'title' = 'Tx Update Changed'`,
        ),
      ).toBe(0);
    });
  });
  ...
  describe("publish failure injection", () => {
    it("revision failure on publish reverts post status", async () => {
      ...
      const pool = getDbPool();
      const res = await pool.query(`SELECT status FROM posts WHERE id = $1`, [
        post.id,
      ]);
  ...
  describe("author setter atomicity", () => {
    it("FK failure during insert leaves existing authors intact", async () => {
      ...
      expect(
        await countRows(
          "post_authors",
          `post_id = '${post.id}' AND user_id = '${testUser.id}'`,
        ),
      ).toBe(1);
    });
  });
  ...
  describe("page creation failure injection", () => {
    it("audit failure rolls back page and authors", async () => {
      ...
      await expect(
        service.createPage(
          { title: "Tx Page Fail", primaryAuthorId: testUser.id },
          testUser.id,
        ),
      ).rejects.toThrow("injected: audit insert failure");
      ...
      expect(
        await countRows(
          "page_authors",
          `page_id IN (SELECT id FROM pages WHERE title = 'Tx Page Fail')`,
        ),
      ).toBe(0);
      expect(
        await countRows("audit_events", `metadata->>'title' = 'Tx Page Fail'`),
      ).toBe(0);
    });
  });
});
