...
describe("Analytics v1 (traffic)", () => {
  ...
  const makeEvent = (
    name: "post.view" | "page.view",
    over: Partial<Parameters<AnalyticsService["ingest"]>[0]> = {},
  ) => ({
  ...
  beforeAll(async () => {
    ...
    overviewService = new AnalyticsOverviewService(
  ...
  beforeEach(async () => {
    const pool = getDbPool();
    await pool.query(
      "TRUNCATE TABLE analytics_events, analytics_daily_metrics CASCADE;",
    );
  });
  ...
  describe("unique visitors accuracy", () => {
    it("Visitor A → 10 views, Visitor B → 2 views ⇒ views=12, visitors=2", async () => {
      ...
      for (let i = 0; i < 10; i++) {
        await analyticsService.ingest(
          makeEvent("post.view", { visitorHash: aHash }),
        );
      }
      for (let i = 0; i < 2; i++) {
        await analyticsService.ingest(
          makeEvent("page.view", { visitorHash: bHash, path: "/about" }),
        );
      }
    ...
    it("period visitors ≠ sum of daily unique visitors (A visits day1 and day2 ⇒ daily 1+1, period 1)", async () => {
      ...
      await analyticsService.ingest(
        makeEvent("post.view", { visitorHash: aHash, occurredAt: now }),
      );
      await analyticsService.ingest(
        makeEvent("post.view", { visitorHash: aHash, occurredAt: yesterday }),
      );
      ...
      const daily =
        await new DrizzleAnalyticsRepository().countDistinctVisitorsByDay(
  ...
  describe("idempotency", () => {
    it("duplicate eventId does not double-count", async () => {
      ...
      await analyticsService.ingest({
        ...event,
        visitorHash: deriveVisitorHash("another", HMAC_SECRET),
      });
      const overview = await overviewService.getOverview({ range: "7d" });
  ...
  describe("bot exclusion", () => {
    it("bot events persist raw but never count in views/visitors", async () => {
      const before = (await overviewService.getOverview({ range: "7d" }))
        .summary.views;
      await analyticsService.ingest(
        makeEvent("post.view", {
          isBot: true,
          visitorHash: deriveVisitorHash("bot", HMAC_SECRET),
        }),
      );
      await analyticsService.ingest(
        makeEvent("post.view", {
          isBot: true,
          visitorHash: deriveVisitorHash("bot2", HMAC_SECRET),
        }),
      );
      const after = (await overviewService.getOverview({ range: "7d" })).summary
        .views;
  ...
  describe("top content and referrers", () => {
    it("orders top content by views and top referrers by domain", async () => {
      ...
      expect(
        overview.referrers.find((r) => r.name === "google.com")?.views,
      ).toBeGreaterThanOrEqual(5);
      expect(
        overview.referrers.find((r) => r.name === "Direct")?.views,
  ...
  describe("referrers", () => {
    it("internal navigation never inflates Direct", async () => {
      ...
      expect(
        overview.referrers.find((r) => r.name === "internal"),
      ).toBeUndefined();
  ...
  describe("privacy helpers", () => {
    it("query strings and full referrer URLs are never stored", async () => {
      ...
      const domain = normalizeReferrerDomain(
        "https://github.com/foo?email=a@b.com",
        "http://localhost:7777",
      );
  ...
  describe("retention", () => {
    it("deletes only traffic events older than the window", async () => {
      ...
      await analyticsService.ingest(
        makeEvent("post.view", {
          occurredAt: old,
          visitorHash: deriveVisitorHash("old", HMAC_SECRET),
        }),
      );
      await analyticsService.ingest(
        makeEvent("post.view", {
          occurredAt: new Date(),
          visitorHash: deriveVisitorHash("fresh", HMAC_SECRET),
        }),
      );

      const deleted = await repo.deleteTrafficEventsBefore(
  ...
  describe("API authorization", () => {
    ...
    it("owner and administrator can read the overview; editor cannot; anonymous cannot", async () => {
      ...
      const editor = await createUserWithRole(
        "a-editor@test.local",
        "editor",
        "AnalyticsPass123!",
      );
      ...
      const editorCookie = await login(
        "a-editor@test.local",
        "AnalyticsPass123!",
      );
    ...
    it("client eventId is forwarded unchanged and reused on retry (idempotency)", async () => {
      ...
      await analyticsService.ingest(
        makeEvent("post.view", {
          eventId: eventIdX,
          visitorHash: deriveVisitorHash("idem-visitor", HMAC_SECRET),
        }),
      );
      await analyticsService.ingest(
        makeEvent("post.view", {
      ...
      const countFor = async (eventId: string): Promise<number> => {
        const { rows } = await pool.query<{ n: string }>(
          "SELECT count(*)::text AS n FROM analytics_events WHERE event_id = $1",
          [eventId],
        );
  ...
  describe("billing entitlement (paid members)", () => {
    ...
    it("matches the existing ACCESS_GRANTING_STATUSES policy (trialing/active only)", async () => {
      const pool = getDbPool();
      await pool.query(
        "TRUNCATE TABLE offers, subscriptions, plans, products, members CASCADE;",
      );
