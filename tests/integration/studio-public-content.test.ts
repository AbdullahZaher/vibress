...
describe("Studio public content (Posts + Pages)", () => {
  ...
  const ALL_CARDS = [
    ...
    },
    {
      ...
      cardData: {
        images: [
          { assetId: "", src: "/g1.png", alt: "one" },
          { assetId: "", src: "/g2.png", alt: "two" },
        ],
      },
    },
    {
      cardType: "video",
      cardData: { assetId: "", src: "/v.mp4", caption: "Video caption" },
    },
    {
      cardType: "audio",
      cardData: { assetId: "", src: "/a.mp3", title: "Podcast" },
    },
    {
      cardType: "file",
      cardData: {
    ...
    {
      cardType: "button",
      cardData: { text: "Buy now", url: "https://store.example/p" },
    },
    {
      cardType: "callout",
      cardData: { text: "Note this", emoji: "💡", backgroundColor: "grey" },
    },
    {
      cardType: "toggle",
      cardData: { heading: "FAQ", content: "Answer text" },
    },
    { cardType: "markdown", cardData: { markdown: "**bold** markdown" } },
    {
      cardType: "html",
      cardData: {
        html: '<div class="widget">Safe widget</div><script>alert(1)</script>',
      },
    },
  ...
  const cardNodes = (type = "studio-card") =>
    ALL_CARDS.map((c) => {
      const data = JSON.parse(JSON.stringify(c.cardData)) as Record<
        string,
        unknown
      >;
  ...
  it("serves all card types through the public Content API (Post)", async () => {
    ...
    await createPublishedPost(
      "Studio Cards Post",
      slug,
      cardNodes("studio-card"),
    );

    const res = await app.inject({
      method: "GET",
      url: `/api/content/v1/posts/${slug}`,
    });
    expect(res.statusCode).toBe(200);
    const body = res.json().post;

    // content: canonical nodes with resolved media URLs
    const children = body.content.root.children as Array<{
      type: string;
      cardType: string;
    ...
    expect(
      (gallery?.cardData.images as Array<{ src: string }>)[0].src,
    ).toContain("/content/media/media/");
    ...
    const created = await app.inject({
      ...
      payload: {
        ...
        content: {
          schema: "vibress-studio",
          version: 1,
          root: { type: "root", children: cardNodes("studio-card") },
        },
      },
    });
    expect(created.statusCode).toBe(201);
