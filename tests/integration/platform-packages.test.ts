...
describe("Platform Packages Suite (H7)", () => {
  ...
  describe("@vibress/observability", () => {
    it("manages request trace context via AsyncLocalStorage", async () => {
      await setRequestTraceContext(
        { requestId: "req-123", actorId: "user-456" },
        async () => {
    ...
    it("records metric counters and gauges", () => {
      ...
      expect(items.find((m) => m.name === "http_requests_total")?.value).toBe(
        2,
      );
      expect(items.find((m) => m.name === "active_connections")?.value).toBe(
        42,
      );
    });
    ...
    it("exports Prometheus text format with process metrics", () => {
      metrics.clear();
      metrics.counter("http_requests_total", 3, {
        method: "GET",
        path: "/api",
        status: "2xx",
      });
      ...
      expect(text).toContain(
        'http_requests_total{method="GET",path="/api",status="2xx"} 3',
      );
    ...
    it("provides a named tracer from the global provider", () => {
      ...
      const scope = (tracer as { instrumentationScope?: { name?: string } })
    ...
    it("continues a remote trace from a W3C traceparent-shaped context", async () => {
      ...
      await withRemoteTraceContext(
        { traceId: remoteTraceId, spanId: remoteSpanId },
        async () => {
    ...
    it("no-ops safely for invalid remote trace context", async () => {
      let ran = false;
      await withRemoteTraceContext(
        { traceId: "not-a-valid-trace-id", spanId: "xx" },
        async () => {
          ran = true;
        },
      );
  ...
  describe("@vibress/ui", () => {
    it("joins classnames cleanly", () => {
      expect(cn("btn", false, "btn-primary", null, undefined, "px-4")).toBe(
  ...
  describe("@vibress/i18n", () => {
    it("translates and interpolates templates", () => {
      ...
      expect(translator.translate("welcome", { name: "World" }, "en")).toBe(
        "Hello, World!",
      );
      expect(translator.translate("welcome", { name: "Mundo" }, "es")).toBe(
        "¡Hola, Mundo!",
      );
      expect(translator.translate("goodbye", { name: "Alice" })).toBe(
        "Goodbye, Alice!",
      );
    });
