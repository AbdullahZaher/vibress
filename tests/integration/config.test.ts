...
describe('Typed configuration', () => {
  ...
  it('loads production with explicit secrets and CORS origins', () => {
    ...
    expect(config.cors.origin).toEqual(['https://admin.example.com', 'https://portal.example.com']);
    expect(config.cors.staffAllowedOrigins).not.toContain('http://localhost:7777');
    expect(config.cors.memberAllowedOrigins).not.toContain('http://localhost:7777');
  });

  it('rejects invalid URLs and invalid delivery modes', () => {
    expect(() => loadConfig({ SITE_URL: 'not-a-url' })).toThrow(ConfigError);
    expect(() => loadConfig({ EVENT_DELIVERY_MODE: 'sideways' })).toThrow(ConfigError);
    expect(() => loadConfig({ CORS_ORIGINS: 'https://ok.example.com,not-a-url' })).toThrow(ConfigError);
  });
  ...
  it('defaults observability to enabled and respects explicit overrides', () => {
    ...
    const disabled = loadConfig({ NODE_ENV: 'development', METRICS_ENABLED: 'false', TRACING_ENABLED: 'false' });
