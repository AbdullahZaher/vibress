...
describe('Auth & Authorization API Security', () => {
  ...
  it('login with invalid password or non-existent email returns generic INVALID_CREDENTIALS', async () => {
    const res1 = await app.inject({
      method: 'POST',
      url: '/api/admin/v1/auth/login',
      payload: { email: 'admin.owner@vibress.test', password: 'WrongPassword123!' },
    });
  ...
  const getCookieFromLogin = async (email: string, pass: string) => {
    ...
    if (!cookieHeader) throw new Error('Login failed to return set-cookie header');
  ...
  it('enforces permission checks on protected endpoints', async () => {
    ...
    const ownerCookie = await getCookieFromLogin('admin.owner@vibress.test', ownerPassword);
    ...
    const limitedCookie = await getCookieFromLogin('limited.contributor@vibress.test', limitedPassword);
  ...
  it('revokes session on logout', async () => {
    const cookieHeader = await getCookieFromLogin('admin.owner@vibress.test', ownerPassword);
