...
test.describe('Batch 12 Platform E2E Suite', () => {
  ...
  test('[API Keys] create, authenticate, scope enforcement, revoke', async ({ request }) => {
    ...
    const deniedRes = await request.post(`${API}/api/machine/v1/events`, {
      headers: { Authorization: `Bearer ${limited.secret}`, 'Content-Type': 'application/json' },
      data: {},
    });
    ...
    // Revoke → rejected
    const revokeRes = await request.post(`${API}/api/admin/v1/api-keys/${key.id}/revoke`, {
      headers: { Origin: API },
    });
  ...
  test('[Webhooks] register endpoint + domain event queues delivery + signature verification', async ({ request }) => {
    await loginAsStaff(request);
    ...
    const createRes = await request.post(`${API}/api/admin/v1/webhook-endpoints`, {
      headers: { Origin: API },
      data: {
        name: 'E2E Receiver',
    ...
    const bad1 = await request.post(`${API}/api/admin/v1/webhook-endpoints`, {
      headers: { Origin: API },
      data: { name: 'Bad', url: 'http://localhost:9000/hook', eventTypes: ['x'] },
    });
    ...
    const bad2 = await request.post(`${API}/api/admin/v1/webhook-endpoints`, {
      headers: { Origin: API },
      data: { name: 'Bad', url: 'http://169.254.169.254/latest/meta-data', eventTypes: ['x'] },
    });
    ...
    const deliveriesRes = await request.get(`${API}/api/admin/v1/webhook-deliveries?endpointId=${endpoint.id}`);
  ...
  test('[Plugins] activate official plugin + secret masked', async ({ request }) => {
    await loginAsStaff(request);

    // Register if not already
    const registerRes = await request.post(`${API}/api/admin/v1/plugins/register`, {
      headers: { Origin: API },
      data: {
        manifest: {
    ...
    const plugin = (await listRes.json()).plugins.find((p: any) => p.manifestId === 'vibress-content-metrics');
    expect(plugin).toBeTruthy();

    // Activate
    const activateRes = await request.post(`${API}/api/admin/v1/plugins/${plugin.id}/activate`, {
      headers: { Origin: API },
    });
    ...
    const setRes = await request.post(`${API}/api/admin/v1/plugins/${plugin.id}/settings`, {
      headers: { Origin: API },
      data: { settings: { webhookUrl: 'https://e2e-secret-endpoint.example.com' } },
    });
    expect(setRes.status()).toBe(200);

    // Secret masked on read — never leaked
    const getRes = await request.get(`${API}/api/admin/v1/plugins/${plugin.id}/settings`);
    ...
    // Deactivate
    const deactivateRes = await request.post(`${API}/api/admin/v1/plugins/${plugin.id}/deactivate`, {
      headers: { Origin: API },
    });
  ...
  test('[Integrations] create with encrypted secret + RBAC 401', async ({ request, browser }) => {
    await loginAsStaff(request);
    ...
    const unauthRes = await freshCtx.request.get(`${API}/api/admin/v1/integrations`);
