...
test.describe('Batch 14 Operations E2E Suite', () => {
  ...
  test('[Settings] staff edits safe setting, audit entry created, secret masked', async ({ request, browser }) => {
    await loginAsStaff(request);

    // Update a public setting
    const updateRes = await request.put(`${API}/api/admin/v1/settings/site/title`, {
      headers: { Origin: API },
    ...
    // Audit entry created
    const auditRes = await request.get(`${API}/api/admin/v1/audit?action=setting.updated`);
    expect(auditRes.status()).toBe(200);
    const events = (await auditRes.json()).events;
    expect(events.length).toBeGreaterThan(0);

    // Secret setting is masked on read
    const setRes = await request.put(`${API}/api/admin/v1/settings/email/smtpHost`, {
      headers: { Origin: API },
      data: { value: 'smtp.secret.example.com' },
    });
    expect(setRes.status()).toBe(200);
    expect(JSON.stringify(await setRes.json())).not.toContain('smtp.secret.example.com');
    ...
    const unauthRes = await freshCtx.request.get(`${API}/api/admin/v1/settings`);
    expect(unauthRes.status()).toBe(401);
    await freshCtx.close();
  });

  test('[Redirects] create works, loop and protected-route blocked', async ({ request }) => {
    await loginAsStaff(request);

    const createRes = await request.post(`${API}/api/admin/v1/redirects`, {
      headers: { Origin: API },
      data: { source: `/e2e-old-${Date.now()}`, destination: '/e2e-new', statusCode: 301 },
    });
  ...
  test('[Export] completes and contains no secret material', async ({ request }) => {
    await loginAsStaff(request);
    ...
    const exportRes = await request.post(`${API}/api/admin/v1/exports`, { headers: { Origin: API } });
    expect(exportRes.status()).toBe(202);
    const job = (await exportRes.json()).job;
    expect(['completed', 'failed']).toContain(job.status);

    if (job.status === 'completed') {
      const artifactRes = await request.get(`${API}/api/admin/v1/import-export-jobs/${job.id}/artifact`);
  ...
  test('[Import] valid import succeeds, invalid fails safely', async ({ request }) => {
    await loginAsStaff(request);
    ...
    const validRes = await request.post(`${API}/api/admin/v1/imports`, {
      ...
      data: {
        ...
        data: { redirects: [{ source: `/imported-e2e-${Date.now()}`, destination: '/imported-target' }] },
      },
    });
  ...
  test('[System] diagnostics permission enforced, search rebuild completes', async ({ request, browser }) => {
    await loginAsStaff(request);

    // Unauthenticated → 401
    const freshCtx = await browser.newContext();
    const unauthRes = await freshCtx.request.get(`${API}/api/admin/v1/system/diagnostics`);
    expect(unauthRes.status()).toBe(401);
    await freshCtx.close();
    ...
    const maintRes = await request.post(`${API}/api/admin/v1/system/maintenance`, {
      headers: { Origin: API },
