...
test.describe('Batch 11 Community E2E Suite', () => {
  ...
  async function ensurePost(request: any): Promise<string> {
    ...
    const createRes = await request.post(`${API}/api/admin/v1/posts`, {
      ...
      data: {
        ...
        content: { schema: 'vibress-studio', version: 1, root: { type: 'root', children: [{ type: 'paragraph', children: [{ type: 'text', text: 'E2E test post for comments', format: 0, mode: 'normal', version: 1 }], direction: null, format: '', indent: 0, version: 1 }] } },
        visibility: 'public',
    ...
    // Publish it
    const publishRes = await request.post(`${API}/api/admin/v1/posts/${postId}/publish`, {
      headers: { Origin: API },
    });
  ...
  test('[Comments + Notifications + Recommendations] Full flow', async ({ page, request }) => {
    ...
    const commentRes = await page.request.post(`${API}/api/members/v1/comments`, {
      headers: { Origin: API },
    ...
    const replyRes = await pageB.request.post(`${API}/api/members/v1/comments`, {
      headers: { Origin: API },
    ...
    const notifRes = await page.request.get(`${API}/api/members/v1/notifications`);
    ...
    const readRes = await page.request.post(`${API}/api/members/v1/notifications/${replyNotif.id}/read`, {
      headers: { Origin: API },
    });
    expect(readRes.status()).toBe(200);

    // Member B cannot edit A's comment (IDOR)
    const editRes = await pageB.request.patch(`${API}/api/members/v1/comments/${commentA}`, {
      headers: { Origin: API },
      data: { body: 'hacked' },
    });
    ...
    const likeRes = await pageB.request.post(`${API}/api/members/v1/comments/${commentA}/like`, {
      headers: { Origin: API },
    });
    ...
    const reportRes = await pageB.request.post(`${API}/api/members/v1/comments/${commentA}/report`, {
      headers: { Origin: API },
      data: { reason: 'spam' },
    });
    ...
    const publicRes = await request.get(`${API}/api/content/v1/posts/${postId}/comments`);
    ...
    const hideRes = await request.post(`${API}/api/admin/v1/comments/${commentA}/hide`, {
      headers: { Origin: API },
    });
    expect(hideRes.status()).toBe(200);

    // Hidden comment disappears from public API
    const publicRes2 = await request.get(`${API}/api/content/v1/posts/${postId}/comments`);
    const publicComments2 = (await publicRes2.json()).comments;
    expect(publicComments2.some((c: any) => c.id === commentA)).toBe(false);

    // Staff restores
    const restoreRes = await request.post(`${API}/api/admin/v1/comments/${commentA}/restore`, {
      headers: { Origin: API },
    });
    expect(restoreRes.status()).toBe(200);

    // Comment reappears
    const publicRes3 = await request.get(`${API}/api/content/v1/posts/${postId}/comments`);
  ...
  test('[Recommendations] click tracking + SSRF blocking', async ({ request }) => {
    await loginAsStaff(request);

    // Create a safe recommendation
    const createRes = await request.post(`${API}/api/admin/v1/recommendations`, {
      headers: { Origin: API },
      data: { url: 'https://example.com/great-resource', title: 'Great Resource' },
    });
    expect(createRes.status()).toBe(201);
    ...
    expect((await listRes.json()).recommendations.some((r: any) => r.id === recId)).toBe(true);

    // Record a click
    const clickRes = await request.post(`${API}/api/content/v1/recommendations/${recId}/click`, {
      data: { sessionId: 'e2e-session' },
    });
    expect(clickRes.status()).toBe(200);

    // Click count visible to admin
    const statsRes = await request.get(`${API}/api/admin/v1/recommendations/${recId}/stats`);
