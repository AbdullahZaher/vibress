...
test.describe('Vibress Full Settings System & Public Runtime Verification', () => {
  ...
  test('1. Admin Settings Navigation & Tabs routing', async ({ page }) => {
    ...
    const tabs = ['General settings', 'Site', 'Membership', 'Growth', 'Advanced'];
  ...
  test('2. General Settings: Title, Description, Locale, Timezone updates & persistence', async ({ request }) => {
    ...
    // Update via API
    const updateRes = await request.put(`${BASE_URL}/api/admin/v1/settings/site/title`, {
      headers: { Origin: BASE_URL },
      data: { value: newTitle },
    });
    expect(updateRes.status()).toBe(200);

    const descRes = await request.put(`${BASE_URL}/api/admin/v1/settings/site/description`, {
      headers: { Origin: BASE_URL },
      data: { value: newDesc },
    });
  ...
  test('3. Site Settings: Navigation & Announcement Bar public enforcement', async ({ request, page }) => {
    await loginAsStaff(request);
    ...
    await request.put(`${BASE_URL}/api/admin/v1/settings/site/announcementEnabled`, {
      headers: { Origin: BASE_URL },
      data: { value: true },
    });

    await request.put(`${BASE_URL}/api/admin/v1/settings/site/announcementText`, {
      headers: { Origin: BASE_URL },
      data: { value: announcementText },
    });

    await request.put(`${BASE_URL}/api/admin/v1/settings/site/announcementUrl`, {
      headers: { Origin: BASE_URL },
  ...
  test('4. Site Privacy: End-to-End public protection barrier & unlock flow', async ({ browser, request }) => {
    await loginAsStaff(request);
    ...
    try {
      ...
      anonPage.on('console', (msg) => console.log('PAGE LOG:', msg.type(), msg.text()));
      anonPage.on('pageerror', (err) => console.log('PAGE ERROR:', err.message));
      anonPage.on('requestfailed', (req) => console.log('REQ FAILED:', req.url(), req.failure()?.errorText));

      await anonPage.goto(`${BASE_URL}/`);
      await anonPage.waitForURL('**/private**', { timeout: 8000 });
      expect(anonPage.url()).toContain('/private');
      await expect(anonPage.locator('h1')).toContainText('This Site is Private');

      // 2b. Attempting forged unsigned cookie vb_site_auth=1 must NOT bypass gate
      await anonContext.addCookies([{ name: 'vb_site_auth', value: '1', domain: 'localhost', path: '/' }]);
      await anonPage.goto(`${BASE_URL}/`);
      await anonPage.waitForURL('**/private**', { timeout: 8000 });
      expect(anonPage.url()).toContain('/private');
      ...
      await expect(anonPage.locator('text=Invalid password')).toBeVisible({ timeout: 5000 });
  ...
  test('5. Growth Settings: SMTP test delivery endpoint', async ({ request }) => {
    await loginAsStaff(request);
    const res = await request.post(`${BASE_URL}/api/admin/v1/settings/test-smtp`, {
      headers: { Origin: BASE_URL },
  ...
  test('6. Advanced Settings: Code Injection in <head> and </body>', async ({ request, page }) => {
    await loginAsStaff(request);
  ...
  test('7. Advanced Settings: Maintenance operation & Integrity checks', async ({ request }) => {
    await loginAsStaff(request);
    // Purge cache maintenance op
    const maintRes = await request.post(`${BASE_URL}/api/admin/v1/system/maintenance`, {
      headers: { Origin: BASE_URL },
    ...
    // Integrity checks
    const integrityRes = await request.get(`${BASE_URL}/api/admin/v1/system/integrity`);
  ...
  test('8. RBAC & Security: Unauthenticated access blocked on settings endpoints', async ({ browser }) => {
    const anonContext = await browser.newContext();

    const unauthSettings = await anonContext.request.get(`${BASE_URL}/api/admin/v1/settings`);
    expect(unauthSettings.status()).toBe(401);

    const unauthAudit = await anonContext.request.get(`${BASE_URL}/api/admin/v1/audit`);
    expect(unauthAudit.status()).toBe(401);

    const unauthDiagnostics = await anonContext.request.get(`${BASE_URL}/api/admin/v1/system/diagnostics`);
  ...
  test('9. Themes Drawer & Danger Zone UI Execution', async ({ page }) => {
    ...
    // Open Themes Drawer
    const manageThemesBtn = page.locator('button', { hasText: 'Change theme' }).first();
    ...
    await expect(page.locator('text=Themes Manager')).toBeVisible({ timeout: 5000 });
    await expect(page.locator('text=Official Theme Registry')).toBeVisible({ timeout: 5000 });
    ...
    const accessZoneBtn = page.locator('button', { hasText: 'Access zone' }).first();
    ...
    await expect(page.locator('text=System Maintenance & Danger Zone')).toBeVisible({ timeout: 5000 });
    const purgeBtn = page.locator('button', { hasText: 'Purge cache' }).first();
    await expect(purgeBtn).toBeVisible({ timeout: 5000 });
    await purgeBtn.click();

    // Verify success feedback
    await expect(page.locator('text=purged successfully')).toBeVisible({ timeout: 5000 });
  });

  test('10. Code Injection RBAC & Security Isolation', async ({ request, playwright }) => {
    await loginAsStaff(request);

    // Superadmin / Owner can update code
    const validUpdate = await request.put(`${BASE_URL}/api/admin/v1/settings/code/headerCode`, {
      headers: { Origin: BASE_URL },
    ...
    const unauthUpdate = await anonRequest.put(`${BASE_URL}/api/admin/v1/settings/code/headerCode`, {
      headers: { Origin: BASE_URL },
