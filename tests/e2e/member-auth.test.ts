...
test.describe('Batch 8 Member Auth E2E Suite', () => {
  ...
  test('[New Member] Full magic-link signup → account', async ({ page, request }) => {
  ...
  test('[Existing Member] Login again keeps same member id, no duplicate', async ({ page, request }) => {
    ...
    const listRes = await request.get(`http://localhost:7777/api/admin/v1/members?search=${encodeURIComponent(email)}`, {
  ...
  test('[Logout] Member cookie revoked, account requires sign-in', async ({ page }) => {
  ...
  test('[Staff + Member Coexistence] Both identities independent', async ({ page }) => {
  ...
  test('[Member Cookie Rejected by Admin API]', async ({ page }) => {
    ...
    const status = await page.evaluate(async () => {
      const res = await fetch('/api/admin/v1/members', { credentials: 'include' });
      return res.status;
    });
    expect(status).toBe(401);
  });

  test('[Disable Member] Session invalidated; re-enable requires new login', async ({ page, request }) => {
    ...
    await loginAsStaff(request);
    const disableRes = await request.post(`http://localhost:7777/api/admin/v1/members/${memberId}/disable`, {
      headers: { Origin: 'http://localhost:7777' },
    });
    ...
    // Re-enable
    await request.post(`http://localhost:7777/api/admin/v1/members/${memberId}/enable`, {
      headers: { Origin: 'http://localhost:7777' },
    });
  ...
  test('[Enumeration] Known vs unknown email identical response', async ({ request }) => {
    const known = 'reuse@example.com';
    const unknown = `nobody-${Date.now()}@example.com`;

    const knownRes = await request.post('http://localhost:7777/api/members/v1/auth/request', {
      headers: { 'Content-Type': 'application/json', Origin: 'http://localhost:7777' },
      data: { email: known },
    });
    const unknownRes = await request.post('http://localhost:7777/api/members/v1/auth/request', {
      headers: { 'Content-Type': 'application/json', Origin: 'http://localhost:7777' },
      data: { email: unknown },
    });
