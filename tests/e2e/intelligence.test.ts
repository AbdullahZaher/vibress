...
test.describe("Batch 13 Intelligence E2E Suite", () => {
  ...
  test("[Automations] trigger → action executes once; deactivated automation does not run", async ({
    page,
    request,
  }) => {
    await loginAsStaff(request);
    ...
    const activateRes = await request.post(
      `${API}/api/admin/v1/automations/${automation.id}/activate`,
      { headers: { Origin: API } },
    );
    ...
    const msg = (mailData.messages || []).find(
    ...
    const memberCookie = String(verifyRes.headers()["set-cookie"] || "").split(
      ";",
    )[0];
    ...
    await waitFor(async () => {
      const runsRes = await request.get(
        `${API}/api/admin/v1/automation-runs?automationId=${automation.id}`,
      );
    ...
    const commentRuns = runs.filter(
      (r: any) => r.triggerEvent === "comment.created",
    );
    ...
    const stepRes = await request.get(
      `${API}/api/admin/v1/automation-runs/${commentRuns[0].id}/steps`,
    );
    ...
    await request.post(
      `${API}/api/admin/v1/automations/${automation.id}/deactivate`,
      { headers: { Origin: API } },
    );
    const runsBefore = (
      await (
  ...
  test("[Automations] durable wait resumes after delay", async ({
    request,
  }) => {
    await loginAsStaff(request);
    ...
    const runRes = await request.post(
      `${API}/api/admin/v1/automations/${automation.id}/run`,
      { headers: { Origin: API } },
    );
