...
test.describe.serial('First-Run Setup Wizard (fresh instance)', () => {
  test('fresh instance: wizard appears, wrong key and validation errors are rejected', async ({ page }) => {
    // Wizard appears
  ...
  test('mobile viewport renders the wizard with labels and keyboard focus', async ({ browser }) => {
    ...
    await expect(page.locator('label[for="setup-key"]')).toHaveText('Setup key');
    await page.close();
  });

  test('complete the wizard end-to-end and enter Admin automatically', async ({ page }) => {
    ...
    await expect(page.locator('body')).toContainText('Create the owner account');
    ...
    await expect(page.locator('body')).toContainText('Analytics', { timeout: 30000 });
  });

  test('reload after installation: no wizard, /admin/setup redirects away', async ({ page }) => {
    await page.goto(`${BASE}/admin`);
  ...
  test('setup stays permanently locked after installation (409 with correct token)', async ({ request }) => {
    const res = await request.post(`${BASE}/api/setup/v1/complete`, {
      ...
      data: {
        ...
        owner: { name: 'Replay', email: `replay-${Date.now()}@example.com`, password: 'StrongE2EPassword123!' },
      },
    });
