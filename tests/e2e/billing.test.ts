...
test.describe('Batch 9 Billing E2E Suite', () => {
  ...
  async function deliverWebhook(request: any, type: string, object: Record<string, unknown>): Promise<void> {
  ...
  test('[Setup] Ensure products/plans/offers exist and webhook secret matches API', async ({ request }) => {
    ...
    const hasRecurring = body.products.some(
      (p: any) => p.plans.some((pl: any) => pl.billingType === 'recurring')
    );
    expect(hasRecurring).toBe(true);
  });

  test('[Paid Member] sign in → plan → checkout → simulated provider completion → active subscription', async ({ page, request }) => {
    // Get the recurring plan
    const catalog = await (await request.get(`${API}/api/content/v1/products`)).json();
    const product = catalog.products.find((p: any) => p.plans.some((pl: any) => pl.billingType === 'recurring'));
    const plan = product.plans.find((pl: any) => pl.billingType === 'recurring');
    ...
    const memberRes = await page.request.get(`${API}/api/members/v1/subscriptions`, { headers: { Origin: API } });
    expect(memberRes.status()).toBe(200);

    // Simulate provider completion: webhook creates the subscription
    const memberId = (await (await page.request.get(`${API}/api/members/v1/me`)).json()).member.id;
    ...
    await expect(page.locator('h1')).toContainText('Your account');
    await expect(page.locator('text=Membership')).toBeVisible({ timeout: 15000 });
    await expect(page.locator(`text=${plan.name}`)).toBeVisible({ timeout: 15000 });
    await expect(page.locator('text=Renews')).toBeVisible({ timeout: 15000 });
  });

  test('[Cancel] active subscription → cancel → scheduled cancellation displayed', async ({ page, request }) => {
    ...
    await expect(page.locator('text=Cancellation scheduled')).toBeVisible({ timeout: 15000 });
  });

  test('[Resume] scheduled cancellation → resume → active state restored', async ({ page, request }) => {
    ...
    await expect(page.locator('text=Cancellation scheduled')).not.toBeVisible({ timeout: 15000 });
  });

  test('[Payment failure] provider failure event → past_due state reflected in Portal', async ({ page, request }) => {
  ...
  test('[Duplicate webhook] replay of the same event does not create duplicates', async ({ page, request }) => {
    ...
    const subs = await (await page.request.get(`${API}/api/members/v1/subscriptions`)).json();
    const matches = subs.subscriptions.filter((s: any) => s.planName === plan.name);
    expect(matches.length).toBe(1);
  });

  test('[Staff] Staff with permission views member subscription safely', async ({ request }) => {
  ...
  test('[Member IDOR] Member A cannot view or cancel Member B subscription', async ({ page, browser }) => {
    ...
    const memberB = (await (await pageB.request.get(`${API}/api/members/v1/me`)).json()).member.id;

    // Create an active subscription for B via webhook
    const catalog = await (await page.request.get(`${API}/api/content/v1/products`)).json();
    const product = catalog.products.find((p: any) => p.plans.some((pl: any) => pl.billingType === 'recurring'));
    const plan = product.plans.find((pl: any) => pl.billingType === 'recurring');
    ...
    const bSubs = await (await pageB.request.get(`${API}/api/members/v1/subscriptions`)).json();
    ...
    const aView = await page.request.get(`${API}/api/members/v1/subscriptions/${bSubId}`);
    expect(aView.status()).toBe(404);

    // A cannot cancel B's subscription
    const aCancel = await page.request.post(`${API}/api/members/v1/subscriptions/${bSubId}/cancel`, {
      headers: { Origin: API },
    });
    expect(aCancel.status()).toBe(404);

    // B's subscription is still active and unchanged
    const bAfter = await (await pageB.request.get(`${API}/api/members/v1/subscriptions`)).json();
