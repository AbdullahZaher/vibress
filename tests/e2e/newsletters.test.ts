...
test.describe('Batch 10 Newsletter & Email E2E Suite', () => {
  async function getLatestMail(to: string): Promise<{ subject: string; html: string; text: string } | null> {
    const res = await fetch('http://127.0.0.1:8025/api/v1/messages');
    const data = await res.json();
    const msg = (data.messages || []).find((m: any) => m.To?.[0]?.Address === to);
    if (!msg) return null;
  ...
  async function deliverEmailWebhook(request: any, event: Record<string, unknown>): Promise<number> {
    ...
    const res = await request.post(`${API}/api/webhooks/v1/email/smtp`, {
      headers: { 'Content-Type': 'application/json', 'x-email-signature': signEmailEvent(payload) },
      data: JSON.parse(payload),
    });
    return res.status();
  }

  test('[Setup] Seed newsletter + product catalog exists', async ({ request }) => {
  ...
  test('[Full flow] create → subscribe → send-now → worker delivers → webhook updates state', async ({ page, request }) => {
    await loginAsStaff(request);
    ...
    const subRes = await page.request.put(`${API}/api/members/v1/newsletter-preferences`, {
      headers: { Origin: API },
    ...
    const testRes = await request.post(`${API}/api/admin/v1/newsletter-test-email`, {
      headers: { Origin: API },
      data: {
        newsletterId,
        ...
        content: {
          ...
          version: 1,
          root: { type: 'root', children: [{ type: 'paragraph', children: [{ type: 'text', text: 'Test body', format: 0, mode: 'normal', version: 1 }], direction: null, format: '', indent: 0, version: 1 }] },
        },
    ...
    const sendRes = await request.post(`${API}/api/admin/v1/newsletter-sends`, {
      ...
      data: {
        ...
        content: {
          schema: 'vibress-studio',
          version: 1,
          root: { type: 'root', children: [{ type: 'paragraph', children: [{ type: 'text', text: 'Campaign body', format: 0, mode: 'normal', version: 1 }], direction: null, format: '', indent: 0, version: 1 }] },
        },
        audience: { filter: 'all' },
        sendNow: true,
      },
    ...
    let campaignMail: { subject: string; html: string; text: string } | null = null;
    for (let i = 0; i < 30; i++) {
    ...
    const unsubMatch = campaignMail!.html.match(/href="([^"]*\/portal\/unsubscribe\?t=[^"]+)"/);
    expect(unsubMatch).toBeTruthy();

    // 6. Send state reflects delivery
    const sendStatusRes = await request.get(`${API}/api/admin/v1/newsletter-sends/${sendId}`);
    ...
    const mailDetail = await (await fetch(`http://127.0.0.1:8025/api/v1/messages`)).json();
    const msg = (mailDetail.messages || []).find((m: any) => m.To?.[0]?.Address === memberEmail && m.Subject === `E2E Campaign ${suffix}`);
    const msgDetail = await (await fetch(`http://127.0.0.1:8025/api/v1/message/${msg.ID}`)).json();
    ...
    const send2Res = await request.post(`${API}/api/admin/v1/newsletter-sends`, {
      headers: { Origin: API },
      data: {
        newsletterId,
  ...
  test('[Hard bounce] webhook bounce suppresses the address and excludes from future sends', async ({ page, request }) => {
    await loginAsStaff(request);
    const suffix = `${Date.now()}`;
    ...
    const createRes = await request.post(`${API}/api/admin/v1/newsletters`, {
      headers: { Origin: API },
      data: { key: `e2e-bounce-${suffix}`, name: `Bounce ${suffix}`, senderName: 'V', senderEmail: 'news@vibress.test' },
    });
    const newsletterId = (await createRes.json()).newsletter.id;
    const memberEmail = `e2e-bounce-${suffix}@example.com`;
    await signupMember(page, memberEmail);
    await page.request.put(`${API}/api/members/v1/newsletter-preferences`, { headers: { Origin: API }, data: { newsletterId, subscribed: true } });
    ...
    for (let i = 0; i < 30; i++) {
      const res = await fetch('http://127.0.0.1:8025/api/v1/messages');
      const data = await res.json();
      const msg = (data.messages || []).find((m: any) => m.To?.[0]?.Address === memberEmail && m.Subject === `Bounce Campaign ${suffix}`);
      if (msg) {
        const detail = await (await fetch(`http://127.0.0.1:8025/api/v1/message/${msg.ID}`)).json();
    ...
    const suppression = suppBody.suppressions.find((s: any) => s.email === memberEmail);
  ...
  test('[Schedule] scheduled send fires at due time via durable scheduler', async ({ page, request }) => {
    await loginAsStaff(request);
    const suffix = `${Date.now()}`;
    const createRes = await request.post(`${API}/api/admin/v1/newsletters`, {
      headers: { Origin: API },
      data: { key: `e2e-sched-${suffix}`, name: `Sched ${suffix}`, senderName: 'V', senderEmail: 'news@vibress.test' },
    });
    const newsletterId = (await createRes.json()).newsletter.id;
    ...
    for (let i = 0; i < 30; i++) {
      ...
      if (mail?.subject === `Scheduled ${suffix}`) { delivered = true; break; }
      await page.waitForTimeout(1000);
    }
    expect(delivered).toBe(true);
  });

  test('[Security] header injection blocked in newsletter names/senders', async ({ request }) => {
    await loginAsStaff(request);
    const suffix = `${Date.now()}`;
    const createRes = await request.post(`${API}/api/admin/v1/newsletters`, {
      headers: { Origin: API },
    ...
    const okRes = await request.post(`${API}/api/admin/v1/newsletters`, {
      headers: { Origin: API },
      data: { key: `e2e-ok-${suffix}`, name: 'Safe', senderName: 'Vibress', senderEmail: 'news@vibress.test' },
    });
