...
test.describe('Batch 4 Media E2E — Delete Protection + Gallery', () => {
  ...
  test('[Delete Protection] referenced image → MEDIA_IN_USE in Media Library', async ({ page, request }) => {
    ...
    const upRes = await request.post('http://localhost:7777/api/admin/v1/media', { headers: { Origin: 'http://localhost:7777' }, multipart: { file: { name: 'dp-final.png', mimeType: 'image/png', buffer: png } } });
    ...
    if (await img.isVisible({ timeout: 8000 }).catch(() => false)) {
      ...
      if (await page.locator('button:has-text("Delete Asset")').isVisible({ timeout: 3000 }).catch(() => false)) {
        page.on('dialog', (d) => d.accept());
        await page.locator('button:has-text("Delete Asset")').click();
        await page.waitForTimeout(1500);
        expect(await page.locator('text=/in use|Cannot delete/i').isVisible({ timeout: 5000 })).toBe(true);
        expect(await page.locator('img[alt="dp-final.png"]').first().isVisible({ timeout: 3000 })).toBe(true);
      }
    }

    // Backend verification as ground truth
    const delRes = await request.delete(`http://localhost:7777/api/admin/v1/media/${assetId}`, { headers: { Origin: 'http://localhost:7777' } });
    expect(delRes.status()).toBe(409);
    expect((await delRes.json()).errors[0].code).toBe('MEDIA_IN_USE');
  });

  test('[Gallery] Insert Card→Picker→2 images→Save→verify refs', async ({ page, request }) => {
    ...
    const png = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==', 'base64');
    await request.post('http://localhost:7777/api/admin/v1/media', { headers: { Origin: 'http://localhost:7777' }, multipart: { file: { name: 'gal-final-a.png', mimeType: 'image/png', buffer: png } } });
    ...
    const result = await page.evaluate(async () => {
      try {
        const res = await fetch('/api/admin/v1/media?search=gal-final&limit=10', { credentials: 'include' });
        const data = await res.json();
        let total = 0;
        if (data.items && Array.isArray(data.items)) {
          for (const item of data.items) {
            const r = await fetch(`/api/admin/v1/media/${item.id}/references`, { credentials: 'include' });
            if (r.ok) {
    ...
    expect(result.total, `Gallery refs check: ${JSON.stringify(result)}`).toBeGreaterThanOrEqual(1);
  });
});
