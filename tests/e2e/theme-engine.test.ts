...
test.describe("Batch 7 Theme Engine E2E Suite", () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    const meRes = await request.get(
      "http://localhost:7777/api/admin/v1/auth/me",
      {
        headers: { Origin: "http://localhost:7777" },
      },
    );
          ...
          content: {
            ...
            root: {
              ...
              children: [
                {
                  type: "paragraph",
                  children: [{ type: "text", text: "Theme engine test body." }],
                },
              ],
            },
          },
    ...
    await request.post(
      `http://localhost:7777/api/admin/v1/posts/${postData.post.id}/publish`,
      {
        headers: { Origin: "http://localhost:7777" },
      },
    );
  ...
  test("[Default Theme] Public post renders with default layout and content", async ({
    page,
  }) => {
    ...
    await expect(page.locator("h1.article-title")).toContainText(
      "Batch 7 Theme Test Post",
    );
    await expect(page.locator(".studio-html-content")).toContainText(
      "Theme engine test body.",
    );
  });

  test("[Preview] Preview Minimal without activating", async ({
    page,
    request,
  }) => {
    await loginOwner(request);
    ...
    const previewRes = await request.post(
      "http://localhost:7777/api/admin/v1/themes/vibress-minimal/preview",
      {
        headers: { Origin: "http://localhost:7777" },
      },
    );
    ...
    const res = await page.goto(
      `http://localhost:7777/preview/${token}/posts/${postSlug}`,
    );
    expect(res?.status()).toBe(200);
    await expect(page.locator('[data-theme="vibress-minimal"]')).toBeAttached();
    await expect(page.locator("h1.gh-article-title")).toContainText(
      "Batch 7 Theme Test Post",
    );
  ...
  test("[Preview Expired] Invalid/expired token redirects safely", async ({
    page,
  }) => {
    const res = await page.goto(
      "http://localhost:7777/preview/invalidtoken123/posts/x",
    );
    expect(res?.status()).toBe(200);
    await expect(page.locator('[data-theme="vibress-default"]')).toBeAttached();
  });

  test("[Theme Switch] Activate Minimal → public renders Minimal with same content", async ({
    page,
    request,
  }) => {
    await loginOwner(request);
    ...
    const actRes = await request.post(
      "http://localhost:7777/api/admin/v1/themes/vibress-minimal/activate",
      {
        headers: { Origin: "http://localhost:7777" },
      },
    );
  ...
  test("[Settings Update] Change setting at runtime reflects on public site", async ({
    page,
    request,
  }) => {
    await loginOwner(request);
    ...
    const settingsRes = await request.patch(
      "http://localhost:7777/api/admin/v1/themes/vibress-minimal/settings",
      {
        headers: {
          "Content-Type": "application/json",
          Origin: "http://localhost:7777",
  ...
  test("[Invalid Activation Atomicity] Failed activation keeps current theme", async ({
    request,
  }) => {
    await loginOwner(request);
    ...
    const badRes = await request.post(
      "http://localhost:7777/api/admin/v1/themes/node%3Afs/activate",
      {
        headers: { Origin: "http://localhost:7777" },
      },
    );
    ...
    const activeRes = await request.get(
      "http://localhost:7777/api/admin/v1/themes/active",
      {
        headers: { Origin: "http://localhost:7777" },
      },
    );
  ...
  test("[Visibility] Drafts stay hidden after theme switch", async ({
    page,
    request,
  }) => {
    await loginOwner(request);
