...
test.describe('Batch 6 Public Web, SEO & Discovery E2E Suite', () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    const tagRes = await request.post('http://localhost:7777/api/admin/v1/tags', {
      headers: { 'Content-Type': 'application/json', Origin: 'http://localhost:7777' },
      data: { name: `E2E Public Tag ${Date.now()}`, slug: `e2e-tag-${Date.now()}` },
    });
        ...
        content: {
          ...
          root: {
            ...
            children: [
              {
                type: 'paragraph',
                children: [{ type: 'text', text: 'Public E2E content body in Vibress Studio.' }],
              },
            ],
          },
        },
    ...
    await request.post(`http://localhost:7777/api/admin/v1/posts/${postId}/publish`, {
    ...
    const schedRes = await request.post('http://localhost:7777/api/admin/v1/posts', {
      headers: { 'Content-Type': 'application/json', Origin: 'http://localhost:7777' },
      data: {
    ...
    await request.post(`http://localhost:7777/api/admin/v1/posts/${schedData.post.id}/schedule`, {
      headers: { 'Content-Type': 'application/json', Origin: 'http://localhost:7777' },
      data: { scheduledAt: futureDate },
    });

    // 6. Create & Publish Page
    const pageRes = await request.post('http://localhost:7777/api/admin/v1/pages', {
      headers: { 'Content-Type': 'application/json', Origin: 'http://localhost:7777' },
      data: {
        ...
        content: {
          ...
          root: {
            type: 'root',
            children: [{ type: 'paragraph', children: [{ type: 'text', text: 'Terms of service page content.' }] }],
          },
        },
        primaryAuthorId: ownerId,
      },
    ...
    await request.post(`http://localhost:7777/api/admin/v1/pages/${pageData.page.id}/publish`, {
      headers: { Origin: 'http://localhost:7777' },
    });

    // 7. Create Post for Unpublish Test
    const unpubRes = await request.post('http://localhost:7777/api/admin/v1/posts', {
      headers: { 'Content-Type': 'application/json', Origin: 'http://localhost:7777' },
      data: {
    ...
    await request.post(`http://localhost:7777/api/admin/v1/posts/${unpubPostId}/publish`, {
      headers: { Origin: 'http://localhost:7777' },
    });
  });

  test('[Public Post] Visit published post page & verify SSR HTML, title, content', async ({ page }) => {
    const res = await page.goto(`http://localhost:7777/posts/${postSlug}`);
    expect(res?.status()).toBe(200);

    await expect(page.locator('h1.article-title')).toContainText('E2E Public Published Post Title');
    await expect(page.locator('.studio-html-content')).toContainText('Public E2E content body in Vibress Studio.');
  });

  test('[SEO Metadata] Inspect document title, meta description, canonical, OG, Twitter & JSON-LD', async ({ page }) => {
    ...
    await expect(metaDesc).toHaveAttribute('content', 'SEO Description Override for Post');

    // Canonical link
    const canonical = page.locator('link[rel="canonical"]');
    await expect(canonical).toHaveAttribute('href', `http://localhost:7777/posts/${postSlug}`);

    // Open Graph & Twitter
    const ogTitle = page.locator('meta[property="og:title"]');
    await expect(ogTitle).toHaveAttribute('content', 'SEO Title Override for Post');
    ...
    expect(parsedLd.mainEntityOfPage['@id']).toBe(`http://localhost:7777/posts/${postSlug}`);
  });

  test('[Draft & Scheduled 404] Draft and scheduled future posts return 404 on Web', async ({ page }) => {
    const draftRes = await page.goto(`http://localhost:7777/posts/${draftSlug}`);
    expect(draftRes?.status()).toBe(404);

    const schedRes = await page.goto(`http://localhost:7777/posts/${schedSlug}`);
    expect(schedRes?.status()).toBe(404);
  });

  test('[Unpublish Flow] Published post returns 200 then 404 after unpublish', async ({ request }) => {
    const initRes = await request.get(`http://localhost:7777/posts/${unpubPostSlug}`);
    ...
    // Unpublish via API
    const unpubApiRes = await request.post(`http://localhost:7777/api/admin/v1/posts/${unpubPostId}/unpublish`, {
      headers: { Origin: 'http://localhost:7777' },
    });
    expect(unpubApiRes.status()).toBe(200);

    const unpubRes = await request.get(`http://localhost:7777/posts/${unpubPostSlug}`);
    expect(unpubRes.status()).toBe(404);
  });

  test('[Public Page] Visit published page & verify title and content', async ({ page }) => {
    const res = await page.goto(`http://localhost:7777/pages/${pageSlug}`);
    expect(res?.status()).toBe(200);

    await expect(page.locator('h1.article-title')).toContainText('E2E Published Terms Page');
    await expect(page.locator('.studio-html-content')).toContainText('Terms of service page content.');
  });

  test('[Tag Archive] Visit tag page & verify tagged published posts list', async ({ page }) => {
    const res = await page.goto(`http://localhost:7777/tags/${tagSlug}`);
    expect(res?.status()).toBe(200);

    await expect(page.locator('h1.article-title')).toContainText('E2E Public Tag');
    await expect(page.locator('.posts-list')).toContainText('E2E Public Published Post Title');
  });

  test('[Author Archive] Visit author page & verify authored published posts list', async ({ page }) => {
  ...
  test('[Discovery Routes] Verify sitemap.xml, robots.txt, and rss.xml', async ({ request }) => {
    // 1. Sitemap XML
    ...
    expect(sitemapXml).toContain(`<loc>http://localhost:7777/posts/${postSlug}</loc>`);
    expect(sitemapXml).toContain(`<loc>http://localhost:7777/pages/${pageSlug}</loc>`);
    ...
    expect(rssXml).toContain(`<link>http://localhost:7777/posts/${postSlug}</link>`);
