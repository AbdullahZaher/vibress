...
test.describe("Admin Content & Vibress Studio E2E Flow", () => {
  test("Complete Post creation, Studio editing, card insertion, publishing, and unpublishing flow", async ({
    page,
  }) => {
    // 1. Login as owner
    ...
    await page.waitForURL((url) => url.pathname.startsWith("/admin/posts/") && !url.pathname.endsWith("/new"));
  ...
  test("Page creation with Studio editor flow", async ({ page }) => {
    ...
    await page.click('button:has-text("Create Page"), button:has-text("Save Draft")');
    await page.waitForURL((url) => url.pathname.startsWith("/admin/pages/") && !url.pathname.endsWith("/new"));
