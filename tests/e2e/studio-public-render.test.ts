...
const cardList = [
  ...
  'Bookmark', 'Embed', 'Button', 'Callout', 'Toggle',
  'Markdown', 'Html', 'Divider',
] as const;
...
async function uploadAsset(request: Page['request'], name: string, mimeType: string, buffer: Buffer) {
...
async function pickAsset(page: Page, title: string) {
  ...
  const asset = modal.locator(`div[title="${title}"], img[alt="${title}"]`).first();
...
test.describe('Studio public rendering', () => {
  ...
  test('[Post] all 13 Studio cards render on the published public page', async ({ page, request }) => {
    await request.post(`${BASE}/api/admin/v1/auth/login`, {
      headers: { 'Content-Type': 'application/json', Origin: 'http://localhost:7777' },
      data: { email: 'owner@example.com', password: 'OwnerPass123!' },
    });
    ...
    await page.waitForFunction(
      () => Array.from(document.querySelectorAll('div.fixed.inset-0 img[alt]:not([alt=""])')).length >= 2,
      null,
      { timeout: 10000 }
    );
    ...
    await page.locator('button:has-text("Asset(s)")').first().click({ timeout: 5000 });
    await page.waitForTimeout(800);
    ...
    await page.locator('input[placeholder="https://..."]').last().fill('https://example.com/bookmark');
    ...
    await closePicker(page);
    await page.locator('input[placeholder="https://..."]').last().fill('https://www.youtube.com/watch?v=abc123def');
    await page.locator('button:has-text("Save")').last().click();
    await page.waitForTimeout(400);
    ...
    await closePicker(page);
    await page.locator('textarea[placeholder="Type callout text here..."]').last().fill('Heads up — important note');
    await page.waitForTimeout(300);

    // --- toggle (heading + content textareas are rendered unconditionally) ---
    await insertCard(page, 'Toggle');
    await closePicker(page);
    await page.locator('textarea[placeholder="Toggle heading..."]').last().fill('Common questions');
    ...
    await expect(page.locator('.kg-bookmark-card a')).toHaveAttribute('href', /example\.com\/bookmark/);
    await expect(page.locator('.kg-bookmark-card')).toContainText('example.com');
    const embedIframe = page.locator('.kg-embed-card iframe');
    await expect(embedIframe).toBeVisible();
    await expect(embedIframe).toHaveAttribute('src', /youtube-nocookie\.com\/embed\/abc123def/);
    await expect(page.locator('.kg-button-card a')).toContainText('Buy now');
    await expect(page.locator('.kg-callout-card')).toContainText('important note');
    await expect(page.locator('details.kg-toggle-card summary')).toContainText('Common questions');
    await expect(page.locator('.kg-toggle-card')).toContainText('This is the answer.');
    await expect(page.locator('.studio-html-content strong:has-text("bold text")')).toBeVisible();
    await expect(page.locator('.studio-html-content div.widget')).toContainText('safe');
    await expect(page.locator('hr')).toHaveCount(1);
  });

  test('[Page] all 13 cards render on the published public page', async ({ page, request }) => {
    await request.post(`${BASE}/api/admin/v1/auth/login`, {
      headers: { 'Content-Type': 'application/json', Origin: 'http://localhost:7777' },
      data: { email: 'owner@example.com', password: 'OwnerPass123!' },
    });
    const me = await (await request.get(`${BASE}/api/admin/v1/auth/me`, { headers: { Origin: BASE } })).json();

    const cards = [
      { type: 'studio-card', cardType: 'image', cardData: { src: `${BASE}/content/media/e2e-img.png`, alt: 'p image' }, version: 1 },
      { type: 'studio-card', cardType: 'gallery', cardData: { images: [{ src: `${BASE}/g1.png`, alt: 'one' }, { src: `${BASE}/g2.png`, alt: 'two' }] }, version: 1 },
      { type: 'studio-card', cardType: 'video', cardData: { src: `${BASE}/v.mp4`, caption: 'vid' }, version: 1 },
      { type: 'studio-card', cardType: 'audio', cardData: { src: `${BASE}/a.mp3`, title: 'pod' }, version: 1 },
      { type: 'studio-card', cardType: 'file', cardData: { src: `${BASE}/f.pdf`, fileName: 'report.pdf', fileSize: '1.2 MB' }, version: 1 },
      { type: 'studio-card', cardType: 'bookmark', cardData: { url: 'https://example.com/a', title: 'Example', description: 'desc' }, version: 1 },
      { type: 'studio-card', cardType: 'embed', cardData: { url: 'https://www.youtube.com/watch?v=xyz789', embedType: 'video' }, version: 1 },
      { type: 'studio-card', cardType: 'button', cardData: { text: 'Sign up', url: 'https://store.example/signup' }, version: 1 },
      { type: 'studio-card', cardType: 'callout', cardData: { text: 'Page note', emoji: '💡' }, version: 1 },
      { type: 'studio-card', cardType: 'toggle', cardData: { heading: 'Page FAQ', content: 'Page answer' }, version: 1 },
      { type: 'studio-card', cardType: 'markdown', cardData: { markdown: '**page bold**' }, version: 1 },
      { type: 'studio-card', cardType: 'html', cardData: { html: '<p class="page-safe">page safe html</p>' }, version: 1 },
    ...
    const created = await request.post(`${BASE}/api/admin/v1/pages`, {
      ...
      data: {
        ...
        content: { schema: 'vibress-studio', version: 1, root: { type: 'root', children: cards } },
      },
    });
    expect(created.status()).toBe(201);
    pageSlug = (await created.json()).page.slug;
    const pageId = (await created.json()).page.id;
    const pub = await request.post(`${BASE}/api/admin/v1/pages/${pageId}/publish`, {
      headers: { 'Content-Type': 'application/json', Origin: 'http://localhost:7777' },
      data: {},
    });
    ...
    await expect(page.locator('.kg-bookmark-card a')).toHaveAttribute('href', /example\.com\/a/);
    await expect(page.locator('.kg-embed-card iframe')).toHaveAttribute('src', /youtube-nocookie\.com\/embed\/xyz789/);
    ...
    await expect(page.locator('details.kg-toggle-card summary')).toContainText('Page FAQ');
    await expect(page.locator('.studio-html-content strong:has-text("page bold")')).toBeVisible();
    await expect(page.locator('p.page-safe')).toContainText('page safe html');
    await expect(page.locator('hr')).toHaveCount(1);
  });

  test('[Security] malicious HTML card is sanitized on the public page', async ({ page, request }) => {
    await request.post(`${BASE}/api/admin/v1/auth/login`, {
      headers: { 'Content-Type': 'application/json', Origin: 'http://localhost:7777' },
      data: { email: 'owner@example.com', password: 'OwnerPass123!' },
    });
    ...
    const payload = '<script>window.__xssE2E=1</script><img src=x onerror="window.__xssE2E=2"><a href="javascript:window.__xssE2E=3">bad</a><p class="safe-text">safe content survives</p>';
    const created = await request.post(`${BASE}/api/admin/v1/posts`, {
      ...
      data: {
        ...
        content: {
          ...
          version: 1,
          root: { type: 'root', children: [{ type: 'studio-card', cardType: 'html', cardData: { html: payload }, version: 1 }] },
        },
      },
    });
    expect(created.status()).toBe(201);
    ...
    await expect(page.locator('p.safe-text')).toContainText('safe content survives');
    const html = await page.locator('.vb-content').innerHTML();
    expect(html).not.toContain('<script');
    expect(html).not.toContain('onerror');
    expect(html).not.toContain('javascript:');
    // No script executed in the page.
    const executed = await page.evaluate(() => (window as unknown as Record<string, unknown>).__xssE2E);
