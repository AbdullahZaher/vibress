...
async function verifySiteAuth(token: string | undefined | null): Promise<boolean> {
  ...
  if (!timestampStr || !signature || !/^[a-f0-9]{64}$/i.test(signature)) return false;

  const timestamp = parseInt(timestampStr, 10);
  if (Number.isNaN(timestamp) || !/^\d+$/.test(timestampStr)) return false;

  const now = Date.now();
  if (timestamp > now + 300_000 || now - timestamp > 30 * 24 * 60 * 60 * 1000) {
    return false;
  }

  try {
    const secret = process.env.VIBRESS_ENCRYPTION_KEY || 'vibress-site-privacy-secret';
    ...
    const sigBuffer = await crypto.subtle.sign('HMAC', key, enc.encode(`v1:${timestampStr}`));
  ...
  if (isProtectedPath) {
    const hasSiteAuth = await verifySiteAuth(request.cookies.get('vb_site_auth')?.value);
    if (!hasSiteAuth) {
      try {
        const siteRes = await fetch(`${API_BASE}/api/content/v1/site`, { cache: 'no-store' });
        if (siteRes.ok) {
