...
function getApiBaseUrl(): string {
  ...
  console.warn(
    "[ContentApiClient] No API_URL or API_PORT set — falling back to http://127.0.0.1:7780 (may not work inside containers)",
  );
...
async function fetchContentApi<T>(
  ...
  try {
    ...
    console.error(
      `[ContentApiClient Error] Network failure fetching ${path}:`,
      error,
    );
    return null;
  }
...
export const ContentApiClient = {
  ...
  async getAuthors(): Promise<PublicAuthorDto[]> {
    const data = await fetchContentApi<{ authors: PublicAuthorDto[] }>(
      "/authors",
    );
    return data?.authors || [];
  },
