...
export function PostCard({ post, isFeatured = false }: PostCardProps) {
  ...
  const imageUrl = post.featureImage?.url || 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=2564&auto=format&fit=crop';

  return (
    <article className={`${postClass} u-shadow`} style={primaryTag?.accentColor ? { '--tag-color': primaryTag.accentColor } as React.CSSProperties : {}}>
      <a className="post-link" href={`/posts/${post.slug}`}>
        ...
        <div className="post-wrapper">
          <header className="post-header">
            {primaryTag && (
              <span className="post-tag">{primaryTag.name}</span>
            )}
            ...
            {authors.map((author, i) => {
              ...
              return (
                <span key={i} className="post-author-link">
                  <img className="post-author-image" src={author?.profileImage || fallbackAvatar} alt={author?.name} loading="lazy" />
                </span>
              );
            })}
          </div>
