...
export class AutomationRunnerWorker {
  ...
  constructor(executor: { execute(action: AutomationAction, context: { runId: string; stepIndex: number; memberId?: string | null; eventPayload: Record<string, unknown> | null }): Promise<{ result: Record<string, unknown> }> }) {
    ...
    this.automationsService = new AutomationsService(repo, {
      enqueueRun: (runId) => this.enqueueRun(runId),
      enqueueDelayedStep: (runId, stepIndex, delayMs) => this.enqueueDelayed(runId, stepIndex, delayMs),
    }, executor);
  }
  ...
  async start(): Promise<void> {
    ...
    this.delayedWorker.on('failed', (job, err) => {
      console.error(`[AutomationRunner] Delayed job ${job?.id} failed:`, err.message);
    });
  }
