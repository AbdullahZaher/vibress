...
export async function listAutomationsApi(): Promise<{
...
export async function createAutomationApi(data: {
  ...
  conditions?: unknown[];
  actions: unknown[];
}): Promise<{ automation: AdminAutomation }> {
  return apiRequest("/automations", {
    method: "POST",
...
export async function activateAutomationApi(
  id: string,
): Promise<{ automation: AdminAutomation }> {
  return apiRequest(`/automations/${id}/activate`, { method: "POST" });
}

export async function deactivateAutomationApi(
  id: string,
): Promise<{ automation: AdminAutomation }> {
  return apiRequest(`/automations/${id}/deactivate`, { method: "POST" });
}

export async function runAutomationApi(
  id: string,
): Promise<{ run: AdminAutomationRun }> {
...
): Promise<{ runs: AdminAutomationRun[]; total: number }> {
  const query = new URLSearchParams();
