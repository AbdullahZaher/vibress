      ...
      <Card className="p-6 bg-transparent border-border shadow-2xs flex items-center justify-between">
        <div>
          <h3 className="font-bold text-sm text-foreground">
            Full Publication Data Backup
          </h3>
          <p className="text-xs text-muted-foreground pt-0.5">
            Export all posts, pages, tags, members, and configuration settings
            in JSON format.
          </p>
        </div>
        <Button
          ...
          <TableBody>
            {jobs.length === 0 ? (
              ...
              jobs.map((j) => (
                ...
                >
                  <TableCell className="pl-6 font-semibold text-foreground capitalize">
                    {j.type}
                  </TableCell>
                  <TableCell>
                    <Badge
