...
export const PageEditor: React.FC<PageEditorProps> = ({
  ...
  useEffect(() => {
    if (pageId) {
      setLoading(true);
      apiRequest<{ page: { id: string; title: string; slug: string; excerpt?: string; metaTitle?: string; metaDescription?: string; canonicalUrl?: string; content?: unknown; status: string; version: number } }>(`/pages/${pageId}`)
        .then((res) => {
          const p = res.page;
          setTitle(p.title);
          setSlug(p.slug);
        ...
        .catch((err: { message?: string }) => setErrorMsg(err.message || 'Failed to load page'))
  ...
  const savePage = useCallback(
    async (isAutosave = false) => {
      ...
      try {
        if (pageId) {
          const res = await apiRequest<{ page: { version: number } }>(`/pages/${pageId}`, {
            method: 'PUT',
    ...
    [title, slug, excerpt, metaTitle, metaDescription, canonicalUrl, studioDoc, currentUserId, pageId, version, autosaveState, onNavigate]
  );
  ...
  const handlePublishToggle = async () => {
    ...
    try {
      const endpoint = status === 'published' ? `/pages/${pageId}/unpublish` : `/pages/${pageId}/publish`;
      const res = await apiRequest<{ page: { status: string; version: number } }>(endpoint, { method: 'POST' });
