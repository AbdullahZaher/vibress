...
export const MetadataSocialCard: React.FC<MetadataSocialCardProps> = ({
  ...
  const siteDesc = description || 'The publication description that will appear on Google search results, X / Twitter, and Facebook.';
  ...
  return (
    <SettingsCard id="general-social" isHighlighted={isHighlighted}>
      <SettingsCardRow
        ...
        currentValue={<span className="text-xs text-muted-foreground font-medium">SERP & Social Previews</span>}
      ...
      {isExpanded && (
        <div className="border-t border-border/40 p-5 bg-muted/15 space-y-4 animate-in slide-in-from-top-2 duration-150">
          <div className="flex items-center justify-between">
            <h5 className="text-xs font-semibold text-foreground">Live Search & Social Previews</h5>
            <Tabs value={activePreview} onValueChange={setActivePreview} className="w-auto">
              <TabsList className="bg-card border border-border/60 p-0.5 h-8">
                <TabsTrigger value="google" className="text-[11px] px-3 py-1 gap-1.5 cursor-pointer">
                  <Search className="h-3 w-3" /> Google Search
                </TabsTrigger>
                <TabsTrigger value="twitter" className="text-[11px] px-3 py-1 gap-1.5 cursor-pointer">
                  X / Twitter
                </TabsTrigger>
                <TabsTrigger value="facebook" className="text-[11px] px-3 py-1 gap-1.5 cursor-pointer">
                  Facebook
                </TabsTrigger>
              </TabsList>
            </Tabs>
            ...
            <div className="p-5 rounded-xl border border-border/80 bg-card font-sans space-y-2 max-w-xl shadow-xs">
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                <div className="flex items-center gap-1 text-[11px]">
                  <span className="font-medium text-foreground">vibress.org</span>
                  <span>›</span>
                  <span>home</span>
                </div>
              </div>
          ...
          {activePreview === 'twitter' && (
            <div className="rounded-2xl border border-border/80 overflow-hidden bg-card max-w-md shadow-xs">
              <div className="h-36 bg-gradient-to-br from-neutral-900 via-neutral-800 to-indigo-950 flex items-center justify-center p-4">
                <span className="text-sm font-bold text-neutral-200">{siteTitle}</span>
              </div>
              <div className="p-3.5 space-y-1 bg-background/50 border-t border-border/50">
                <p className="text-[10px] text-muted-foreground uppercase font-mono tracking-wider">vibress.org</p>
                <h4 className="text-xs font-bold text-foreground line-clamp-1">{siteTitle}</h4>
                <p className="text-[11px] text-muted-foreground line-clamp-2">{siteDesc}</p>
              </div>
            </div>
          )}
          ...
          {activePreview === 'facebook' && (
            <div className="rounded-xl border border-border/80 overflow-hidden bg-card max-w-md shadow-xs">
              ...
              <div className="p-3.5 space-y-1 bg-background/50 border-t border-border/50">
                <p className="text-[10px] text-muted-foreground uppercase font-semibold">VIBRESS.ORG</p>
                <h4 className="text-xs font-bold text-foreground line-clamp-1">{siteTitle}</h4>
                <p className="text-[11px] text-muted-foreground line-clamp-2">{siteDesc}</p>
              </div>
            </div>
