...
import { Palette, Image as ImageIcon, Check, Sparkles, RefreshCw } from 'lucide-react';
...
interface DesignBrandingCardProps {
  ...
  onChange?: ((key: 'accentColor' | 'iconUrl' | 'logoUrl' | 'coverUrl', value: string) => void) | undefined;
      ...
      <SettingsCardRow
        ...
        currentValue={
          <div className="flex items-center gap-2">
            ...
            />
            <span className="font-mono text-xs text-foreground uppercase">{accentColor}</span>
          </div>
        }
      ...
      {isExpanded && (
        <div className="border-t border-border/40 p-5 bg-muted/15 space-y-5 animate-in slide-in-from-top-2 duration-150">
          ...
          <div className="rounded-xl border border-border/70 overflow-hidden bg-card p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xs">
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-muted-foreground">
                Accent colors are applied throughout themes, buttons, links, and newsletters.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-1.5 bg-muted/40 px-3 py-1.5 rounded-lg border border-border/50">
                <div
                  ...
                  style={{ backgroundColor: accentColor }}
                />
                <span className="font-mono text-xs font-bold text-foreground uppercase">{accentColor}</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <label className="text-xs font-semibold text-foreground">Publication accent color</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 border-t border-border/40">
            <div className="p-4 rounded-xl border border-border/60 bg-card space-y-2.5">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-foreground">Publication Icon</span>
                <span className="text-[10px] text-muted-foreground">Square PNG (min 60x60)</span>
              </div>
              <div className="flex items-center gap-3">
                {iconUrl ? (
                  <img src={iconUrl} alt="Site Icon" className="size-11 rounded-xl object-cover shadow-xs border border-border/50" />
                ) : (
                  <div
                ...
                >
                  {uploadingIcon ? <RefreshCw className="h-3.5 w-3.5 mr-1 animate-spin" /> : <ImageIcon className="h-3.5 w-3.5 mr-1" />}
            ...
            <div className="p-4 rounded-xl border border-border/60 bg-card space-y-2.5">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-foreground">Publication Logo</span>
                <span className="text-[10px] text-muted-foreground">Transparent SVG / PNG</span>
              </div>
              <div className="flex items-center gap-3">
                {logoUrl ? (
                  <img src={logoUrl} alt="Logo" className="h-11 max-w-[120px] object-contain p-1 rounded-lg bg-muted/40 border border-border/60" />
                ) : (
                ...
                <Button
                  ...
                  className="text-xs h-8 cursor-pointer bg-card hover:bg-muted"
                >
                  {uploadingLogo ? <RefreshCw className="h-3.5 w-3.5 mr-1 animate-spin" /> : <ImageIcon className="h-3.5 w-3.5 mr-1" />}
