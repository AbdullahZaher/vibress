...
interface AnnouncementBarCardProps {
  ...
  onChange?: ((key: 'announcementEnabled' | 'announcementText' | 'announcementUrl', value: unknown) => void) | undefined;
  ...
  return (
    <SettingsCard id="site-announcement" isHighlighted={isHighlighted}>
      <SettingsCardRow
        ...
        currentValue={
          enabled ? (
            <Badge variant="default" className="text-[10px] gap-1 px-2 py-0.5 bg-primary text-primary-foreground">
      ...
      {isExpanded && (
        <div className="border-t border-border/50 p-5 bg-muted/10 space-y-4 animate-in slide-in-from-top-2 duration-150">
          <div className="flex items-start justify-between gap-4 p-3.5 rounded-lg border border-border/50 bg-card">
            <div>
              <p className="text-xs font-semibold text-foreground">Show announcement bar</p>
          ...
          {enabled && (
            <div className="space-y-3 pt-1 animate-in fade-in duration-150">
              <div className="space-y-1.5">
                <label htmlFor="card-announce-text" className="text-xs font-semibold text-foreground">
                  Announcement text
                </label>
                <Input
              ...
              <div className="space-y-1.5">
                <label htmlFor="card-announce-url" className="text-xs font-semibold text-foreground">
                  Target link URL (Optional)
                </label>
                <Input
                  id="card-announce-url"
                  value={url}
                  onChange={(e) => onChange?.('announcementUrl', e.target.value)}
