...
import { listOffersApi, createOfferApi, disableOfferApi, listProductsApi, AdminOffer, AdminProduct } from '../../../lib/api/billing';

interface OffersPromotionsCardProps {
  isHighlighted?: boolean | undefined;
}

export const OffersPromotionsCard: React.FC<OffersPromotionsCardProps> = ({ isHighlighted }) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (productsRes.products && productsRes.products.length > 0 && !productId) {
        ...
        <SettingsCardRow
          ...
          currentValue={
            <Badge variant="outline" className="text-xs font-mono text-muted-foreground">
              {activeOffersCount} Active {activeOffersCount === 1 ? 'Offer' : 'Offers'}
            </Badge>
          }
          ...
          <div className="w-full max-w-lg bg-card border border-border/80 rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between p-5 border-b border-border/60 bg-muted/20">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-foreground">Offers & Discounts</h3>
              </div>
              <button
                type="button"
            ...
            <div className="p-6 space-y-4">
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <h4 className="text-xs font-semibold text-foreground">Promotional Campaign Codes</h4>
                ...
                <div className="divide-y divide-border/40 rounded-xl border border-border/60 bg-muted/10 overflow-hidden max-h-52 overflow-y-auto">
                  {offers.length === 0 ? (
                    <div className="p-4 text-center text-xs text-muted-foreground">No active discount codes.</div>
                  ) : (
                    offers.map((offer) => (
                      <div key={offer.id} className="p-3 flex items-center justify-between">
                        <div>
                          <div className="flex items-center gap-2">
              ...
              {isCreating ? (
                <form onSubmit={handleCreate} className="space-y-3 p-4 rounded-xl border border-primary/40 bg-primary/5">
