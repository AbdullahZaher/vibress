...
export function useMembershipSettings() {
  const [initial, setInitial] =
    useState<MembershipSettingsState>(DEFAULT_STATE);
    ...
    try {
      ...
      if (membersNs) {
        for (const s of membersNs.settings) {
          if (s.key === "signupEnabled")
            loaded.signupEnabled = Boolean(s.value);
          if (s.key === "defaultNewsletterOptIn")
            loaded.defaultNewsletterOptIn = Boolean(s.value);
        }
      }
      ...
      if (securityNs) {
        for (const s of securityNs.settings) {
          if (s.key === "memberSessionTtlHours")
            loaded.memberSessionTtlHours = Number(s.value) || 720;
        }
      }
  ...
  const updateField = useCallback(
    <K extends keyof MembershipSettingsState>(
      key: K,
      value: MembershipSettingsState[K],
    ) => {
