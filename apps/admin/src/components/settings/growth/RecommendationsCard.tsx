...
import { listRecommendationsApi, createRecommendationApi, archiveRecommendationApi, AdminRecommendation } from '../../../lib/api/recommendations';

interface RecommendationsCardProps {
  isHighlighted?: boolean | undefined;
}

export const RecommendationsCard: React.FC<RecommendationsCardProps> = ({ isHighlighted }) => {
        ...
        <SettingsCardRow
          ...
          currentValue={
            <Badge variant="outline" className="text-xs font-mono text-muted-foreground">
              {activeCount} {activeCount === 1 ? 'Recommendation' : 'Recommendations'}
            </Badge>
          }
          actionLabel="Manage"
          onAction={handleOpen}
          ...
          <div className="w-full max-w-lg bg-card border border-border/80 rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between p-5 border-b border-border/60 bg-muted/20">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-foreground">Publication Recommendations</h3>
              </div>
              <button
                type="button"
            ...
            <div className="p-6 space-y-4">
              {loading ? (
                <div className="flex items-center justify-center py-10 text-muted-foreground text-xs">
                  <RefreshCw className="h-4 w-4 animate-spin mr-2" /> Loading recommendations...
                </div>
                ...
                <div className="p-8 rounded-xl border border-dashed border-border/70 text-center space-y-2 bg-muted/10">
                  ...
                  <p className="text-[11px] text-muted-foreground max-w-sm mx-auto">
                    Recommend newsletters and blogs to your subscribers to cross-promote with fellow creators.
                  </p>
                </div>
              ) : (
                <div className="space-y-2.5 max-h-56 overflow-y-auto">
                  {items.map((rec) => (
                    <div key={rec.id} className="p-3 rounded-xl border border-border/60 bg-muted/10 flex items-center justify-between">
                      <div>
                        <h5 className="text-xs font-bold text-foreground">{rec.title}</h5>
                        <p className="text-[11px] text-muted-foreground font-mono">{rec.url}</p>
                        {rec.description && <p className="text-[11px] text-muted-foreground/80 mt-0.5">{rec.description}</p>}
                      </div>
                      <button
                        type="button"
              ...
              {isCreating ? (
                <form onSubmit={handleCreate} className="p-4 rounded-xl border border-primary/40 bg-primary/5 space-y-3 animate-in fade-in duration-150">
                  <h4 className="text-xs font-bold text-foreground">Recommend a Publication</h4>
                  ...
                  <div className="flex justify-end gap-2 pt-1">
                    <Button type="button" size="sm" variant="ghost" onClick={() => setIsCreating(false)} className="text-xs h-8 cursor-pointer">
                      Cancel
                    </Button>
                ...
                <div className="flex justify-end pt-2 border-t border-border/40">
                  <Button size="sm" onClick={() => setIsCreating(true)} className="gap-1.5 text-xs cursor-pointer">
