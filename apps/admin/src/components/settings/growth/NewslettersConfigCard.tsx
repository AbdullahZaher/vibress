...
export const NewslettersConfigCard: React.FC<NewslettersConfigCardProps> = ({
  ...
  const handleSendTest = async () => {
    ...
    try {
      ...
      setTestResult({
        success: true,
        message: "Test email successfully queued & dispatched!",
      });
    } catch (err: unknown) {
      setTestResult({
        success: false,
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-background/70 backdrop-blur-xs animate-in fade-in duration-150">
          <div className="w-full max-w-xl bg-card border-l border-border/80 shadow-2xl h-full flex flex-col justify-between animate-in slide-in-from-right duration-200">
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1">
              ...
              <div className="space-y-3 pt-4 border-t border-border/50">
                <div className="flex items-center justify-between">
                  <h4 className="text-xs font-semibold text-foreground flex items-center gap-1.5">
                    <Server className="h-3.5 w-3.5 text-primary" /> SMTP
                    Delivery Server
                  </h4>
                  <Badge
                    variant="outline"
                ...
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground mt-1">
                    Integrated with internal MailPit SMTP relay for
                    transactional and newsletter broadcasts.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3 pt-4 border-t border-border/50">
                <h4 className="text-xs font-semibold text-foreground flex items-center gap-1.5">
                  <Sparkles className="h-3.5 w-3.5 text-primary" /> Test SMTP
                  Delivery
                </h4>
                <p className="text-[11px] text-muted-foreground leading-relaxed">
                  Send a live test email to verify DNS records, SMTP relay, and
                  template styling.
                </p>
                <div className="space-y-2">
                  ...
                  {testResult && (
                    <p
                      className={`text-xs font-medium ${testResult.success ? "text-emerald-500" : "text-destructive"} animate-in fade-in`}
                    >
                      {testResult.message}
                    </p>
                  )}
