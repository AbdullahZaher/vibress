...
import { Download, Upload, FileJson, X, RefreshCw, CheckCircle2 } from 'lucide-react';
import { Badge } from '../../ui/badge';
import { createExportApi, createImportApi, validateImportApi } from '../../../lib/api/operations';

interface ImportExportCardProps {
  isHighlighted?: boolean | undefined;
}

export const ImportExportCard: React.FC<ImportExportCardProps> = ({ isHighlighted }) => {
          ...
          <div className="w-full max-w-xl bg-card border border-border/80 rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between p-5 border-b border-border/60 bg-muted/20">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-foreground">Import & Export Content</h3>
              </div>
              <button
                type="button"
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-4 rounded-xl border border-border/60 bg-muted/10 space-y-2.5 flex flex-col justify-between">
                  <div>
                    <h4 className="text-xs font-semibold text-foreground">Export all content</h4>
                    <p className="text-[11px] text-muted-foreground mt-1">
                      Generate a complete JSON archive of all posts, pages, tags, and authors.
                    </p>
                  </div>
                  <Button
                    size="sm"
                ...
                <div className="p-4 rounded-xl border border-border/60 bg-muted/10 space-y-2.5 flex flex-col justify-between">
                  <div>
                    <h4 className="text-xs font-semibold text-foreground">Import content</h4>
                    <p className="text-[11px] text-muted-foreground mt-1">
                      Upload an exported JSON archive from Vibress, Ghost, or Substack.
                    </p>
                  </div>
                  <input
                    ref={fileInputRef}
                  ...
                  >
                    ...
                    <span>{importing ? 'Validating...' : 'Universal Import'}</span>
                  </Button>
                </div>
              </div>
              ...
              {statusMessage && (
                <div className={`p-3.5 rounded-xl border text-xs flex items-center gap-2 ${
                  ...
                  {statusMessage.success && <CheckCircle2 className="h-4 w-4 shrink-0" />}
