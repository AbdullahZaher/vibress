...
export const AuditLogsCard: React.FC<AuditLogsCardProps> = ({ isHighlighted }) => {
    ...
    <>
      <SettingsCard id="advanced-audit" isHighlighted={isHighlighted}>
        <SettingsCardRow
          ...
          currentValue={
            <Badge variant="secondary" className="text-xs font-mono gap-1 text-emerald-600 dark:text-emerald-400">
          ...
          <div className="w-full max-w-2xl bg-card border-l border-border/80 shadow-2xl h-full flex flex-col justify-between animate-in slide-in-from-right duration-200">
            ...
            <div className="flex items-center justify-between p-5 border-b border-border/60 bg-muted/20">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-foreground">Audit & Security Activity Log</h3>
              </div>
              <button
                type="button"
            ...
            <div className="p-6 overflow-y-auto space-y-4 flex-1">
              {loading ? (
                <div className="flex items-center justify-center py-16 text-muted-foreground text-xs">
                  <RefreshCw className="h-4 w-4 animate-spin mr-2" /> Loading audit records...
                </div>
              ) : events.length === 0 ? (
                <div className="p-8 rounded-xl border border-dashed border-border/70 text-center space-y-2 bg-muted/10">
                  <p className="text-xs text-muted-foreground">No recent audit log entries recorded.</p>
                </div>
              ) : (
                <div className="divide-y divide-border/40 rounded-xl border border-border/60 bg-muted/10 overflow-hidden">
                  {events.map((ev) => (
                    <div key={ev.id} className="flex items-center justify-between p-3.5 text-xs">
                      <div className="flex items-center gap-2.5">
                        <ShieldCheck className="h-4 w-4 text-emerald-500 shrink-0" />
                        <div>
                          <span className="font-semibold text-foreground font-mono">{ev.action}</span>
                          {ev.actorUserId && (
                            <p className="text-[11px] text-muted-foreground font-mono">User: {ev.actorUserId}</p>
                          )}
                        </div>
                      </div>
