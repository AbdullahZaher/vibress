  ...
  const items: CommandItem[] = useMemo(() => {
    const list: CommandItem[] = [
      ...
      // Settings
      {
        ...
        description: 'Customize active theme, typography, and styling',
        category: 'Settings' as const,
        icon: <Palette className="h-4 w-4 text-orange-400" />,
        keywords: ['theme', 'themes', 'design', 'appearance', 'molten', 'minimal', 'default', 'css', 'style'],
        action: () => onNavigate('/admin/settings/themes'),
      },
      {
        id: 'settings-billing',
        title: 'Billing & Subscriptions Settings',
        description: 'Stripe integration, tiers, and pricing plans',
        category: 'Settings' as const,
        icon: <CreditCard className="h-4 w-4 text-emerald-400" />,
        keywords: ['billing', 'stripe', 'plans', 'subscriptions', 'pricing', 'payments'],
        action: () => onNavigate('/admin/settings/billing'),
      },
      {
        id: 'settings-newsletters',
        title: 'Newsletter & Email Settings',
        description: 'Broadcast emails and newsletter configurations',
        category: 'Settings' as const,
        icon: <Mail className="h-4 w-4 text-yellow-400" />,
        keywords: ['newsletter', 'email', 'broadcast', 'smtp'],
        action: () => onNavigate('/admin/newsletters'),
      },
      {
        id: 'settings-storage',
        title: 'Storage & S3 Adapter Settings',
        description: 'Configure Local, S3, or R2 cloud storage',
        category: 'Settings' as const,
        icon: <HardDrive className="h-4 w-4 text-blue-400" />,
        keywords: ['storage', 's3', 'r2', 'upload', 'aws', 'cdn'],
        action: () => onNavigate('/admin/settings/storage'),
      },
      {
        id: 'settings-platform',
        title: 'Platform & Security Settings',
        description: 'Domain, SEO, metadata, and security policies',
        category: 'Settings' as const,
        icon: <Sliders className="h-4 w-4 text-purple-400" />,
        keywords: ['platform', 'security', 'domain', 'seo', 'meta', 'general'],
        action: () => onNavigate('/admin/settings/platform'),
      },
      {
        id: 'settings-operations',
        title: 'Operations & Maintenance',
        description: 'Database health, backups, and audit logs',
        category: 'Settings' as const,
        icon: <Activity className="h-4 w-4 text-red-400" />,
        keywords: ['operations', 'logs', 'audit', 'health', 'maintenance', 'backup'],
        action: () => onNavigate('/admin/settings/operations'),
      },
