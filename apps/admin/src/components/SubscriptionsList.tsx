...
import { CreditCard, ShieldAlert, CheckCircle2, AlertCircle } from 'lucide-react';
...
export function SubscriptionsList() {
  ...
  const refresh = useCallback(async (resetOffset = false) => {
    ...
    try {
      const params: { status?: string; limit?: number; offset?: number } = { limit: 20, offset: nextOffset };
  ...
  return (
    <div className="space-y-8 w-full max-w-7xl mx-auto">
      {/* Header */}
      <div>
        <h1 className="text-xl font-bold tracking-tight text-foreground">Paid Subscriptions</h1>
      </div>
              ...
              <TableRow>
                <TableCell colSpan={5} className="h-36 text-center text-muted-foreground">
                  <div className="flex flex-col items-center justify-center space-y-1">
                    ...
                    <p className="text-xs font-medium">No subscriptions match filter.</p>
                  </div>
                </TableCell>
              </TableRow>
            ) : (
              subscriptions.map((sub) => (
                <TableRow key={sub.id} className="hover:bg-muted/40 border-border">
                  <TableCell className="pl-6 font-mono text-xs text-foreground truncate max-w-xs">{sub.memberId}</TableCell>
                  <TableCell className="text-xs font-semibold text-foreground">{sub.planId}</TableCell>
                  <TableCell className="text-xs font-mono text-muted-foreground uppercase">{sub.billingInterval}</TableCell>
      ...
      {/* Detail Dialog */}
      <Dialog isOpen={!!selected} onClose={() => setSelected(null)} title="Subscription Lifecycle Details">
        {selected && (
          <div className="space-y-4 pt-2 text-xs">
            <div className="p-4 rounded-lg bg-muted/30 border border-border space-y-1 font-mono">
              <p className="text-foreground font-bold">Subscription ID: {selected.id}</p>
              <p className="text-muted-foreground">Member ID: {selected.memberId}</p>
              <p className="text-muted-foreground">Product ID: {selected.productId}</p>
              <p className="text-muted-foreground">Current Period Ends: {selected.currentPeriodEnd ? new Date(selected.currentPeriodEnd).toLocaleDateString() : 'N/A'}</p>
            </div>
