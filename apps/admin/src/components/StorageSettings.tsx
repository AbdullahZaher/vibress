...
import { HardDrive, CheckCircle2, AlertCircle, Trash2, Plug } from 'lucide-react';

const S3_PRESET_DEFAULTS: Record<string, { region: string; endpoint: string; forcePathStyle: boolean }> = {
  'aws-s3': { region: 'us-east-1', endpoint: '', forcePathStyle: false },
  'cloudflare-r2': { region: 'auto', endpoint: 'https://<account_id>.r2.cloudflarestorage.com', forcePathStyle: true },
  ...
  'backblaze-b2': { region: 'us-west-004', endpoint: 'https://s3.us-west-004.backblazeb2.com', forcePathStyle: false },
  hetzner: { region: 'hel1', endpoint: 'https://hel1.your-objectstorage.com', forcePathStyle: false },
...
export const StorageSettings: React.FC = () => {
  ...
  const createMutation = useMutation({
    mutationFn: (input: CreateStorageConfigurationInput) => createStorageConfigurationApi(input),
    onSuccess: () => {
  ...
  const activateMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['storage-configurations'] }),
  });
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      setTestResult({
        ...
        message: res.result.connected ? `Successfully connected to ${res.result.bucket}` : 'Connection failed',
      });
    } catch (err: unknown) {
      const e = err instanceof Error ? err : new Error(String(err));
  ...
  return (
    <div className="space-y-8 w-full max-w-7xl mx-auto">
      {/* Header */}
      <div>
        <h1 className="text-xl font-bold tracking-tight text-foreground">Storage Engine Configuration</h1>
      </div>
      ...
      <div className="space-y-3">
        <h3 className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">CONFIGURED STORAGE PROVIDERS</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {isLoading ? (
            <div className="col-span-2 p-8 text-center text-xs text-muted-foreground">Loading storage providers...</div>
          ) : configs.length === 0 ? (
            ...
            configs.map((config) => (
              <Card key={config.id} className="p-5 bg-transparent border-border shadow-2xs space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <HardDrive className="h-4 w-4 text-primary" />
                    <h4 className="font-bold text-xs text-foreground">{config.name}</h4>
                  </div>
                  {config.isActive ? (
                ...
                <div className="text-xs space-y-1 font-mono text-muted-foreground">
                  ...
                  {config.endpoint && <p className="truncate">Endpoint: {config.endpoint}</p>}
                </div>
      ...
      <div className="space-y-3">
        <h3 className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">ADD S3 STORAGE PROVIDER</h3>
        <Card className="p-6 bg-transparent border-border shadow-2xs space-y-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-xs font-medium text-foreground">Configuration Name</label>
                <Input value={name} onChange={(e) => setName(e.target.value)} required className="h-8 text-xs bg-card border-border" />
              </div>

              <div className="space-y-1">
                <label className="text-xs font-medium text-foreground">Provider Preset</label>
                <select
                  value={providerType}
              ...
              <div className="space-y-1">
                ...
                <Input value={bucket} onChange={(e) => setBucket(e.target.value)} required className="h-8 text-xs bg-card border-border" />
              </div>

              <div className="space-y-1">
                <label className="text-xs font-medium text-foreground">Region</label>
                <Input value={region} onChange={(e) => setRegion(e.target.value)} required className="h-8 text-xs bg-card border-border" />
              </div>

              <div className="space-y-1 sm:col-span-2">
                <label className="text-xs font-medium text-foreground">Endpoint URL (Required for R2, MinIO, Spaces)</label>
                <Input value={endpoint} onChange={(e) => setEndpoint(e.target.value)} placeholder="http://127.0.0.1:9000" className="h-8 text-xs font-mono bg-card border-border" />
              </div>

              <div className="space-y-1">
                <label className="text-xs font-medium text-foreground">Access Key ID</label>
                <Input value={accessKeyId} onChange={(e) => setAccessKeyId(e.target.value)} required type="password" className="h-8 text-xs font-mono bg-card border-border" />
              </div>

              <div className="space-y-1">
                <label className="text-xs font-medium text-foreground">Secret Access Key</label>
                <Input value={secretAccessKey} onChange={(e) => setSecretAccessKey(e.target.value)} required type="password" className="h-8 text-xs font-mono bg-card border-border" />
              </div>
            </div>

            {testResult && (
              <div className={`p-3 rounded-lg text-xs font-medium flex items-center gap-2 ${testResult.success ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20' : 'bg-red-500/10 text-red-600 dark:text-red-400 border border-red-500/20'}`}>
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              >
                <Plug className="h-3.5 w-3.5" /> {testing ? 'Testing...' : 'Test S3 Connection'}
              </Button>
