...
async function ensurePost(): Promise<string> {
  ...
  const existing = await db
    .select()
    .from(posts)
    .where(eq(posts.slug, "community-test-post"))
    .limit(1);
...
describe("Batch 11 — Community Integration & Security", () => {
  ...
  it("IDOR: member B cannot edit member A comment", async () => {
    const res = await app.inject({
      method: "PATCH",
      url: `/api/members/v1/comments/${commentA}`,
      headers: { cookie: memberB.cookie, origin: "http://localhost:7777" },
      payload: { body: "hacked" },
    });
  ...
  it("member likes a comment", async () => {
    const res = await app.inject({
      method: "POST",
      url: `/api/members/v1/comments/${commentA}/like`,
      headers: { cookie: memberB.cookie, origin: "http://localhost:7777" },
    });
    expect(res.statusCode).toBe(200);
  ...
  it("member reports a comment", async () => {
    const res = await app.inject({
      method: "POST",
      url: `/api/members/v1/comments/${commentA}/report`,
      headers: { cookie: memberB.cookie, origin: "http://localhost:7777" },
  ...
  it("reply generates notification for parent author", async () => {
    const res = await app.inject({
      method: "GET",
      url: "/api/members/v1/notifications",
      headers: { cookie: memberA.cookie },
    });
    expect(res.statusCode).toBe(200);
    expect(res.json().notifications.length).toBeGreaterThan(0);
    const replyNotif = res
      .json()
      .notifications.find((n: any) => n.type === "comment.reply");
  ...
  it("unread count works", async () => {
    const res = await app.inject({
      method: "GET",
      url: "/api/members/v1/notifications/unread-count",
      headers: { cookie: memberA.cookie },
    });
    expect(res.statusCode).toBe(200);
  ...
  it("mark one read", async () => {
    ...
    const firstUnread = listRes
      .json()
      .notifications.find((n: any) => !n.readAt);
    if (firstUnread) {
      const res = await app.inject({
        method: "POST",
        url: `/api/members/v1/notifications/${firstUnread.id}/read`,
        headers: { cookie: memberA.cookie, origin: "http://localhost:7777" },
      });
      expect(res.statusCode).toBe(200);
    }
  ...
  it("mark all read", async () => {
    const res = await app.inject({
      method: "POST",
      url: "/api/members/v1/notifications/read-all",
      headers: { cookie: memberA.cookie, origin: "http://localhost:7777" },
    });
    expect(res.statusCode).toBe(200);
  ...
  it("admin restores the comment", async () => {
    const res = await app.inject({
      method: "POST",
      url: `/api/admin/v1/comments/${commentA}/restore`,
      headers: { cookie: staffCookie, origin: "http://localhost:7777" },
    });
    expect(res.statusCode).toBe(200);
  ...
  it("admin creates a recommendation with a safe URL", async () => {
    const res = await app.inject({
      method: "POST",
      url: "/api/admin/v1/recommendations",
      headers: { cookie: staffCookie, origin: "http://localhost:7777" },
  ...
  it("public can list active recommendations", async () => {
    const res = await app.inject({
      method: "GET",
      url: "/api/content/v1/recommendations",
    });
    expect(res.statusCode).toBe(200);
  ...
  it("public click tracking records an event", async () => {
    ...
    const res = await app.inject({
      method: "POST",
      url: `/api/content/v1/recommendations/${recId}/click`,
      payload: { sessionId: "test-session" },
    });
    expect(res.statusCode).toBe(200);
    const statsRes = await app.inject({
      method: "GET",
      url: `/api/admin/v1/recommendations/${recId}/stats`,
      headers: { cookie: staffCookie },
    });
