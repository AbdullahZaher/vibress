...
describe("API hardening", () => {
  ...
  it("preserves explicit 4xx messages in production", async () => {
    ...
    app.get("/h4/bad-request", async () => {
      const err = new Error("Bad input visible to client") as Error & {
        statusCode: number;
        code: string;
      };
      err.statusCode = 400;
    ...
    expect(res.json().errors[0]).toMatchObject({
      ...
      message: "Bad input visible to client",
    });
  });
  ...
  it("exposes Prometheus metrics when metrics are enabled", async () => {
    ...
    const baselineCount = Number(
      baseline.body.match(
        /http_requests_total\{method="GET",path="\/api",status="2xx"\} (\d+)/,
      )?.[1] || 0,
    );
    ...
    const countMatch = metricsRes.body.match(
      /http_requests_total\{method="GET",path="\/api",status="2xx"\} (\d+)/,
    );
