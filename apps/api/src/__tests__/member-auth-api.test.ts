...
describe('Batch 8 — Member API Integration & Security', () => {
  ...
  it('raw auth token never stored in database (hash only)', async () => {
    ...
    const rawToken = mailer.sent[mailer.sent.length - 1]!.magicLinkUrl.split('token=')[1]!;
  ...
  it('creates member and verifies magic link end-to-end', async () => {
    ...
    await authService.requestAuthLink(email);
    const rawToken = mailer.sent[mailer.sent.length - 1]!.magicLinkUrl.split('token=')[1]!;
    const { member, sessionToken } = await authService.verifyAndCreateSession(rawToken);
    ...
    // Token single-use
    await expect(authService.verifyAndCreateSession(rawToken)).rejects.toThrow();
  });
