...
describe('Batch 13 — Intelligence Integration & Security', () => {
  ...
  it('public search finds indexed published content', async () => {
    ...
    const res = await app.inject({ method: 'GET', url: '/api/content/v1/search?q=Intelligence' });
    expect(res.statusCode).toBe(200);
    const body = res.json();
  ...
  it('restricted content (members visibility) is never searchable', async () => {
    ...
    const res = await app.inject({ method: 'GET', url: '/api/content/v1/search?q=Members+Only+Secret' });
    expect(res.statusCode).toBe(200);
    const body = res.json();
    // Restricted content is not indexed, so it cannot appear
    expect(body.results.some((r: any) => r.title.includes('Members Only Secret'))).toBe(false);
  });

  it('search rejects an over-long query (DoS protection)', async () => {
    const res = await app.inject({ method: 'GET', url: `/api/content/v1/search?q=${'a'.repeat(200)}` });
    expect(res.statusCode).toBe(400);
    expect(res.json().errors[0].code).toBe('QUERY_TOO_LONG');
  });

  it('search rejects wildcard-only queries', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/content/v1/search?q=***' });
    expect(res.statusCode).toBe(400);
  ...
  it('search is rate limited (200/min in test)', async () => {
    ...
    let limited = false;
    for (let i = 0; i < 210; i++) {
      const res = await app.inject({ method: 'GET', url: '/api/content/v1/search?q=test', remoteAddress: '203.0.113.7' });
  ...
  it('analytics metrics require staff auth (401)', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/admin/v1/analytics/metrics' });
  ...
  it('search rebuild requires search.manage (owner has it)', async () => {
    const res = await app.inject({
      method: 'POST', url: '/api/admin/v1/search/rebuild',
  ...
  it('search index count endpoint works', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/admin/v1/search/index-count', headers: { cookie: staffCookie } });
    expect(res.statusCode).toBe(200);
  ...
  it('admin creates an automation', async () => {
    const res = await app.inject({
      method: 'POST', url: '/api/admin/v1/automations',
      headers: { cookie: staffCookie, origin: 'http://localhost:7777' },
      payload: {
        ...
        actions: [{ type: 'webhook', config: { url: 'https://receiver.example.com/hook', eventType: 'member.welcome' } }],
      },
    });
  ...
  it('admin cannot create an automation with an invalid trigger', async () => {
    const res = await app.inject({
      method: 'POST', url: '/api/admin/v1/automations',
      headers: { cookie: staffCookie, origin: 'http://localhost:7777' },
      payload: { key: 'bad', name: 'Bad', triggerEvent: 'pwn.all', actions: [] },
    });
    expect(res.statusCode).toBe(400);
  ...
  it('admin activates the automation', async () => {
    const listRes = await app.inject({ method: 'GET', url: '/api/admin/v1/automations', headers: { cookie: staffCookie } });
    const automation = listRes.json().automations[listRes.json().automations.length - 1];
    const res = await app.inject({
      method: 'POST', url: `/api/admin/v1/automations/${automation.id}/activate`,
      headers: { cookie: staffCookie, origin: 'http://localhost:7777' },
    });
    expect(res.statusCode).toBe(200);
  ...
  it('manual run works for an active automation', async () => {
    ...
    const automation = listRes.json().automations[listRes.json().automations.length - 1];
    const res = await app.inject({
      method: 'POST', url: `/api/admin/v1/automations/${automation.id}/run`,
      headers: { cookie: staffCookie, origin: 'http://localhost:7777' },
    });
    expect(res.statusCode).toBe(201);
  ...
  it('automation run history is inspectable', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/admin/v1/automation-runs', headers: { cookie: staffCookie } });
    expect(res.statusCode).toBe(200);
    expect(res.json().runs.length).toBeGreaterThan(0);
  });

  it('automation routes require RBAC (401 unauth)', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/admin/v1/automations' });
    expect(res.statusCode).toBe(401);
  });

  it('automation runs list requires staff auth', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/admin/v1/automation-runs' });
    expect(res.statusCode).toBe(401);
  });
  ...
  it('CSRF: automation creation without origin is rejected', async () => {
    const res = await app.inject({
      method: 'POST', url: '/api/admin/v1/automations',
      headers: { cookie: staffCookie },
      payload: { key: 'no-origin', name: 'X', triggerEvent: 'member.created', actions: [] },
    });
