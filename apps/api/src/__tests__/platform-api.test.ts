...
describe('Batch 12 — Platform Integration & Security', () => {
  ...
  it('admin creates an integration (secrets encrypted at rest)', async () => {
    const res = await app.inject({
      method: 'POST', url: '/api/admin/v1/integrations',
      headers: { cookie: staffCookie, origin: 'http://localhost:7777' },
      payload: { key: `ci-${Date.now()}`, type: 'external', name: 'CI Integration', secrets: { apiToken: 'top-secret' } },
    });
    ...
    const rows = await db.select().from(integrations).where(eq(integrations.key, `ci-${Date.now()}`)).limit(1);
  });

  it('integration routes require staff auth (401)', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/admin/v1/integrations' });
    expect(res.statusCode).toBe(401);
  });

  it('member cookie cannot access admin platform routes', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/admin/v1/integrations', headers: { cookie: 'vibress_member_session=forged' } });
    expect(res.statusCode).toBe(401);
  });
  ...
  it('creates an API key: raw secret returned once, hash stored', async () => {
    const res = await app.inject({
      method: 'POST', url: '/api/admin/v1/api-keys',
      headers: { cookie: staffCookie, origin: 'http://localhost:7777' },
  ...
  it('machine endpoint authenticates with a valid key', async () => {
    ...
    const res = await app.inject({
      method: 'GET', url: '/api/machine/v1/status',
  ...
  it('scope enforcement: key without the required scope gets 403', async () => {
    ...
    const res = await app.inject({
      method: 'POST', url: '/api/machine/v1/events',
  ...
  it('revoked key is rejected', async () => {
    ...
    const revokeRes = await app.inject({
      method: 'POST', url: `/api/admin/v1/api-keys/${keyId}/revoke`,
      headers: { cookie: staffCookie, origin: 'http://localhost:7777' },
    });
  ...
  it('API key routes require RBAC (401 unauth, staff cookie only)', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/admin/v1/api-keys' });
    expect(res.statusCode).toBe(401);
  });
  ...
  it('webhook endpoint registration rejects localhost (SSRF)', async () => {
    const res = await app.inject({
      method: 'POST', url: '/api/admin/v1/webhook-endpoints',
      headers: { cookie: staffCookie, origin: 'http://localhost:7777' },
      payload: { name: 'Bad', url: 'http://localhost:9999/hook', eventTypes: ['comment.created'] },
    });
  ...
  it('webhook endpoint registration rejects private IPs (SSRF)', async () => {
    const res = await app.inject({
      method: 'POST', url: '/api/admin/v1/webhook-endpoints',
      headers: { cookie: staffCookie, origin: 'http://localhost:7777' },
      payload: { name: 'Bad', url: 'http://192.168.1.10/hook', eventTypes: ['comment.created'] },
    });
    expect(res.statusCode).toBe(400);
    expect(res.json().errors[0].code).toBe('UNSAFE_URL');
  });
  ...
  it('webhook endpoint registration accepts a public URL and encrypts the secret', async () => {
    const res = await app.inject({
      method: 'POST', url: '/api/admin/v1/webhook-endpoints',
      headers: { cookie: staffCookie, origin: 'http://localhost:7777' },
      payload: { name: 'Public receiver', url: 'https://receiver.example.com/hook', secret: 'hook-secret-123', eventTypes: ['comment.created'] },
    });
    expect(res.statusCode).toBe(201);
    const body = res.json();
  ...
  it('webhook endpoint routes require RBAC (401 unauth)', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/admin/v1/webhook-endpoints' });
    expect(res.statusCode).toBe(401);
  });
  ...
  it('registers the official example plugin (valid manifest)', async () => {
    const res = await app.inject({
      method: 'POST', url: '/api/admin/v1/plugins/register',
      headers: { cookie: staffCookie, origin: 'http://localhost:7777' },
      payload: {
        manifest: {
    ...
    expect(['registered', 'active', 'inactive', 'error']).toContain(res.json().plugin.status);
  });
  ...
  it('rejects a plugin with an unsupported API version', async () => {
    const res = await app.inject({
      ...
      payload: {
        manifest: {
          id: 'bad-plugin', name: 'Bad', version: '1.0.0', vibressApiVersion: '9.9.9', entrypoint: 'x.ts', capabilities: [],
        },
      },
    });
    expect(res.statusCode).toBe(400);
  ...
  it('rejects a plugin with an unknown capability', async () => {
    const res = await app.inject({
      ...
      payload: {
        manifest: {
          id: 'evil-plugin', name: 'Evil', version: '1.0.0', vibressApiVersion: SDK_VERSION, entrypoint: 'x.ts',
  ...
  it('activates the official plugin (proves SDK/lifecycle boundary)', async () => {
    const listRes = await app.inject({ method: 'GET', url: '/api/admin/v1/plugins', headers: { cookie: staffCookie } });
    const plugin = listRes.json().plugins.find((p: any) => p.manifestId === 'vibress-content-metrics');
    expect(plugin).toBeTruthy();

    const res = await app.inject({
      method: 'POST', url: `/api/admin/v1/plugins/${plugin.id}/activate`,
      headers: { cookie: staffCookie, origin: 'http://localhost:7777' },
    });
    expect(res.statusCode).toBe(200);
  ...
  it('plugin settings: secret stored encrypted and masked on read', async () => {
    ...
    const setRes = await app.inject({
      method: 'POST', url: `/api/admin/v1/plugins/${plugin.id}/settings`,
      headers: { cookie: staffCookie, origin: 'http://localhost:7777' },
      payload: { settings: { webhookUrl: 'https://secret-endpoint.example.com' } },
    });
    expect(setRes.statusCode).toBe(200);

    const getRes = await app.inject({ method: 'GET', url: `/api/admin/v1/plugins/${plugin.id}/settings`, headers: { cookie: staffCookie } });
    expect(getRes.statusCode).toBe(200);
    const settings = getRes.json().settings;
    const secret = settings.find((s: any) => s.key === 'webhookUrl');
    expect(secret.masked).toBe(true);
    expect(JSON.stringify(getRes.json())).not.toContain('secret-endpoint.example.com');
  ...
  it('plugin routes require RBAC (401 unauth)', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/admin/v1/plugins' });
    expect(res.statusCode).toBe(401);
  });
  ...
  it('plugin deactivation works', async () => {
    ...
    const plugin = listRes.json().plugins.find((p: any) => p.manifestId === 'vibress-content-metrics');
    const res = await app.inject({
      method: 'POST', url: `/api/admin/v1/plugins/${plugin.id}/deactivate`,
      headers: { cookie: staffCookie, origin: 'http://localhost:7777' },
    });
    expect(res.statusCode).toBe(200);
