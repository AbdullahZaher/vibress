...
class FakeBillingProvider implements BillingProvider {
  ...
  async createCustomer(_input: BillingCreateCustomerInput): Promise<string> {
...
describe("Batch 9 — Billing Integration & Security", () => {
  ...
  beforeAll(async () => {
    ...
    const product = await productsService.createProduct(
      { key: `premium${runSuffix}`, name: "Premium" },
      null,
    );
    ...
    planId = plan.id;
    const freeProduct = await productsService.createProduct(
      { key: `free${runSuffix}`, name: "Free Membership" },
      null,
    );
    freeProductId = freeProduct.id;
    const freePlan = await plansService.createPlan(
      {
    ...
    const member = await signupMember(
      app,
      `member-a-${memberAEmailSuffix}@example.com`,
    );
  ...
  it("public catalog exposes only active public products with plans", async () => {
    ...
    expect(freeProduct.plans.some((pl: any) => pl.id === freePlanId)).toBe(
      true,
    );
  });
  ...
  it("client-supplied amount cannot override server price", async () => {
    ...
    expect(res.statusCode).toBe(200);
    const session =
      provider.checkoutSessions[provider.checkoutSessions.length - 1]!;
  ...
  it("duplicate checkout is controlled: second checkout for same member+product rejected", async () => {
    ...
    const memberRepo = new DrizzleMemberRepository();
    const subRepo = new DrizzleSubscriptionRepository();
    const memberA = await memberRepo.findByEmailNormalized(
      `member-a-${memberAEmailSuffix}@example.com`,
    );
  ...
  it("checkout validates Host-header independent return URLs (no open redirect)", async () => {
    ...
    const session =
      provider.checkoutSessions[provider.checkoutSessions.length - 1]!;
    expect(session.successUrl.startsWith("https://portal.example.test")).toBe(
      true,
    );
  ...
  it("free plan checkout returns internal portal URL, not provider URL", async () => {
    ...
    expect(res.statusCode).toBe(200);
    expect(
      res.json().checkoutUrl.startsWith("https://portal.example.test/account"),
    ).toBe(true);
  });
  ...
  it("member lists own subscriptions", async () => {
    const res = await app.inject({
      method: "GET",
      url: "/api/members/v1/subscriptions",
  ...
  it("IDOR: member A cannot view member B subscription", async () => {
    ...
    const res = await app.inject({
      method: "GET",
      url: `/api/members/v1/subscriptions/${otherSub.id}`,
  ...
  it("webhook rejects invalid signature", async () => {
    const { payload } = signedPayload("customer.subscription.updated", {
      id: "sub_x",
    });
    const res = await app.inject({
      method: "POST",
  ...
  it("webhook accepts valid signed event and creates subscription", async () => {
    ...
    expect(res.statusCode).toBe(200);
    const subRepo = new DrizzleSubscriptionRepository();
    const found = await subRepo.findByProviderSubscriptionId(
      "stripe",
      `sub_run_${runSuffix}`,
    );
  ...
  it("webhook out-of-order: older event does not overwrite newer state", async () => {
    ...
    const { payload: p1, header: h1 } = signedPayload(
    ...
    const res = await app.inject({
      ...
      headers: {
        "content-type": "application/json",
        "stripe-signature": oldHeader,
      },
      payload: oldPayload,
    });
    expect(res.statusCode).toBe(200);
    // State should NOT be overwritten by the old event
    const after = await subRepo.findByProviderSubscriptionId(
      "stripe",
      `sub_run_${runSuffix}`,
    );
    expect(after!.providerEventTimestamp!.getTime()).toBeGreaterThanOrEqual(
      newerTimestamp * 1000,
    );
  });
  ...
  it("member cancels own subscription at period end", async () => {
    const subRepo = new DrizzleSubscriptionRepository();
    const subs = await subRepo.list({ memberId: memberB });
    const active = subs.subscriptions.find(
      (s) => s.status === "active" && s.planId === planId,
    );
    if (!active) {
  ...
  it("member resumes own scheduled cancellation", async () => {
    const subRepo = new DrizzleSubscriptionRepository();
    const subs = await subRepo.list({ memberId: memberB });
    const scheduled = subs.subscriptions.find(
  ...
  it("admin subscription detail includes billing event history", async () => {
    ...
    const res = await app.inject({
      method: "GET",
      url: `/api/admin/v1/subscriptions/${subs.subscriptions[0]!.id}`,
      headers: { cookie: staffCookie },
    });
    expect(res.statusCode).toBe(200);
  ...
  it("member subscriptions view for admin shows member subscriptions", async () => {
    const res = await app.inject({
      method: "GET",
      url: `/api/admin/v1/members/${memberB}/subscriptions`,
      headers: { cookie: staffCookie },
    });
    expect(res.statusCode).toBe(200);
    expect(res.json().subscriptions.length).toBeGreaterThan(0);
