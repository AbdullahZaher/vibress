...
export async function runIntegrityChecks(): Promise<
  Array<{ check: string; status: "ok" | "warning" | "error"; detail?: string }>
> {
  const db = getDb();
  const results: Array<{
    check: string;
  ...
  try {
    ...
    results.push({
      ...
      detail:
        orphaned.length === 0
          ? "All mappings reference valid plans"
          : `${orphaned.length} mapping(s) reference missing plans`,
    });
  } catch (err: unknown) {
    results.push({
      check: "billing_plan_mappings",
  ...
  try {
    ...
    const publishedRes = await postsService.listPosts({
      publishedOnly: true,
      limit: 10000,
    });
    const publishedPublic = publishedRes.posts.filter(
  ...
  try {
    const mediaRows = await db
      ...
      .from(mediaAssets);
    const unknownProviders = mediaRows.filter(
      (m) => !["local", "s3"].includes(m.storageProvider || ""),
    );
    results.push({
      check: "media_storage",
      ...
      detail:
        unknownProviders.length === 0
          ? "All media reference known providers"
          : `${unknownProviders.length} media row(s) reference unknown providers`,
    });
  } catch (err: unknown) {
    results.push({
      check: "media_storage",
  ...
  try {
    const recs = await db
      .select({ id: recommendations.id, url: recommendations.url })
...
async function checkMigrationVersion(): Promise<number | null> {
  try {
    const rows = await getDb().execute(
      sql`SELECT "idx" FROM "drizzle"."__drizzle_migrations" ORDER BY "idx" DESC LIMIT 1`,
    );
