...
export const manifest = {
  ...
  description:
    "Official example plugin that logs content events for demonstration.",
  entrypoint: "index.ts",
...
export const plugin: PluginModule = {
  async activate(context: PluginContext): Promise<void> {
    ...
    context.log(
      `Configured log level: ${String(context.settings.logLevel ?? "info")}`,
      "info",
    );
  ...
  async onEvent(eventName: string, payload: unknown): Promise<void> {
    ...
    if (eventName === "post.published") {
      console.log(
        `[vibress-content-metrics] post published: ${JSON.stringify(payload).slice(0, 200)}`,
      );
    }
  },
};
