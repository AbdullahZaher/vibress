...
export function createApiLogger(): ReturnType<typeof createLogger> {
  ...
  const minLevel = VALID_LOG_LEVELS.includes(configuredLevel) ? configuredLevel : 'info';
  return createLogger('api', { minLevel });
}

export const appLogger = createApiLogger();

const TRACEPARENT_RE = /^([0-9a-f]{32})-([0-9a-f]{16})-[0-9a-f]{16}-[0-9a-f]{2}$/;
...
function requestPath(request: FastifyRequest): string {
  const routeUrl = (request.routeOptions as { url?: string } | undefined)?.url as string | undefined;
...
function recordRequestMetrics(request: FastifyRequest, statusCode: number): void {
