...
export async function memberNewsletterRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/newsletter-preferences', {
    ...
    handler: async (req, reply) => {
      const preferences = await newslettersService.listPreferencesForMember(req.member!.id);
  ...
  fastify.put('/newsletter-preferences', {
    ...
    handler: async (req, reply) => {
      ...
      if (!parsed.success) {
        return reply.status(400).send({ errors: [{ code: 'VALIDATION_ERROR', message: 'Invalid payload', requestId: req.id }] });
      }
      try {
  ...
  fastify.post('/unsubscribe', {
    handler: async (req, reply) => {
      ...
      if (!body.token || typeof body.token !== 'string') {
        return reply.status(400).send({ errors: [{ code: 'INVALID_UNSUBSCRIBE_TOKEN', message: 'Invalid unsubscribe token', requestId: req.id }] });
      }
      try {
