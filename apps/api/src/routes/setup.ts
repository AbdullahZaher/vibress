...
import type { SetupCompleteResponse, SetupStatusResponse } from '@vibress/api-contracts';
import { SetupDomainError, setupTokenMatches, isSetupTokenInsecure } from '@vibress/setup';
...
export function ensureSetupTokenConfigured(): void {
  ...
  if (!devSetupTokenShown) {
    ...
    console.log('==============================================================');
    console.log('DEVELOPMENT ONLY — First-run setup token (this process only, never persisted):');
    console.log(`  ${devSetupToken}`);
    console.log('In production, set VIBRESS_SETUP_TOKEN in the environment instead.');
    console.log('==============================================================');
  }
}
...
async function validateSetupOrigin(req: FastifyRequest, reply: FastifyReply) {
  ...
  if (!origin || !allowedOrigins.includes(origin)) {
    return reply.status(403).send({
      errors: [{ code: 'INVALID_ORIGIN', message: 'Invalid request origin', requestId: req.id }],
    });
  }
}
...
async function setupStateGate(req: FastifyRequest, reply: FastifyReply) {
  ...
  if (installed) {
    return reply.status(409).send({
      errors: [{ code: 'SETUP_ALREADY_COMPLETED', message: 'Setup has already been completed', requestId: req.id }],
    });
  }
}
...
async function requireSetupToken(req: FastifyRequest, reply: FastifyReply) {
  ...
  if (typeof header !== 'string' || !configured || !setupTokenMatches(header, configured)) {
    appLogger.warn('setup token rejected', { requestId: req.id, ipAddress: req.ip });
    return reply.status(401).send({
      errors: [{ code: 'INVALID_SETUP_TOKEN', message: 'Invalid setup token', requestId: req.id }],
    });
  }
}
...
export async function setupRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/complete', {
    ...
    handler: async (req, reply) => {
          ...
          errors: [
            {
              ...
              message: parseResult.error.errors[0]?.message || 'Invalid setup request payload',
              requestId: req.id,
            },
          ],
        });
      ...
      try {
        const result = await setupService.completeSetup(parseResult.data, {
        ...
        try {
          const session = await authService.loginStaff(parseResult.data.owner.email, parseResult.data.owner.password, {
            ipAddress: req.ip,
            userAgent: req.headers['user-agent'] || null,
            requestId: req.id,
          });
          ...
          appLogger.error('setup completed but auto-login failed; owner can sign in manually', { requestId: req.id }, sessionErr as Error);
        }
        ...
        if (err instanceof Error) {
          ...
          if (code === 'EMAIL_ALREADY_EXISTS') {
            return reply.status(400).send({
              errors: [{ code: 'EMAIL_ALREADY_EXISTS', message: 'An account with this email already exists', requestId: req.id }],
            });
          }
        }
        throw err;
