...
export async function adminNewsletterRoutes(fastify: FastifyInstance) {
  ...
  fastify.post("/newsletters", {
    preHandler: [
      requireStaffSession,
      requirePermission("newsletters.manage"),
      validateOrigin,
    ],
    handler: async (req, reply) => {
  ...
  fastify.patch("/newsletters/:id", {
    ...
    handler: async (req, reply) => {
      ...
      const parsed = AdminNewsletterInputSchema.omit({ key: true })
        .partial()
        .safeParse(req.body);
      if (!parsed.success)
        return sendError(
      ...
      try {
        const newsletter = await newslettersService.updateNewsletter(
          id,
  ...
  fastify.post("/newsletter-sends", {
    ...
    handler: async (req, reply) => {
      ...
      const { newsletterId, subject, content, audience, scheduledAt, sendNow } =
        parsed.data;
      ...
      const audienceCount = (
        await newslettersService.computeAudience(
          newsletterId,
          audienceDefinition,
        )
      ).length;
  ...
  fastify.post("/newsletter-test-email", {
    ...
    handler: async (req, reply) => {
      ...
      for (const to of recipients) {
        try {
          const { html, text } = newslettersService.renderEmailHtml(
            pseudoSend,
            "test",
            "test-token",
          );
