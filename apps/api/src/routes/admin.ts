...
export async function adminRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/users/invite', {
    ...
    handler: async (req, reply) => {
      const body = req.body as { email?: string; name?: string; roleKey?: string } | undefined;
      if (!body || !body.email) {
        return reply.status(400).send({ errors: [{ code: 'VALIDATION_ERROR', message: 'Email is required', requestId: req.id }] });
      }
      ...
      if (existing) {
        return reply.status(409).send({ errors: [{ code: 'EMAIL_ALREADY_EXISTS', message: 'User with this email already exists', requestId: req.id }] });
      }

      const role = await rolesService.findByKey(roleKey);
      if (!role) {
        return reply.status(400).send({ errors: [{ code: 'INVALID_ROLE', message: `Unknown role: ${roleKey}`, requestId: req.id }] });
      }
