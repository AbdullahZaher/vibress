...
export async function adminBillingRoutes(fastify: FastifyInstance) {
  ...
  fastify.post("/offers", {
    ...
    handler: async (req, reply) => {
      ...
      try {
        ...
        if (err instanceof OfferDomainError)
          return sendError(reply, err.code, err.message, req.id);
        throw err;
      }
    },
    ...
    handler: async (req, reply) => {
      ...
      try {
        const offer = await offersService.updateOffer(
          id,
          parsed.data as UpdateOfferData,
          req.user!.id,
        );
  ...
  fastify.get("/subscriptions", {
    ...
    handler: async (req, reply) => {
      ...
      if (!parsed.success)
        return sendError(reply, "VALIDATION_ERROR", "Invalid filter", req.id);
  ...
  fastify.get("/subscriptions/:id", {
    ...
    handler: async (req, reply) => {
      ...
      return reply
        .status(200)
        .send({ subscription: toAdminSubscriptionDto(sub), events });
    },
  });
  ...
  fastify.post("/subscriptions/:id/cancel", {
    ...
    handler: async (req, reply) => {
      ...
      try {
        ...
        return reply
          .status(200)
          .send({ subscription: toAdminSubscriptionDto(updated) });
      } catch (err: unknown) {
