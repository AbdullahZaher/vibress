...
export async function publicCatalogRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/products/:key', async (req, reply) => {
    ...
    if (!product || product.status !== 'active' || product.visibility !== 'public') {
      return reply.status(404).send({ errors: [{ code: 'PRODUCT_NOT_FOUND', message: 'Product not found', requestId: req.id }] });
    }
    const plans = (await plansService.listActivePublicPlans()).filter((p) => p.productId === product.id);
