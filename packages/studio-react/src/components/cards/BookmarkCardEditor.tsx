    ...
    >
      ...
      >
        <div className="flex flex-col flex-1 p-3 px-4 justify-between min-w-0">
          <div>
            <div className="font-semibold truncate mb-1 text-sm text-foreground">
              {cardData.title || cardData.url}
            </div>
          ...
          <div className="flex items-center mt-2 text-[11px] text-muted-foreground">
            {cardData.icon && (
              <img
                src={cardData.icon}
                alt=""
                className="w-3.5 h-3.5 mr-1.5 rounded"
              />
            )}
            <span className="truncate">
              {cardData.publisher || new URL(cardData.url).hostname}
            </span>
            {cardData.author && (
              <span className="before:content-['•'] before:mx-1 truncate">
                {cardData.author}
              </span>
            )}
          </div>
