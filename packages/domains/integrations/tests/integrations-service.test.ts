...
} from "../src/application/integrations-service";
import {
...
describe("IntegrationsService", () => {
  const integrationRepo: IntegrationRepository = {
    create: vi.fn(async (d) =>
      makeIntegration({
        ...
        type: d.type,
        name: d.name,
        encryptedSecrets: d.secrets || null,
      }),
    ),
  ...
  it("creates an integration with encrypted secrets", async () => {
    ...
    expect(integration.encryptedSecrets!.apiToken).not.toContain(
      "super-secret",
    );
    // Decrypts correctly
    const decrypted = await service.decryptIntegrationSecret(
      integration,
      "apiToken",
    );
  ...
  it("masks secret values in the public DTO", async () => {
    ...
    const integration = makeIntegration({
      encryptedSecrets: { apiToken: encryptSecret("abc") },
    });
  ...
  it("createApiKey returns the raw secret exactly once and stores only a hash", async () => {
    ...
    const created = await service.createApiKey(
      { name: "CI token", scopes: ["content.read"] },
      "u1",
    );
    expect(created.secret.startsWith("vk_")).toBe(true);
    expect(created.prefix).toBe(
      created.secret.split("_")[0] + "_" + created.secret.split("_")[1],
    );
  ...
  it("secret update is replace-only: existing secrets preserved when not provided", async () => {
    const existing = makeIntegration({
      encryptedSecrets: { apiToken: encryptSecret("old-token") },
    });
