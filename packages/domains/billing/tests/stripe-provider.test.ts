...
function makeSignedEvent(type: string, object: Record<string, unknown>): { payload: string; header: string } {
...
describe('StripeBillingProvider webhook contract', () => {
  const provider = new StripeBillingProvider({ secretKey: 'sk_test_dummy', webhookSecret: WEBHOOK_SECRET });

  it('verifies a valid signed payload', async () => {
    const { payload, header } = makeSignedEvent('customer.subscription.updated', { id: 'sub_1', status: 'active' });
  ...
  it('verifies a valid signed Buffer payload (raw body)', async () => {
    const { payload, header } = makeSignedEvent('customer.subscription.updated', { id: 'sub_1', status: 'active' });
    expect(await provider.verifyWebhookSignature(Buffer.from(payload), header)).toBe(true);
  });

  it('rejects an invalid signature', async () => {
    const { payload } = makeSignedEvent('customer.subscription.updated', { id: 'sub_1' });
    expect(await provider.verifyWebhookSignature(payload, 't=1,v1=bogus')).toBe(false);
  });
  ...
  it('rejects a signature with an unknown webhook secret', async () => {
    const { payload } = makeSignedEvent('customer.subscription.updated', { id: 'sub_1' });
    const wrongProvider = new StripeBillingProvider({ secretKey: 'sk_test_dummy', webhookSecret: 'whsec_wrong' });
    expect(await wrongProvider.verifyWebhookSignature(payload, Stripe.webhooks.generateTestHeaderString({ payload, secret: 'whsec_other' }))).toBe(false);
  });

  it('parses a webhook event into provider-neutral shape', async () => {
    const { payload } = makeSignedEvent('checkout.session.completed', { id: 'cs_1', status: 'complete' });
  ...
  it('maps provider errors to stable Vibress error codes', async () => {
    ...
    await expect(provider.createCustomer({ email: 'a@b.com' })).rejects.toMatchObject({ code: 'BILLING_PROVIDER_UNAVAILABLE' });
  });
});
