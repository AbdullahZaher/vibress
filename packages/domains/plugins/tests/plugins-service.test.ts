...
import { PluginsService, PluginDomainError } from '../src/application/plugins-service';
import { PluginRepository, PluginSettingRepository, Plugin, PluginSetting } from '../src/domain/plugin';
...
describe('PluginsService', () => {
  const pluginRepo: PluginRepository = {
    create: vi.fn(async (d) => makePlugin({ manifestId: d.manifestId, name: d.name, capabilities: d.capabilities })),
  ...
  it('rejects an incompatible Vibress API version', async () => {
    const service = makeService();
    await expect(service.registerPlugin({ ...validManifest, vibressApiVersion: '0.5.0' }, null))
      .rejects.toMatchObject({ code: 'INVALID_MANIFEST' });
  });

  it('rejects an unknown capability', async () => {
    const service = makeService();
    await expect(service.registerPlugin({ ...validManifest, capabilities: ['pwn.all'] }, null))
      .rejects.toMatchObject({ code: 'INVALID_MANIFEST' });
  });

  it('rejects a missing entrypoint', async () => {
    const service = makeService();
    await expect(service.registerPlugin({ ...validManifest, entrypoint: '' }, null))
      .rejects.toMatchObject({ code: 'INVALID_MANIFEST' });
  });
  ...
  it('activates a plugin via the host and marks it active', async () => {
    const hostWith = { loadModule: vi.fn(async () => ({ activate: vi.fn(async () => undefined) })) };
    const repoWith: PluginRepository = {
      ...pluginRepo,
  ...
  it('activation failure marks the plugin error and is isolated (throws domain error)', async () => {
    const hostWith = { loadModule: vi.fn(async () => ({ activate: vi.fn(async () => { throw new Error('boom'); }) })) };
    const repoWith: PluginRepository = {
      ...pluginRepo,
      findById: vi.fn(async () => makePlugin()),
      updateStatus: vi.fn(async (id, status) => makePlugin({ id, status })),
    ...
    const service = makeService({ host: hostWith, pluginRepo: repoWith });
    await expect(service.activatePlugin('p1', 'u1')).rejects.toMatchObject({ code: 'PLUGIN_ACTIVATION_FAILED' });
  ...
  it('settings: secrets encrypted at rest and masked on read', async () => {
    ...
    const settingRepoWith: PluginSettingRepository = {
      set: vi.fn(async (pluginId, key, value, encryptedValue, isSecret) => {
        settingsStored.push({ id: 's', pluginId, key, value, encryptedValue, isSecret, updatedAt: new Date(), createdAt: new Date() });
      }),
      listForPlugin: vi.fn(async () => settingsStored),
      getSecret: vi.fn(async () => settingsStored.find((s) => s.isSecret)?.encryptedValue || null),
    };
    const repoWith: PluginRepository = {
      ...pluginRepo,
      findById: vi.fn(async () => makePlugin({
        settingsSchema: { apiToken: { type: 'string', secret: true }, logLevel: { type: 'string', secret: false } },
      })),
    };
    const service = makeService({ settingRepo: settingRepoWith, pluginRepo: repoWith });

    await service.setSettings('p1', { apiToken: 'secret-token', logLevel: 'debug' });
  ...
  it('settings: replace-only for secrets (existing kept when value empty)', async () => {
    ...
    const repoWith: PluginRepository = {
      ...pluginRepo,
      findById: vi.fn(async () => makePlugin({ settingsSchema: { apiToken: { type: 'string', secret: true } } })),
    };
    const service = makeService({ settingRepo: settingRepoWith, pluginRepo: repoWith });
