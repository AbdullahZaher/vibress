...
import { PlansService, PlanDomainError } from '../src/application/plans-service';
...
describe('PlansService', () => {
  const repo: PlanRepository = {
    create: vi.fn(async (d) => makePlan({ key: d.key, billingType: d.billingType || 'recurring', billingInterval: d.billingInterval || null, amountMinor: d.amountMinor || 0, currency: d.currency || 'USD' })),
    findById: vi.fn(async () => null),
    findByKey: vi.fn(async () => null),
    update: vi.fn(async (id) => makePlan()),
    archive: vi.fn(async (id) => makePlan({ status: 'archived', archivedAt: new Date() })),
  ...
  it('rejects an invalid billing interval', async () => {
    await expect(
      service.createPlan({ productId: 'product-1', key: 'weekly', name: 'Weekly', billingInterval: 'week', amountMinor: 500, currency: 'USD' }, null)
    ).rejects.toMatchObject({ code: 'VALIDATION_ERROR' });
  });

  it('rejects a negative amount', async () => {
    await expect(
      service.createPlan({ productId: 'product-1', key: 'neg', name: 'Neg', billingInterval: 'month', amountMinor: -5, currency: 'USD' }, null)
    ).rejects.toMatchObject({ code: 'VALIDATION_ERROR' });
  });

  it('rejects trial days over the maximum bound', async () => {
    await expect(
      service.createPlan({ productId: 'product-1', key: 'trial', name: 'Trial', billingInterval: 'month', amountMinor: 500, currency: 'USD', trialDays: 500 }, null)
    ).rejects.toMatchObject({ code: 'VALIDATION_ERROR' });
  });
  ...
  it('rejects a plan for a missing product', async () => {
    ...
    await expect(
      missing.createPlan({ productId: 'nope', key: 'x', name: 'X', billingInterval: 'month', amountMinor: 500, currency: 'USD' }, null)
  ...
  it('rejects a duplicate plan key within a product', async () => {
    ...
    await expect(
      dupService.createPlan({ productId: 'product-1', key: 'monthly', name: 'Monthly', billingInterval: 'month', amountMinor: 1000, currency: 'USD' }, null)
    ).rejects.toMatchObject({ code: 'VALIDATION_ERROR' });
  });
