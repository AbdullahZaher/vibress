...
export function validateAnalyticsEvent(data: IngestEventData): IngestEventData {
  ...
  if (
    ...
    !ALLOWED_EVENT_NAMES.includes(
      data.eventName as (typeof ALLOWED_EVENT_NAMES)[number],
    )
  ) {
    throw new AnalyticsDomainError(
      "INVALID_EVENT",
      `Unknown analytics event: ${data.eventName}`,
    );
  }
  ...
  if (
    data.properties &&
    Object.keys(data.properties).length > MAX_PROPERTY_KEYS
  ) {
    throw new AnalyticsDomainError(
      "INVALID_EVENT",
      "properties has too many keys",
    );
  }
  ...
  if (data.path !== undefined && data.path !== null) {
    if (
      ...
      throw new AnalyticsDomainError(
        "INVALID_EVENT",
        "path must be a short normalized path",
      );
    }
  }
  if (data.visitorHash !== undefined && data.visitorHash !== null) {
    if (
      typeof data.visitorHash !== "string" ||
      data.visitorHash.length > 64 ||
      !/^[0-9a-f]{64}$/.test(data.visitorHash)
    ) {
      throw new AnalyticsDomainError(
        "INVALID_EVENT",
        "visitorHash must be a 64-char hex digest",
      );
    }
  }
  if (data.referrerDomain !== undefined && data.referrerDomain !== null) {
    if (
      typeof data.referrerDomain !== "string" ||
      data.referrerDomain.length > 253
    ) {
      throw new AnalyticsDomainError(
        "INVALID_EVENT",
        "referrerDomain must be a short domain",
      );
    }
  }
...
export class AnalyticsService {
  ...
  async ingest(data: IngestEventData): Promise<void> {
    ...
    domainEvents.emit("analytics.event_ingested", {
      eventId: validated.eventId,
      eventName: validated.eventName,
      isBot: !!validated.isBot,
    });
  }
  ...
  async getMetrics(
    ...
    metrics: Array<{ date: string; name: string; count: number }>;
    from: string;
    to: string;
    timezone: string;
  }> {
    ...
    if (query.metricName) {
      ...
      for (const name of names) {
        ...
        for (const r of rows)
          all.push({ date: r.metricDate, name: r.metricName, count: r.count });
      }
      metrics = all;
    }
