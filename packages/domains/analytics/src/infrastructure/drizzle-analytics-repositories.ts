...
export class DrizzleAnalyticsRepository implements AnalyticsRepository {
  ...
  async upsertDailyMetric(metric: {
    ...
    await db
      ...
      .onConflictDoUpdate({
        target: [
          analyticsDailyMetrics.metricDate,
          analyticsDailyMetrics.metricName,
          analyticsDailyMetrics.dimensionKey,
          analyticsDailyMetrics.dimensionValue,
        ],
        set: {
          count: sql`${analyticsDailyMetrics.count} + ${metric.count}`,
          updatedAt: new Date(),
        },
      });
  }
