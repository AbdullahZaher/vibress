...
describe("normalizePath", () => {
  it("strips query strings and fragments", () => {
    expect(normalizePath("/posts/hello?utm_source=x&token=abc")).toBe(
      "/posts/hello",
    );
  ...
  it("ensures a leading slash and bounds length", () => {
    expect(normalizePath("posts/hello")).toBe("/posts/hello");
    expect(normalizePath(`/x${"a".repeat(600)}`).length).toBeLessThanOrEqual(
      512,
    );
  });
});
...
describe("normalizeReferrerDomain", () => {
  it("returns bare domains without query strings or paths", () => {
    expect(
      normalizeReferrerDomain("https://www.google.com/search?q=vibress"),
    ).toBe("google.com");
    expect(normalizeReferrerDomain("https://github.com/foo/bar?ref=x")).toBe(
      "github.com",
    );
  });
...
describe("deriveVisitorHash", () => {
  ...
  it("differs for different ids and never contains the raw id", () => {
    ...
    expect(deriveVisitorHash("id-1", "secret")).not.toBe(
      deriveVisitorHash("id-2", "secret"),
    );
  });
});
