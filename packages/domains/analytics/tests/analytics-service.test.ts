...
} from "../src/application/analytics-service";
...
describe("validateAnalyticsEvent", () => {
  ...
  it("rejects an unknown event name (bounded envelope)", () => {
    const event: IngestEventData = {
      eventId: "evt-2",
      eventName: "hacked.event",
    };
    expect(() => validateAnalyticsEvent(event)).toThrowError(
  ...
  it("rejects a missing eventId", () => {
    expect(() =>
      validateAnalyticsEvent({ eventName: "post.view" } as any),
...
describe("AnalyticsService", () => {
  ...
  it("ingest aggregates into daily metrics (UTC date bucket)", async () => {
    ...
    await service.ingest({
      eventId: "evt-x",
      eventName: "member.signup",
      occurredAt: new Date("2026-08-08T10:00:00Z"),
    });
    expect(repo.upsertDailyMetric).toHaveBeenCalledWith(
  ...
  it("rebuild re-aggregates a day from raw events", async () => {
    ...
    expect(repo.rebuildDay).toHaveBeenCalledWith(
      "2026-08-08",
      expect.any(Array),
    );
  });
