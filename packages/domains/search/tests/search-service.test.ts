...
import {
  ...
  SearchDomainError,
  sanitizeSearchQuery,
} from "../src/application/search-service";
import {
  SearchRepository,
  SearchDocumentInput,
  SearchResult,
...
describe("sanitizeSearchQuery", () => {
  ...
  it("rejects queries over the length bound", () => {
    expect(() => sanitizeSearchQuery("a".repeat(200))).toThrowError(
      expect.objectContaining({ code: "QUERY_TOO_LONG" }),
    );
  });

  it("rejects empty queries", () => {
    expect(() => sanitizeSearchQuery("   ")).toThrowError(
...
describe("SearchService", () => {
  ...
  it("rebuild clears the index and re-indexes all content", async () => {
    ...
    const count = await service.rebuild({
      listIndexableContent: async () => docs,
    });
    expect(count).toBe(3);
