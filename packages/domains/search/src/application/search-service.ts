import { SearchRepository, SearchDocumentInput, SearchResult } from '../domain/search';
...
export function sanitizeSearchQuery(q: string): string {
  ...
  if (trimmed.length > MAX_QUERY_LENGTH) {
    throw new SearchDomainError('QUERY_TOO_LONG', `Query exceeds ${MAX_QUERY_LENGTH} characters`);
  }
  // Normalize whitespace, strip control characters (charCodeAt loop), collapse spaces
  let cleaned = '';
  for (let i = 0; i < trimmed.length; i++) {
    const code = trimmed.charCodeAt(i);
    if (code < 32 || code === 127) { cleaned += ' '; continue; }
    cleaned += trimmed[i];
  }
...
export class SearchService {
  ...
  async search(q: string, limit = 20, offset = 0): Promise<{ results: SearchResult[]; total: number }> {
    const sanitized = sanitizeSearchQuery(q);
    return this.repo.query(sanitized, Math.min(Math.max(limit, 1), 50), Math.max(offset, 0));
  }

  async indexDocument(doc: SearchDocumentInput): Promise<void> {
    if (!doc.title.trim()) return;
    await this.repo.upsert(doc);
    domainEvents.emit('search.indexed', { entityType: doc.entityType, entityId: doc.entityId });
  }
