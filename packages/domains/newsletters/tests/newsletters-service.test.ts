...
import {
  NewslettersService,
  NewsletterDomainError,
} from "../src/application/newsletters-service";
...
describe("NewslettersService", () => {
  const newsletterRepo: NewsletterRepository = {
    ...
    archive: vi.fn(async (id) =>
      makeNewsletter({ status: "archived", archivedAt: new Date() }),
    ),
    list: vi.fn(async () => []),
  };
  ...
  const sendRepo: SendRepository = {
    create: vi.fn(async (d) =>
      makeSend({ subject: d.subject, newsletterId: d.newsletterId }),
    ),
  ...
  it("creates a newsletter with a normalized key", async () => {
    ...
    const nl = await service.createNewsletter(
      {
        key: "Weekly",
        name: "Weekly",
        senderName: "V",
        senderEmail: "n@example.com",
      },
      "u1",
    );
  ...
  it("rejects a newsletter with an invalid sender email", async () => {
    ...
    await expect(
      service.createNewsletter(
        { key: "ok", name: "X", senderName: "V", senderEmail: "not-an-email" },
        null,
      ),
    ).rejects.toMatchObject({ code: "VALIDATION_ERROR" });
  });
  ...
  it("rejects a duplicate newsletter key", async () => {
    const dupRepo = {
      ...newsletterRepo,
      findByKey: vi.fn(async () => makeNewsletter()),
    };
  ...
  it("rejects subscribing to an archived newsletter", async () => {
    const archivedRepo = {
      ...newsletterRepo,
      findById: vi.fn(async () => makeNewsletter({ status: "archived" })),
    };
    const service = makeService({ newsletterRepo: archivedRepo });
    await expect(
      service.setSubscription("m1", "nl-1", true),
    ).rejects.toMatchObject({ code: "NEWSLETTER_NOT_FOUND" });
  });
  ...
  it("computes audience: excludes unsubscribed, suppressed, and unverified members", async () => {
    ...
    const audRepo: MemberAudienceRepository = {
      listAudienceMembers: vi.fn(async () => members),
    };
    ...
    const audience = await service.computeAudience("nl-1", {
      filter: "all",
      productId: null,
      planId: null,
    });
  ...
  it("starts a send idempotently (snapshot once)", async () => {
    ...
    const service = makeService({
      audienceRepo: audRepo,
      preferenceRepo: prefRepoAll,
      sendRepo: sendRepoWith,
    });

    let recipientCalls = 0;
    const { recipientCount } = await service.startSend(
      "send-1",
      async (rows) => {
        recipientCalls++;
        return rows.length;
  ...
  it("unsubscribes with a valid scoped token", async () => {
    ...
    expect(prefRepoWith.setSubscription).toHaveBeenCalledWith(
      "m1",
      "nl-1",
      false,
    );
  ...
  it("rejects a forged unsubscribe token", async () => {
    const service = makeService();
    await expect(
      service.unsubscribeWithToken("forged-token"),
