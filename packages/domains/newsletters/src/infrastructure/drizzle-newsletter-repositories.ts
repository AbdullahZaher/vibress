import { getDb, newsletters, NewsletterRow, newsletterPreferences, NewsletterPreferenceRow, newsletterSends, NewsletterSendRow } from '@vibress/database';
...
export class DrizzleNewsletterRepository implements NewsletterRepository {
  ...
  async findByKey(key: string): Promise<Newsletter | null> {
    const db = getDb();
    const rows = await db.select().from(newsletters).where(eq(newsletters.key, key)).limit(1);
    const row = rows[0];
    if (!row) return null;
    return this.mapToDomain(row);
  }
  ...
  async update(id: string, data: UpdateNewsletterData): Promise<Newsletter> {
    ...
    const [row] = await db.update(newsletters).set(payload).where(eq(newsletters.id, id)).returning();
  ...
  async list(filter?: { includeArchived?: boolean }): Promise<Newsletter[]> {
    ...
    const rows = await db
      .select()
      .from(newsletters)
      .where(filter?.includeArchived ? undefined : isNull(newsletters.archivedAt))
...
export class DrizzleNewsletterPreferenceRepository implements NewsletterPreferenceRepository {
  async setSubscription(memberId: string, newsletterId: string, subscribed: boolean): Promise<NewsletterPreference> {
    ...
    const existing = await db
      ...
      .where(and(eq(newsletterPreferences.memberId, memberId), eq(newsletterPreferences.newsletterId, newsletterId)))
      .limit(1);
    ...
    if (existing[0]) {
      const [row] = await db
        ...
        .set({
          subscribed,
          subscribedAt: subscribed ? (existing[0].subscribedAt || now) : existing[0].subscribedAt,
  ...
  async get(memberId: string, newsletterId: string): Promise<NewsletterPreference | null> {
    const db = getDb();
    const rows = await db
      .select()
      .from(newsletterPreferences)
  ...
  async listForMember(memberId: string): Promise<NewsletterPreference[]> {
    const db = getDb();
    const rows = await db.select().from(newsletterPreferences).where(eq(newsletterPreferences.memberId, memberId));
    return rows.map((r) => this.mapToDomain(r));
  }
...
export class DrizzleSendRepository implements SendRepository {
  ...
  async findById(id: string): Promise<NewsletterSend | null> {
    const db = getDb();
    const rows = await db.select().from(newsletterSends).where(eq(newsletterSends.id, id)).limit(1);
    const row = rows[0];
    if (!row) return null;
    return this.mapToDomain(row);
  }

  async updateStatus(id: string, status: SendStatus, patch?: Partial<NewsletterSend>): Promise<NewsletterSend> {
    const db = getDb();
    const payload: Record<string, unknown> = { status, updatedAt: new Date() };
    if (patch) {
      if (patch.totalRecipients !== undefined) payload.totalRecipients = patch.totalRecipients;
      if (patch.sentRecipients !== undefined) payload.sentRecipients = patch.sentRecipients;
      if (patch.failedRecipients !== undefined) payload.failedRecipients = patch.failedRecipients;
    ...
    const [row] = await db.update(newsletterSends).set(payload).where(eq(newsletterSends.id, id)).returning();
    if (!row) throw new Error(`Send not found: ${id}`);
    return this.mapToDomain(row);
  }

  async list(filter?: { status?: SendStatus; newsletterId?: string; limit?: number; offset?: number }): Promise<{ sends: NewsletterSend[]; total: number }> {
    const db = getDb();
    const limit = Math.min(filter?.limit || 20, 100);
    const offset = filter?.offset || 0;
    const conditions = [];
    if (filter?.status) conditions.push(eq(newsletterSends.status, filter.status));
    if (filter?.newsletterId) conditions.push(eq(newsletterSends.newsletterId, filter.newsletterId));
    const whereClause = conditions.length ? and(...conditions) : undefined;

    const countRes = await db.select({ total: count() }).from(newsletterSends).where(whereClause);
    const rows = await db.select().from(newsletterSends).where(whereClause).orderBy(desc(newsletterSends.createdAt)).limit(limit).offset(offset);
    return { sends: rows.map((r) => this.mapToDomain(r)), total: Number(countRes[0]?.total || 0) };
  }
  ...
  async findDueScheduled(now: Date, limit: number): Promise<NewsletterSend[]> {
    ...
    const rows = await db
      ...
      .where(and(eq(newsletterSends.status, 'scheduled'), lte(newsletterSends.scheduledAt, now)))
