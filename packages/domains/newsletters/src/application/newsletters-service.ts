...
export class NewslettersService {
  ...
  async createNewsletter(
    ...
    if (!/^[a-z0-9][a-z0-9-]*$/.test(key)) {
      throw new NewsletterDomainError(
        "VALIDATION_ERROR",
        "Newsletter key must be lowercase alphanumeric with hyphens",
      );
    }
    if (data.name.trim().length > 100) {
      throw new NewsletterDomainError(
        "VALIDATION_ERROR",
        "Newsletter name is too long",
      );
    }
  ...
  async setSubscription(
    ...
    const pref = await this.deps.preferenceRepo.setSubscription(
      memberId,
      newsletterId,
      subscribed,
    );
  ...
  async resolveAudience(
    ...
    if (pref) return pref;
    return this.deps.preferenceRepo.setSubscription(
      memberId,
      newsletterId,
      true,
    );
  ...
  async computeAudience(
    ...
    const all = await this.deps.audienceRepo.listAudienceMembers(
      definition.filter,
  ...
  async createSend(
    ...
    const send = await this.deps.sendRepo.create({
      ...data,
      createdBy: actorId || undefined,
    });
    if (send.status === "scheduled") {
      domainEvents.emit("newsletter.send_scheduled", {
        sendId: send.id,
  ...
  ): Promise<{ send: NewsletterSend; recipientCount: number }> {
    return runInTransaction(() => this.startSendTx(sendId, createRecipients));
  }

  private async startSendTx(
    sendId: string,
    createRecipients: (
      rows: Array<{
        memberId: string | null;
    ...
    const audience = await this.computeAudience(
      send.newsletterId,
      send.audience,
    );
    ...
    domainEvents.emit("newsletter.send_started", {
      sendId,
      newsletterId: send.newsletterId,
    });
  ...
  async completeSend(sendId: string): Promise<NewsletterSend> {
    ...
    if (!send)
      throw new NewsletterDomainError("SEND_NOT_FOUND", "Send not found");
    const updated = await this.deps.sendRepo.updateStatus(sendId, "sent", {
      completedAt: new Date(),
    });
    domainEvents.emit("newsletter.sent", {
      sendId,
      newsletterId: send.newsletterId,
    });
    return updated;
  ...
  async failSend(sendId: string, error: string): Promise<NewsletterSend> {
    const updated = await this.deps.sendRepo.updateStatus(sendId, "failed", {
      lastError: error.slice(0, 500),
      completedAt: new Date(),
    });
    domainEvents.emit("newsletter.send_failed", {
      sendId,
      newsletterId: updated.newsletterId,
      error,
    });
    return updated;
  }
  ...
  async unsubscribeWithToken(
    ...
    const pref = await this.deps.preferenceRepo.get(
      claims.memberId,
      newsletterId,
    );
    if (!pref || pref.subscribed) {
      ...
      domainEvents.emit("member.newsletter_unsubscribed", {
  ...
  private unwrapToken(
    token: string,
  ): { memberId: string; sendId: string } | null {
  ...
  renderEmailHtml(
    ...
    const bodyHtml = renderStudioDocumentToHtml(send.content, {
      target: "email",
    });
