...
describe("SettingsService", () => {
  ...
  it("updates a valid typed setting", async () => {
    ...
    const record = await service.updateSetting(
      "site",
      "title",
      "New Title",
      "u1",
  ...
  it("rejects an unknown namespace", async () => {
    const service = makeService();
    await expect(
      service.updateSetting("nope", "title", "x", null),
  ...
  it("rejects an unknown setting key", async () => {
    const service = makeService();
    await expect(
      service.updateSetting("site", "nope", "x", null),
  ...
  it("rejects an invalid value per schema validation", async () => {
    const service = makeService();
    await expect(
      service.updateSetting("publishing", "postsPerPage", 0, null),
    ).rejects.toMatchObject({ code: "VALIDATION_ERROR" });
    await expect(
      service.updateSetting("publishing", "defaultPostStatus", "banana", null),
    ).rejects.toMatchObject({ code: "VALIDATION_ERROR" });
    await expect(
      service.updateSetting("email", "fromEmail", "not-an-email", null),
    ).rejects.toMatchObject({ code: "VALIDATION_ERROR" });
  });
  ...
  it("coerces numbers and booleans", async () => {
    ...
    const boolRecord = await service.updateSetting(
      "members",
      "signupEnabled",
      "false",
      null,
  ...
  it("updates extended site branding and navigation settings", async () => {
    const service = makeService();
    const color = await service.updateSetting(
      "site",
      "accentColor",
      "#10b981",
      "u1",
    ...
    const tz = await service.updateSetting(
      "site",
      "timezone",
      "Asia/Riyadh",
      "u1",
    ...
    const nav = await service.updateSetting(
      "site",
      "primaryNav",
      JSON.stringify([{ label: "Home", url: "/" }]),
      "u1",
    );
    expect(nav.value).toEqual([{ label: "Home", url: "/" }]);

    const announce = await service.updateSetting(
      "site",
      "announcementEnabled",
      true,
      "u1",
  ...
  it("updates security settings (isPrivate, passwordHash)", async () => {
    const service = makeService();
    const isPrivate = await service.updateSetting(
      "security",
      "isPrivate",
      true,
      "u1",
    ...
    const hash = await service.updateSetting(
      "security",
      "passwordHash",
      "argon2id$hashed",
      "u1",
  ...
  it("updates analytics, comments, and code injection settings", async () => {
    const service = makeService();
    const ga = await service.updateSetting(
      "analytics",
      "gaId",
      "G-1234567890",
      "u1",
    ...
    const comments = await service.updateSetting(
      "comments",
      "commentAccess",
      "paid",
      "u1",
    ...
    const code = await service.updateSetting(
      "code",
      "headerCode",
  ...
  it("getPublicSettings only returns public classification", async () => {
    const repoWith: SettingRepository = {
      ...
      getMany: vi.fn(async (namespace) => {
        if (namespace === "site")
          return [makeRecord({ value: "Public Title" })];
        return [];
      }),
    };
  ...
  it("audit entries are recorded for changes (no raw secret values)", async () => {
    ...
    expect(auditWith.record).toHaveBeenCalledWith(
      expect.objectContaining({
