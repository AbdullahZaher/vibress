...
export const SETTING_NAMESPACES: SettingNamespaceDefinition[] = [
  {
    ...
    settings: [
      ...
      },
      {
        ...
        default: "en",
        validate: (v) =>
          /^[a-z]{2}(-[A-Za-z0-9]{2,8})*$/.test(String(v))
            ? null
            : "Invalid locale",
      },
      {
      ...
      },
      {
        ...
        default: "#6366f1",
        validate: (v) =>
          /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(String(v))
            ? null
            : "Invalid hex color",
      },
    ...
    settings: [
      {
        ...
        default: "draft",
        validate: (v) =>
          ["draft", "published"].includes(String(v)) ? null : "Invalid status",
      },
      {
        key: "postsPerPage",
        type: "number",
        ...
        default: 10,
        validate: (v) =>
          Number(v) > 0 && Number(v) <= 100 ? null : "Must be 1-100",
      },
    ],
  },
  {
    ...
    settings: [
      {
        ...
        default: "USD",
        validate: (v) =>
          /^[A-Z]{3}$/.test(String(v)) ? null : "Invalid currency",
      },
      {
    ...
    settings: [
      {
        ...
        default: 500,
        validate: (v) =>
          Number(v) > 0 && Number(v) <= 4096 ? null : "Must be 1-4096",
      },
    ],
  },
  {
    ...
    settings: [
      {
        ...
        default: 5,
        validate: (v) =>
          Number(v) >= 0 && Number(v) <= 20 ? null : "Must be 0-20",
      },
    ],
  },
  {
    ...
    settings: [
      ...
      {
        key: "memberSessionTtlHours",
        type: "number",
        classification: "staff-visible",
        default: 720,
        validate: (v) =>
          Number(v) >= 1 && Number(v) <= 8760 ? null : "Must be 1-8760",
      },
      {
        key: "authRateLimitPerMinute",
        type: "number",
        classification: "internal",
        default: 20,
        validate: (v) => (Number(v) >= 1 ? null : "Must be positive"),
      },
    ],
  },
  {
    ...
    settings: [
      {
        ...
        default: "all",
        validate: (v) =>
          ["all", "paid", "disabled"].includes(String(v))
            ? null
            : "Invalid comment access",
      },
      {
        key: "preModeration",
        type: "boolean",
...
const NAMESPACE_MAP = new Map(
  SETTING_NAMESPACES.map((ns) => [ns.namespace, ns]),
);
...
export class SettingsService {
  ...
  maskForStaff(value: unknown, classification: string): unknown {
    if (classification === "secret" || classification === "internal")
      return "••••••••";
    return value;
  }
  ...
  async getPublicSettings(): Promise<Record<string, Record<string, unknown>>> {
    ...
    for (const ns of SETTING_NAMESPACES) {
      ...
      for (const def of ns.settings) {
        ...
        publicValues[def.key] = storedSetting
          ? storedSetting.value
          : def.default;
      }
      if (Object.keys(publicValues).length > 0)
        result[ns.namespace] = publicValues;
    }
    return result;
  }
   ...
   */
  async getStaffSettings(): Promise<
    Array<{
      namespace: string;
  ...
  async updateSetting(
    ...
    return runInTransaction(() =>
      this.updateSettingTx(namespace, key, value, actorId),
    );
  }

  private async updateSettingTx(
    namespace: string,
    key: string,
    value: unknown,
    actorId: string | null,
    ...
    if (!ns)
      throw new SettingsDomainError(
        "UNKNOWN_NAMESPACE",
        `Unknown settings namespace: ${namespace}`,
      );
      ...
      case "json":
        ...
        if (typeof value === "string") {
          try {
            ...
            throw new SettingsDomainError(
              "VALIDATION_ERROR",
              `${def.key} must be valid JSON`,
            );
          }
        }
        throw new SettingsDomainError(
          "VALIDATION_ERROR",
          `${def.key} must be JSON`,
        );
      case "string":
        return String(value ?? "");
      default:
