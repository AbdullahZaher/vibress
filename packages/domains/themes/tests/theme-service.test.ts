...
const registry: ThemeDefinitionRegistry = {
  ...
  get: (id) => {
    if (id === defaultThemeManifest.id) return { manifest: defaultThemeManifest, settingsSchema: defaultThemeSettingsSchema };
    if (id === minimalThemeManifest.id) return { manifest: minimalThemeManifest, settingsSchema: minimalThemeSettingsSchema };
    return null;
  },
  list: () => [
    { manifest: defaultThemeManifest, settingsSchema: defaultThemeSettingsSchema },
    { manifest: minimalThemeManifest, settingsSchema: minimalThemeSettingsSchema },
  ],
  validate: (manifest: unknown) => {
    if (typeof manifest === 'object' && manifest && (manifest as any).id) return manifest as any;
    throw new ThemeError('THEME_INVALID', 'invalid');
  },
  checkCompatibility: (manifest: any) => {
    if (manifest.themeApi !== 1) throw new ThemeError('THEME_INCOMPATIBLE', 'incompatible');
  },
};
...
describe('Theme Domain — Activation & Settings', () => {
  ...
  it('rejects unknown theme and retains previous active theme', async () => {
    ...
    await expect(service.activateTheme('does-not-exist', 'actor-1')).rejects.toThrow(ThemeError);
  ...
  it('rejects incompatible theme API version', async () => {
    ...
    await expect(badService.activateTheme('vibress-minimal', 'actor-1')).rejects.toThrow(ThemeError);
    const stillActive = await service.getActiveThemeConfiguration();
    expect(stillActive?.themeId).toBe('vibress-default');
  });
  ...
  it('updates settings with validation and rejects invalid values', async () => {
    ...
    await expect(
      service.updateThemeSettings('vibress-minimal', { accentColor: 'javascript:evil' }, 'actor-1')
    ).rejects.toThrow(ThemeError);

    await expect(
      service.updateThemeSettings('vibress-minimal', { notASetting: true }, 'actor-1')
    ).rejects.toThrow(ThemeError);
  });
