 ...
 * versions.
 */
export function validateImportEnvelope(envelope: unknown): {
  format: string;
  version: number;
  ...
  if (e.format !== "vibress") {
    throw new ImportExportDomainError(
      "INVALID_FORMAT",
      `Unsupported format: ${String(e.format)}`,
    );
  }
  ...
  if (!Number.isInteger(version) || version !== IMPORT_FORMAT_VERSION) {
    throw new ImportExportDomainError(
      "UNSUPPORTED_VERSION",
      `Unsupported import version: ${String(e.version)}`,
    );
  }
  if (!e.data || typeof e.data !== "object" || Array.isArray(e.data)) {
    throw new ImportExportDomainError(
      "INVALID_FORMAT",
      "Import payload is missing a data object",
    );
  }
  return {
    format: "vibress",
...
export interface ImportProcessor {
  process(
    data: Record<string, unknown>,
  ): Promise<{ posts: number; pages: number; tags: number; redirects: number }>;
}
...
export class ImportExportService {
  ...
  async cancelJob(id: string): Promise<void> {
    ...
    if (!job)
      throw new ImportExportDomainError("JOB_NOT_FOUND", "Job not found");
  ...
  async runExport(
    jobId: string,
  ): Promise<{ envelope: unknown; artifactKey: string }> {
    const job = await this.jobRepo.findById(jobId);
    if (!job)
      throw new ImportExportDomainError("JOB_NOT_FOUND", "Job not found");
