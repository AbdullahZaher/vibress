...
export class DrizzleImportExportJobRepository implements JobRepository {
  ...
  async list(
    ...
    if (filter.status)
      conditions.push(eq(importExportJobs.status, filter.status));
    ...
    const rows = await db
      ...
      .offset(offset);
    return {
      jobs: rows.map((r) => this.mapToDomain(r)),
  ...
  async updateStatus(
    ...
    if (patch?.errorSummary !== undefined)
      payload.errorSummary = patch.errorSummary;
    if (patch?.artifactKey !== undefined)
      payload.artifactKey = patch.artifactKey;
    if (patch?.artifactExpiresAt !== undefined)
      payload.artifactExpiresAt = patch.artifactExpiresAt;
