...
export class DrizzleSubscriptionRepository implements SubscriptionRepository {
  ...
  async findByProviderSubscriptionId(provider: string, providerSubscriptionId: string): Promise<Subscription | null> {
    const db = getDb();
    const rows = await db
      .select()
      .from(subscriptions)
      .where(and(eq(subscriptions.provider, provider), eq(subscriptions.providerSubscriptionId, providerSubscriptionId)))
      .limit(1);
    const row = rows[0];
    if (!row) return null;
    return this.mapToDomain(row);
  ...
  async update(id: string, data: UpdateSubscriptionData): Promise<Subscription> {
    ...
    if (data.providerSubscriptionId !== undefined) updatePayload.providerSubscriptionId = data.providerSubscriptionId;
    if (data.providerCustomerId !== undefined) updatePayload.providerCustomerId = data.providerCustomerId;
    if (data.currentPeriodStart !== undefined) updatePayload.currentPeriodStart = data.currentPeriodStart;
    if (data.currentPeriodEnd !== undefined) updatePayload.currentPeriodEnd = data.currentPeriodEnd;
    if (data.trialStart !== undefined) updatePayload.trialStart = data.trialStart;
    if (data.trialEnd !== undefined) updatePayload.trialEnd = data.trialEnd;
    if (data.cancelAtPeriodEnd !== undefined) updatePayload.cancelAtPeriodEnd = data.cancelAtPeriodEnd;
    ...
    if (data.providerEventTimestamp !== undefined) updatePayload.providerEventTimestamp = data.providerEventTimestamp;

    const [row] = await db.update(subscriptions).set(updatePayload).where(eq(subscriptions.id, id)).returning();
    if (!row) throw new Error(`Subscription not found: ${id}`);
    return this.mapToDomain(row);
  }

  async list(filter: ListSubscriptionsFilter = {}): Promise<{ subscriptions: Subscription[]; total: number }> {
    const db = getDb();
    const limit = Math.min(filter.limit || 20, 100);
    const offset = filter.offset || 0;

    const conditions = [];
    if (filter.memberId) conditions.push(eq(subscriptions.memberId, filter.memberId));
    if (filter.status) conditions.push(eq(subscriptions.status, filter.status));
    if (filter.productId) conditions.push(eq(subscriptions.productId, filter.productId));
  ...
  async findActiveForMember(memberId: string, productId: string): Promise<Subscription | null> {
    const db = getDb();
    const rows = await db
      .select()
      .from(subscriptions)
  ...
  async hasActiveForMember(memberId: string): Promise<boolean> {
    ...
    const rows = await db
      ...
      .from(subscriptions)
      .where(and(eq(subscriptions.memberId, memberId), inArray(subscriptions.status, ACCESS_GRANTING_STATUSES)))
      .limit(1);
