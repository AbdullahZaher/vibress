...
import { SubscriptionsService, SubscriptionDomainError } from '../src/application/subscriptions-service';
...
describe('SubscriptionsService', () => {
  const repo: SubscriptionRepository = {
    ...
    update: vi.fn(async (id, d) => makeSubscription({ id, ...(d as Record<string, unknown>) })),
  ...
  it('ignores out-of-order provider events (older timestamps)', async () => {
    const current = makeSubscription({ providerEventTimestamp: new Date('2026-06-10') });
    const outOfOrderRepo: SubscriptionRepository = {
      ...repo,
      findById: vi.fn(async () => current),
    };
    const s = new SubscriptionsService(outOfOrderRepo);
    const result = await s.applyProviderUpdate('sub-1', { status: 'cancelled' }, new Date('2026-06-01'));
  ...
  it('applies newer provider events', async () => {
    const current = makeSubscription({ providerEventTimestamp: new Date('2026-06-01') });
    const newerRepo: SubscriptionRepository = {
      ...repo,
      findById: vi.fn(async () => current),
      update: vi.fn(async (id, d) => makeSubscription({ ...(d as Record<string, unknown>), id })),
    };
    const s = new SubscriptionsService(newerRepo);
    const result = await s.applyProviderUpdate('sub-1', { status: 'cancelled' }, new Date('2026-06-10'));
  ...
  it('rejects cancellation of an already-cancelled subscription', async () => {
    ...
    await expect(s.cancelAtPeriodEnd('sub-1')).rejects.toMatchObject({ code: 'SUBSCRIPTION_NOT_CANCELLABLE' });
  });

  it('resumes a scheduled cancellation', async () => {
    const scheduledRepo: SubscriptionRepository = {
      ...repo,
      findById: vi.fn(async () => makeSubscription({ cancelAtPeriodEnd: true })),
    };
  ...
  it('rejects resume when not scheduled for cancellation', async () => {
    await expect(service.resume('sub-1')).rejects.toMatchObject({ code: 'SUBSCRIPTION_NOT_RESUMABLE' });
  });

  it('rejects resume of a cancelled subscription', async () => {
    const cancelledRepo: SubscriptionRepository = {
      ...repo,
      findById: vi.fn(async () => makeSubscription({ status: 'cancelled', cancelAtPeriodEnd: true })),
    };
    const s = new SubscriptionsService(cancelledRepo);
    await expect(s.resume('sub-1')).rejects.toMatchObject({ code: 'SUBSCRIPTION_NOT_RESUMABLE' });
  });

  it('access policy: trialing and active grant access; past_due does not', () => {
    expect(service.hasAccess(makeSubscription({ status: 'trialing' }))).toBe(true);
    expect(service.hasAccess(makeSubscription({ status: 'active' }))).toBe(true);
    expect(service.hasAccess(makeSubscription({ status: 'past_due' }))).toBe(false);
    expect(service.hasAccess(makeSubscription({ status: 'unpaid' }))).toBe(false);
    expect(service.hasAccess(makeSubscription({ status: 'cancelled' }))).toBe(false);
    expect(service.hasAccess(makeSubscription({ status: 'expired' }))).toBe(false);
    expect(service.hasAccess(makeSubscription({ status: 'incomplete' }))).toBe(false);
  });
});
