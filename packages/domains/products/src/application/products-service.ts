...
import { Product, CreateProductData, UpdateProductData } from '../domain/product';
...
export class ProductsService {
  ...
  async createProduct(data: CreateProductData, actorId: string | null): Promise<Product> {
    const key = data.key.trim().toLowerCase();
    if (!PRODUCT_KEY_REGEX.test(key)) {
      throw new ProductDomainError('VALIDATION_ERROR', 'Product key must be lowercase alphanumeric with hyphens');
    }
    if (data.name.trim().length > 100) {
      throw new ProductDomainError('VALIDATION_ERROR', 'Product name is too long');
    }
    const existing = await this.repo.findByKey(key);
    if (existing) {
      throw new ProductDomainError('VALIDATION_ERROR', 'Product key already exists');
    }
  ...
  async updateProduct(id: string, data: UpdateProductData, actorId: string | null): Promise<Product> {
  ...
  async listProducts(filter?: { status?: 'active' | 'archived'; includeArchived?: boolean }): Promise<Product[]> {
