import { getDb, permissions, rolePermissions, userRoles, roles } from '@vibress/database';
...
export class DrizzlePermissionRepository implements PermissionRepository {
  ...
  async findByKey(key: string): Promise<Permission | null> {
    const db = getDb();
    const rows = await db.select().from(permissions).where(eq(permissions.key, key)).limit(1);
    const row = rows[0];
    if (!row) return null;
    return this.mapToDomain(row);
  }
