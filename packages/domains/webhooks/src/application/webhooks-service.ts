import { WebhookRepository, WebhookEndpoint, WebhookDelivery } from '../domain/webhook';
...
export class WebhooksService {
  constructor(private repo: WebhookRepository, private dispatcher: DeliveryDispatcher) {}

  // ---------------- Endpoint management ----------------

  async createEndpoint(data: { name: string; url: string; secret?: string | null; eventTypes: string[] }, actorId: string | null): Promise<WebhookEndpoint> {
    if (!data.name.trim()) throw new WebhookDomainError('VALIDATION_ERROR', 'Name is required');
    if (!isSafeUrl(data.url)) {
      throw new WebhookDomainError('UNSAFE_URL', 'Webhook URL must be http/https and not point to a private/localhost address');
    }
    if (!Array.isArray(data.eventTypes) || data.eventTypes.length === 0) {
      throw new WebhookDomainError('VALIDATION_ERROR', 'At least one event type is required');
    }
    ...
    domainEvents.emit('webhook.endpoint_created', { endpointId: endpoint.id, actorId });
    return endpoint;
  }

  async updateEndpoint(id: string, data: { name?: string; url?: string; secret?: string | null; enabled?: boolean; eventTypes?: string[] }, actorId: string | null): Promise<WebhookEndpoint> {
    const existing = await this.repo.findEndpointById(id);
    if (!existing) throw new WebhookDomainError('WEBHOOK_NOT_FOUND', 'Webhook endpoint not found');
  ...
  async dispatchEvent(eventName: string, payload: unknown): Promise<number> {
    ...
    const eventPayload = JSON.stringify({ id: eventId, type: eventName, timestamp: new Date().toISOString(), data: payload });
    const payloadHash = crypto.createHash('sha256').update(eventPayload).digest('hex');
   ...
   */
  async deliver(deliveryId: string): Promise<{ status: string; responseStatus: number | null }> {
    const delivery = await this.getDeliveryWithEndpoint(deliveryId);
    if (!delivery) throw new WebhookDomainError('DELIVERY_NOT_FOUND', 'Delivery not found');
  ...
  async listDeliveries(filter?: { endpointId?: string; status?: string; limit?: number; offset?: number }): Promise<{ deliveries: WebhookDelivery[]; total: number }> {
  ...
  async retryFailedDeliveries(): Promise<number> {
    const { deliveries } = await this.repo.listDeliveries({ status: 'failed', limit: 100 });
    let retried = 0;
  ...
  private async getDeliveryWithEndpoint(deliveryId: string): Promise<{ deliveryRow: WebhookDelivery; endpoint: WebhookEndpoint } | null> {
