...
import {
  WebhooksService,
  WebhookDomainError,
} from "../src/application/webhooks-service";
import {
  WebhookRepository,
  WebhookEndpoint,
  WebhookDelivery,
} from "../src/domain/webhook";
...
describe("WebhooksService", () => {
  const repo: WebhookRepository = {
    createEndpoint: vi.fn(async (d) =>
      makeEndpoint({ name: d.name, url: d.url, eventTypes: d.eventTypes }),
    ),
  ...
  it("rejects a localhost webhook URL (SSRF)", async () => {
    ...
    await expect(
      service.createEndpoint(
        { name: "Bad", url: "http://localhost:8080/hook", eventTypes: ["x"] },
        null,
      ),
  ...
  it("rejects a private IP webhook URL (SSRF)", async () => {
    ...
    await expect(
      service.createEndpoint(
        { name: "Bad", url: "http://10.0.0.5/hook", eventTypes: ["x"] },
        null,
      ),
    ).rejects.toMatchObject({ code: "UNSAFE_URL" });
  });
  ...
  it("rejects a non-http webhook URL", async () => {
    ...
    await expect(
      service.createEndpoint(
        { name: "Bad", url: "ftp://example.com/hook", eventTypes: ["x"] },
        null,
      ),
    ).rejects.toMatchObject({ code: "UNSAFE_URL" });
  });
