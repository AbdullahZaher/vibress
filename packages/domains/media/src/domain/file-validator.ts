...
import { AssetType, DEFAULT_MEDIA_LIMITS, MediaLimitsConfig, UploadMediaInput } from './asset';
import {
...
const DANGEROUS_EXTENSIONS = new Set([
  'exe', 'dll', 'bat', 'cmd', 'sh', 'php', 'pl', 'py', 'rb', 'js', 'cjs', 'mjs',
  'html', 'htm', 'xhtml', 'svg', 'xml', 'app', 'vbs', 'ps1', 'jar', 'scr', 'msi',
]);
...
export function sanitizeFilename(rawFilename: string): { sanitizedFilename: string; extension: string } {
...
export function detectFileSignature(buffer: Buffer): { mime: string; ext: string; assetType: AssetType } | null {
  if (buffer.length < 4) {
    return null;
  }
...
export function extractImageDimensions(buffer: Buffer, mime: string): { width: number; height: number } | null {
  try {
...
export function validateAndDetectFile(
  ...
  if (DANGEROUS_EXTENSIONS.has(extension)) {
    throw new MediaTypeNotAllowedError(`File extension '.${extension}' is not allowed`);
  }
  ...
  if (DANGEROUS_MIMES.has(declaredMime)) {
    throw new MediaTypeNotAllowedError(`MIME type '${declaredMime}' is dangerous and blocked`);
  }
  ...
  let finalMime = declaredMime || (detected ? detected.mime : 'application/octet-stream');
  ...
  if (detected) {
    ...
    if (declaredMime && !declaredMime.startsWith(detected.assetType) && declaredMime !== 'application/octet-stream') {
    ...
    if (ALLOWED_IMAGE_MIMES.has(declaredMime)) {
      throw new MediaMimeMismatchError(declaredMime, 'unknown/unrecognized-image-signature');
    }
    if (ALLOWED_VIDEO_MIMES.has(declaredMime)) {
      throw new MediaMimeMismatchError(declaredMime, 'unknown/unrecognized-video-signature');
    }
    if (ALLOWED_AUDIO_MIMES.has(declaredMime)) {
      throw new MediaMimeMismatchError(declaredMime, 'unknown/unrecognized-audio-signature');
    }
  }
  ...
  const displayName = input.displayName && input.displayName.trim().length > 0
