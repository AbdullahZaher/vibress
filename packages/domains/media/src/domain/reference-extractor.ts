...
const MEDIA_URL_KEY_REGEX =
  /\/content\/media\/media\/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})\//i;
const MEDIA_URL_SIMPLE_REGEX =
  /\/content\/media\/([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})\//i;

export function extractMediaReferencesFromDocument(
  doc: unknown,
  ...
  function traverse(obj: unknown, path: string) {
    ...
    for (const key of ["src", "url"]) {
      if (typeof record[key] === "string") {
        ...
        const matchKey =
          val.match(MEDIA_URL_KEY_REGEX) || val.match(MEDIA_URL_SIMPLE_REGEX);
