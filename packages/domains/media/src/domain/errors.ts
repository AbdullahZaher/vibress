...
export class MediaTooLargeError extends MediaError {
  constructor(maxSize: number, actualSize: number) {
    super(
      `File size ${actualSize} bytes exceeds maximum allowed limit of ${maxSize} bytes`,
      "MEDIA_TOO_LARGE",
    );
...
export class MediaMimeMismatchError extends MediaError {
  constructor(declared: string, detected: string) {
    super(
      `Declared MIME type '${declared}' does not match detected file signature '${detected}'`,
      "MEDIA_MIME_MISMATCH",
    );
...
export class MediaInUseError extends MediaError {
  ...
  constructor(id: string, referenceCount: number) {
    super(
      `Media asset '${id}' is currently in use by ${referenceCount} reference(s)`,
      "MEDIA_IN_USE",
    );
