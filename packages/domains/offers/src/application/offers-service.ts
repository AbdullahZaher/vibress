...
export class OffersService {
  ...
  async createOffer(data: CreateOfferData, actorId: string | null): Promise<Offer> {
    ...
    if (!OFFER_KEY_REGEX.test(key)) {
      throw new OfferDomainError('VALIDATION_ERROR', 'Offer key must be lowercase alphanumeric with hyphens');
    }
    const existing = await this.repo.findByKey(key);
    if (existing) {
      throw new OfferDomainError('VALIDATION_ERROR', 'Offer key already exists');
    }
  ...
  async updateOffer(id: string, data: UpdateOfferData, actorId: string | null): Promise<Offer> {
    const existing = await this.repo.findById(id);
    if (!existing) throw new OfferDomainError('OFFER_NOT_FOUND', 'Offer not found');
  ...
  async listOffers(filter?: { status?: 'active' | 'disabled' }): Promise<Offer[]> {
  ...
  async validateOffer(offer: Offer, planId: string, now: Date): Promise<void> {
    ...
    if (offer.planId && offer.planId !== planId) {
      throw new OfferDomainError('OFFER_INVALID', 'Offer does not apply to this plan');
    }
    if (offer.maxRedemptions !== null && offer.redemptionCount >= offer.maxRedemptions) {
      throw new OfferDomainError('OFFER_REDEMPTION_LIMIT_REACHED', 'Offer redemption limit reached');
    }
  }
  ...
  private validateOfferData(data: CreateOfferData): void {
    if (data.discountType === 'percentage') {
      if (!Number.isInteger(data.discountValue) || data.discountValue <= 0 || data.discountValue > 100) {
        throw new OfferDomainError('VALIDATION_ERROR', 'Percentage discount must be 1-100');
      }
    } else if (data.discountType === 'fixed_amount') {
      if (!Number.isInteger(data.discountValue) || data.discountValue < 0) {
        throw new OfferDomainError('VALIDATION_ERROR', 'Fixed discount must be non-negative');
      }
    } else {
      throw new OfferDomainError('VALIDATION_ERROR', 'Invalid discount type');
    }

    if (!['once', 'repeating', 'forever'].includes(data.durationType || 'once')) {
    ...
    if ((data.durationType || 'once') === 'repeating') {
      ...
      if (!Number.isInteger(cycles) || cycles < 1) {
        throw new OfferDomainError('VALIDATION_ERROR', 'Repeating discount requires a positive cycle count');
      }
    }
    if (data.startsAt && data.endsAt && data.startsAt.getTime() > data.endsAt.getTime()) {
      throw new OfferDomainError('VALIDATION_ERROR', 'Offer start must be before end');
    }
    if (data.maxRedemptions !== undefined && data.maxRedemptions !== null) {
      if (!Number.isInteger(data.maxRedemptions) || data.maxRedemptions < 1) {
        throw new OfferDomainError('VALIDATION_ERROR', 'Max redemptions must be positive');
      }
    }
  }
}
