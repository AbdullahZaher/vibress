  ...
  async createAutomation(
    ...
    if (
      ...
      !ALLOWED_TRIGGERS.includes(
        data.triggerEvent as (typeof ALLOWED_TRIGGERS)[number],
      )
    ) {
      throw new AutomationDomainError(
        "INVALID_TRIGGER",
        `Trigger ${data.triggerEvent} is not allowed`,
      );
    }
    if (!data.name.trim())
      throw new AutomationDomainError("VALIDATION_ERROR", "Name is required");
  ...
  async executeRun(runId: string): Promise<void> {
    ...
    try {
      for (let i = 0; i < definition.actions.length; i++) {
        ...
        if (!step) {
          step = await this.repo.createStep({
            runId,
            stepIndex: i,
        ...
        if (action.type === "wait") {
          ...
          await this.repo.updateStepStatus(step.id, "waiting", {
        ...
        await this.repo.updateStepStatus(step.id, "executing", {
          attempts: step.attempts + 1,
        });
        try {
          ...
          await this.repo.updateStepStatus(step.id, "failed", {
            error: errMsg,
          });
          throw err;
        }
