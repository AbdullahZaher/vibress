import { getDb, automations, AutomationRow, automationVersions, AutomationVersionRow, automationRuns, AutomationRunRow, automationRunSteps, AutomationRunStepRow } from '@vibress/database';
...
export class DrizzleAutomationRepository implements AutomationRepository {
  ...
  async findByKey(key: string): Promise<Automation | null> {
    const db = getDb();
    const rows = await db.select().from(automations).where(eq(automations.key, key)).limit(1);
    const row = rows[0];
    if (!row) return null;
    return this.mapAutomationToDomain(row);
  }
  ...
  async update(id: string, data: Partial<CreateAutomationData>): Promise<Automation> {
    ...
    if (data.triggerEvent !== undefined) payload.triggerEvent = data.triggerEvent;
    ...
    const [row] = await db.update(automations).set(payload).where(eq(automations.id, id)).returning();
    if (!row) throw new Error(`Automation not found: ${id}`);
    return this.mapAutomationToDomain(row);
  }

  async updateStatus(id: string, status: AutomationStatus): Promise<Automation> {
    const db = getDb();
    const [row] = await db.update(automations).set({ status, updatedAt: new Date() }).where(eq(automations.id, id)).returning();
    if (!row) throw new Error(`Automation not found: ${id}`);
    return this.mapAutomationToDomain(row);
  }
  ...
  async listActiveByTrigger(triggerEvent: string): Promise<Automation[]> {
    ...
    const rows = await db
      .select()
      .from(automations)
      .where(and(eq(automations.status, 'active'), eq(automations.triggerEvent, triggerEvent)));
    return rows.map((r) => this.mapAutomationToDomain(r));
  }

  async createVersion(automationId: string, version: number, definition: AutomationDefinition): Promise<AutomationVersion> {
    const db = getDb();
    const [row] = await db.insert(automationVersions).values({
  ...
  async getVersion(automationId: string, version: number): Promise<AutomationVersion | null> {
    const db = getDb();
    const rows = await db
      .select()
  ...
  async createRun(data: { automationId: string; version: number; runKey: string; triggerEvent: string; eventPayload: Record<string, unknown> | null; depth: number; correlationId: string | null }): Promise<AutomationRun> {
    const db = getDb();
    const now = new Date();
    const [row] = await db.insert(automationRuns).values({
      id: crypto.randomUUID(),
  ...
  async findRun(automationId: string, runKey: string): Promise<AutomationRun | null> {
    const db = getDb();
    const rows = await db
      .select()
      .from(automationRuns)
      .where(and(eq(automationRuns.automationId, automationId), eq(automationRuns.runKey, runKey)))
      .limit(1);
    const row = rows[0];
    if (!row) return null;
    return this.mapRunToDomain(row);
  ...
  async listRuns(filter: { automationId?: string; status?: string; limit?: number; offset?: number } = {}): Promise<{ runs: AutomationRun[]; total: number }> {
    ...
    const conditions = [];
    if (filter.automationId) conditions.push(eq(automationRuns.automationId, filter.automationId));
    if (filter.status) conditions.push(eq(automationRuns.status, filter.status));
    const whereClause = conditions.length ? and(...conditions) : undefined;

    const countRes = await db.select({ total: count() }).from(automationRuns).where(whereClause);
    const rows = await db.select().from(automationRuns).where(whereClause).orderBy(desc(automationRuns.createdAt)).limit(limit).offset(offset);
    return { runs: rows.map((r) => this.mapRunToDomain(r)), total: Number(countRes[0]?.total || 0) };
  }

  async updateRunStatus(id: string, status: string, patch?: Partial<AutomationRun>): Promise<void> {
    const db = getDb();
    ...
    await db.update(automationRuns).set(payload).where(eq(automationRuns.id, id));
  }

  async incrementRunDepth(id: string): Promise<void> {
    const db = getDb();
    await db.update(automationRuns).set({ depth: sql`${automationRuns.depth} + 1` }).where(eq(automationRuns.id, id));
  }

  async createStep(data: { runId: string; stepIndex: number; actionType: string }): Promise<AutomationRunStep> {
    const db = getDb();
    const [row] = await db.insert(automationRunSteps).values({
      id: crypto.randomUUID(),
      runId: data.runId,
  ...
  async findStep(runId: string, stepIndex: number): Promise<AutomationRunStep | null> {
    const db = getDb();
    const rows = await db
      .select()
      .from(automationRunSteps)
      .where(and(eq(automationRunSteps.runId, runId), eq(automationRunSteps.stepIndex, stepIndex)))
      .limit(1);
    const row = rows[0];
    if (!row) return null;
    return this.mapStepToDomain(row);
  ...
  async updateStepStatus(id: string, status: string, patch?: Partial<AutomationRunStep>): Promise<void> {
    const db = getDb();
    const payload: Record<string, unknown> = { status };
    ...
    await db.update(automationRunSteps).set(payload).where(eq(automationRunSteps.id, id));
  }

  async listSteps(runId: string): Promise<AutomationRunStep[]> {
    const db = getDb();
    const rows = await db.select().from(automationRunSteps).where(eq(automationRunSteps.runId, runId)).orderBy(automationRunSteps.stepIndex);
    return rows.map((r) => this.mapStepToDomain(r));
  }

  async findWaitingRuns(limit: number): Promise<AutomationRun[]> {
    const db = getDb();
    const rows = await db.select().from(automationRuns).where(eq(automationRuns.status, 'waiting')).orderBy(automationRuns.createdAt).limit(limit);
    return rows.map((r) => this.mapRunToDomain(r));
  }

  async findWaitingSteps(limit: number): Promise<AutomationRunStep[]> {
    const db = getDb();
    const rows = await db.select().from(automationRunSteps).where(eq(automationRunSteps.status, 'waiting')).orderBy(automationRunSteps.createdAt).limit(limit);
    return rows.map((r) => this.mapStepToDomain(r));
  }
