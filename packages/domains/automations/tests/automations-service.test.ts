...
} from "../src/application/automations-service";
...
function makeAutomation(overrides: Partial<Automation> = {}): Automation {
  return {
    ...
    conditions: [],
    actions: [
      { type: "webhook", config: { url: "https://receiver.example.com/hook" } },
    ],
    status: "active",
    version: 1,
    createdBy: null,
...
describe("evaluateConditions (declarative, no eval)", () => {
  ...
  it("equals matches on dotted field paths", () => {
    ...
    expect(
      evaluateConditions(
        [{ field: "subscription.status", op: "equals", value: "cancelled" }],
        payload,
      ),
    ).toBe(false);
  });
  ...
  it("not_equals and exists work", () => {
    ...
    expect(
      evaluateConditions([{ field: "memberId", op: "exists" }], payload),
    ).toBe(true);
    expect(
      evaluateConditions([{ field: "planId", op: "exists" }], payload),
    ).toBe(false);
    expect(
      evaluateConditions(
        [{ field: "memberId", op: "not_equals", value: "m2" }],
        payload,
      ),
    ).toBe(true);
  });
  ...
  it("multiple conditions are ANDed", () => {
    ...
    expect(
      evaluateConditions(
        [
          ...conditions,
          { field: "productId", op: "equals" as const, value: "p2" },
        ],
        payload,
      ),
    ).toBe(false);
...
describe("AutomationsService", () => {
  ...
  const dispatcher = {
    ...
    enqueueDelayedStep: vi.fn(async () => undefined),
  };
  ...
  it("rejects an unsupported trigger", async () => {
    ...
    await expect(
      service.createAutomation(
        { key: "x", name: "X", triggerEvent: "pwn.all", actions: [] },
        null,
      ),
  ...
  it("rejects an automation with too many actions", async () => {
    ...
    await expect(
      service.createAutomation(
        { key: "x", name: "X", triggerEvent: "member.created", actions },
        null,
      ),
  ...
  it("cannot activate an automation with no actions", async () => {
    const repoWith: AutomationRepository = {
      ...repo,
      findById: vi.fn(async () => makeAutomation({ actions: [] })),
    };
    const service = makeService({ repo: repoWith });
    await expect(service.activateAutomation("a1", "u1")).rejects.toMatchObject({
  ...
  it("handleEvent only matches active automations for the trigger", async () => {
    const repoWith: AutomationRepository = {
      ...repo,
      listActiveByTrigger: vi.fn(async (trigger) =>
        trigger === "member.created" ? [makeAutomation()] : [],
      ),
    };
    const service = makeService({ repo: repoWith });
    const created = await service.handleEvent("member.created", {
      memberId: "m1",
    });
    expect(created).toBe(1);
    expect(
      await service.handleEvent("comment.created", { commentId: "c1" }),
    ).toBe(0);
  });
  ...
  it("conditions filter which events create runs", async () => {
    ...
    const service = makeService({ repo: repoWith });
    const created = await service.handleEvent("subscription.activated", {
      memberId: "m1",
      productId: "premium",
    });
    expect(created).toBe(1);
    const skipped = await service.handleEvent("subscription.activated", {
      memberId: "m1",
      productId: "other",
    });
  ...
  it("executeRun completes steps and marks the run completed", async () => {
    ...
    expect(repoWith.updateRunStatus).toHaveBeenCalledWith(
      "r1",
      "completed",
      expect.anything(),
    );
  ...
  it("wait actions persist run state and schedule a delayed resume", async () => {
    ...
    const repoWith: AutomationRepository = {
      ...
      getVersion: vi.fn(async () => ({
        ...
        definition: {
          conditions: [],
          actions: [{ type: "wait", config: { delayMs: 5000 } }],
        },
        createdAt: new Date(),
      })),
      findStep: vi.fn(async () => null),
    ...
    expect(dispatcherWith.enqueueDelayedStep).toHaveBeenCalledWith(
      "r1",
      0,
      5000,
    );
  ...
  it("manual run requires an active automation", async () => {
    const repoWith: AutomationRepository = {
      ...repo,
      findById: vi.fn(async () => makeAutomation({ status: "draft" })),
    };
    const service = makeService({ repo: repoWith });
    await expect(service.manualRun("a1", "u1")).rejects.toMatchObject({
