...
export class StorageService {
  ...
  async createConfiguration(
    ...
    await this.auditRepo.record({
      ...
      targetId: config.id,
      metadata: { name: config.name, providerType: config.providerType, bucket: config.bucket },
    });
  ...
  async updateConfiguration(
    ...
    const updatePayload: UpdateStorageConfigurationData & { encryptedCredentials?: string | null } = {
      ...data,
    };
    ...
    const updated = await this.storageRepo.updateConfiguration(id, updatePayload);
    ...
    await this.auditRepo.record({
      ...
      targetType: 'storage_configuration',
      targetId: updated.id,
      metadata: { name: updated.name, providerType: updated.providerType, credentialsChanged: !!data.credentials },
    });
  ...
  async activateConfiguration(id: string, actorId: string): Promise<StorageConfiguration> {
    ...
    await this.auditRepo.record({
      ...
      metadata: { name: activated.name, providerType: activated.providerType, providerName: provider.name },
    });
  ...
  async deleteConfiguration(id: string, actorId: string): Promise<void> {
    ...
    if (existing.isActive) {
      throw new StorageError('Cannot delete active storage configuration. Activate another configuration first.', 'STORAGE_PROVIDER_IN_USE');
    }
  ...
  async testConnection(
    ...
    if ('id' in data && data.id) {
      ...
      if (!existing) {
        throw new StorageError(`Storage configuration not found: ${data.id}`, 'STORAGE_CONFIGURATION_NOT_FOUND');
      }
      ...
      if (existing.publicBaseUrl) s3Config.publicBaseUrl = existing.publicBaseUrl;
    } else {
    ...
    await this.auditRepo.record({
      ...
      metadata: { connected: result.connected, providerType: result.providerType, bucket: result.bucket },
    });

    return result;
  }

  async instantiateAndRegisterProvider(config: StorageConfiguration): Promise<S3StorageProvider | LocalStorageProvider> {
  ...
  async initializeStartupProvider(): Promise<void> {
    try {
      ...
      if (activeConfig) {
        const provider = await this.instantiateAndRegisterProvider(activeConfig);
        this.registry.setActiveProvider(provider.name);
      } else {
      ...
      console.warn('Failed to initialize active storage provider on startup. Falling back to local provider.', err);
      const localProvider = new LocalStorageProvider();
      this.registry.register(localProvider);
      this.registry.setActiveProvider(localProvider.name);
    }
  ...
  private decryptConfigCredentials(config: StorageConfiguration): Record<string, string> {
