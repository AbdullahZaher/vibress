import { S3Client, PutObjectCommand, GetObjectCommand, DeleteObjectCommand, HeadObjectCommand, HeadBucketCommand, CreateMultipartUploadCommand, UploadPartCommand, CompleteMultipartUploadCommand, AbortMultipartUploadCommand, } from '@aws-sdk/client-s3';
...
export class S3StorageProvider {
    ...
    async headObject(key) {
        ...
        try {
            ...
            if (res.ETag)
                result.etag = res.ETag.replace(/"/g, '');
            if (res.LastModified)
                result.lastModified = res.LastModified;
            return result;
        }
        catch (err) {
