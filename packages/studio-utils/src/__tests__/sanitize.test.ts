...
describe("sanitizeStudioHtml — allowlist sanitizer", () => {
  ...
  it("strips event handler attributes (onerror, onclick, onload, onmouseover)", () => {
    const out = sanitizeStudioHtml(
      '<img src="x.png" onerror="alert(1)"><div onclick="x()" onload="y()">t</div>',
    );
  ...
  it("drops iframes without an approved provider src, keeps approved ones", () => {
    const bad = sanitizeStudioHtml(
      '<iframe srcdoc="<script>alert(1)</script>"></iframe>',
    );
    expect(bad).not.toContain("iframe");
    const evil = sanitizeStudioHtml(
      '<iframe src="https://evil.example/x"></iframe>',
    );
    expect(evil).not.toContain("iframe");
    const good = sanitizeStudioHtml(
  ...
  it("strips style attributes and data-* attributes", () => {
    const out = sanitizeStudioHtml(
      '<div style="position:fixed" data-x="1">t</div>',
    );
  ...
  it("preserves safe semantic markup", () => {
    ...
    for (const frag of [
      ...
      "<pre>",
      "<figure>",
      '<img src="/content/media/media/a/b.png" alt="pic">',
...
describe("stripHtml", () => {
  it("extracts plain text from html", () => {
    expect(
      stripHtml('<p>Hello <strong>world</strong></p><div onclick="x">!</div>'),
    ).toBe("Hello world !");
  });
});
...
describe("getEmbedProvider — provider allowlist", () => {
  it("accepts youtube watch/embed/short links", () => {
    expect(
      getEmbedProvider("https://www.youtube.com/watch?v=dQw4w9WgXcQ")?.embedUrl,
    ).toBe("https://www.youtube-nocookie.com/embed/dQw4w9WgXcQ");
    expect(getEmbedProvider("https://youtu.be/dQw4w9WgXcQ")?.embedUrl).toBe(
      "https://www.youtube-nocookie.com/embed/dQw4w9WgXcQ",
    );
    expect(
      getEmbedProvider("https://www.youtube.com/embed/dQw4w9WgXcQ")?.embedUrl,
    ).toBe("https://www.youtube-nocookie.com/embed/dQw4w9WgXcQ");
    expect(
      getEmbedProvider("https://www.youtube-nocookie.com/embed/dQw4w9WgXcQ")
        ?.kind,
    ).toBe("youtube");
  });

  it("accepts vimeo links", () => {
    expect(getEmbedProvider("https://vimeo.com/123456")?.embedUrl).toBe(
      "https://player.vimeo.com/video/123456",
    );
    expect(
      getEmbedProvider("https://player.vimeo.com/video/123456")?.kind,
    ).toBe("vimeo");
  });
