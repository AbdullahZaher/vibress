...
export const ThemeManifestSchema = z.object({
  id: z
    ...
    .min(1)
    .max(100)
    .regex(/^[a-z0-9][a-z0-9-]*$/, 'Theme ID must be lowercase alphanumeric with hyphens'),
  name: z.string().trim().min(1).max(100),
  version: z.string().trim().regex(/^\d+\.\d+\.\d+$/, 'Theme version must be valid semver'),
...
export function validateThemeManifest(manifest: unknown): ThemeManifest {
  ...
  if (!parsed.success) {
    throw new ThemeError('THEME_INVALID', `Invalid theme manifest: ${parsed.error.message}`);
  }
  return parsed.data;
}
...
export function validateThemeSettings(schema: ThemeSettingsSchema, input: Record<string, unknown>): Record<string, unknown> {
...
function validateSettingValue(
  ...
  switch (def.type) {
    ...
    case 'boolean':
      ...
      return { ok: true, value };
    case 'number': {
      if (typeof value !== 'number' || Number.isNaN(value)) return { ok: false, error: 'expected number' };
      if (def.min !== undefined && value < def.min) return { ok: false, error: `value below min ${def.min}` };
      if (def.max !== undefined && value > def.max) return { ok: false, error: `value above max ${def.max}` };
      return { ok: true, value };
    }
    case 'color': {
      if (typeof value !== 'string') return { ok: false, error: 'expected color string' };
    ...
    case 'select': {
      ...
      if (!def.options.includes(value)) {
        return { ok: false, error: `invalid option, allowed: ${def.options.join(', ')}` };
      }
      return { ok: true, value };
    }
    default:
