...
export const newsletterPreferences = pgTable('newsletter_preferences', {
  ...
  return {
    uniqueMemberNewsletterIdx: uniqueIndex('newsletter_prefs_member_newsletter_idx').on(table.memberId, table.newsletterId),
...
export type NewNewsletterPreferenceRow = typeof newsletterPreferences.$inferInsert;

export const newsletterSends = pgTable('newsletter_sends', {
  id: text('id').primaryKey(),
  newsletterId: text('newsletter_id').notNull().references(() => newsletters.id, { onDelete: 'restrict' }),
  ...
  return {
    ...
    scheduledAtIdx: index('newsletter_sends_scheduled_at_idx').on(table.scheduledAt),
    newsletterIdx: index('newsletter_sends_newsletter_idx').on(table.newsletterId),
  };
});
...
export const emailRecipients = pgTable('email_recipients', {
  ...
  return {
    ...
    sendStatusIdx: index('email_recipients_send_status_idx').on(table.sendId, table.status),
    uniqueMemberSendIdx: uniqueIndex('email_recipients_member_send_idx').on(table.memberId, table.sendId),
  };
});
...
export const emailEvents = pgTable('email_events', {
  id: text('id').primaryKey(),
  recipientId: text('recipient_id').notNull().references(() => emailRecipients.id, { onDelete: 'cascade' }),
  sendId: text('send_id').references(() => newsletterSends.id, { onDelete: 'cascade' }),
  memberId: text('member_id').references(() => members.id, { onDelete: 'set null' }),
...
export const emailSuppressions = pgTable('email_suppressions', {
  ...
  return {
    uniqueEmailIdx: uniqueIndex('email_suppressions_email_reason_idx').on(table.email, table.reason),
...
export const providerEvents = pgTable('provider_events', {
  ...
  return {
    uniqueProviderEventIdx: uniqueIndex('provider_events_provider_event_idx').on(table.provider, table.providerEventId),
  };
});
