import { pgTable, text, timestamp, integer, boolean, index, uniqueIndex, jsonb, numeric } from 'drizzle-orm/pg-core';
...
export const plans = pgTable('plans', {
  ...
  return {
    ...
    uniqueKeyPerProductIdx: uniqueIndex('plans_unique_key_per_product_idx').on(table.productId, table.key),
  };
});
...
export const billingCustomers = pgTable('billing_customers', {
  ...
  return {
    uniqueMemberProviderIdx: uniqueIndex('billing_customers_member_provider_idx').on(table.memberId, table.provider),
...
export const billingPlanMappings = pgTable('billing_plan_mappings', {
  id: text('id').primaryKey(),
  ...
  return {
    uniquePlanProviderIdx: uniqueIndex('billing_plan_mappings_plan_provider_idx').on(table.planId, table.provider),
  };
});
...
export const subscriptions = pgTable('subscriptions', {
  ...
  providerEventTimestamp: timestamp('provider_event_timestamp', { withTimezone: true }),
  createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
}, (table) => {
  return {
    ...
    memberStatusIdx: index('subscriptions_member_status_idx').on(table.memberId, table.status),
  };
});
...
export const billingWebhookEvents = pgTable('billing_webhook_events', {
  ...
  return {
    uniqueProviderEventIdx: uniqueIndex('billing_webhook_events_provider_event_idx').on(table.provider, table.providerEventId),
  };
});
