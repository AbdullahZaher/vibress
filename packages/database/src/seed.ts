...
export const SYSTEM_ROLES = [
  {
    key: "owner",
    name: "Owner",
    description: "Site Owner with full administrative access",
    isSystem: true,
  },
  {
    key: "administrator",
...
export const SYSTEM_PERMISSIONS = [
  ...
  { key: "redirects.read", description: "Read redirect rules" },
  {
    ...
    description: "Create, edit, and manage redirects",
  },
...
export const seedDatabase = async (options?: SeedOptions): Promise<void> => {
  ...
  for (const permDef of SYSTEM_PERMISSIONS) {
    const existing = await db
      .select()
      .from(permissions)
      .where(eq(permissions.key, permDef.key))
      .limit(1);
    let permId: string;
    if (existing.length === 0) {
  ...
  const authorRoleRows = await db
    .select()
    .from(roles)
    .where(eq(roles.key, "author"))
    .limit(1);
  const contributorRoleRows = await db
    .select()
    .from(roles)
    .where(eq(roles.key, "contributor"))
    .limit(1);
  ...
  const skipDevUsers =
    options?.skipDevUsers ?? process.env.NODE_ENV === "production";
  if (!skipDevUsers) {
