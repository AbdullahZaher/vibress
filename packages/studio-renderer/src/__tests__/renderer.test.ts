...
describe("renderStudioDocumentToHtml — all Studio cards", () => {
  ...
  it("renders image card with numeric pixel dimensions from the media pipeline", () => {
    ...
    expect(html).toContain(
      '<img src="/p.png" alt="p" width="1200" height="630">',
    );
  ...
  it("renders audio card", () => {
    const html = renderStudioDocumentToHtml(
      doc([card("audio", { src: "/a.mp3", title: "Podcast" })]),
    );
  ...
  it("renders embed card only for approved providers; others become safe links", () => {
    const yt = renderStudioDocumentToHtml(
      doc([
        card("embed", {
    ...
    const evil = renderStudioDocumentToHtml(
      doc([
        card("embed", {
  ...
  it("renders toggle card as details/summary", () => {
    const html = renderStudioDocumentToHtml(
      doc([card("toggle", { heading: "FAQ", content: "Answer" })]),
    );
  ...
  it("renders html card sanitized (script stripped, safe html kept)", () => {
    const html = renderStudioDocumentToHtml(
      doc([
        card("html", {
          html: '<div class="widget">Safe <strong>widget</strong></div><script>alert(1)</script><img src=x onerror=alert(2)>',
        }),
      ]),
    );
    expect(html).toContain(
      '<div class="widget">Safe <strong>widget</strong></div>',
    );
...
describe("unknown/corrupt cards — graceful failure", () => {
  ...
  it("never leaks card payloads for unknown/corrupt/transient cards (HTML + plain text)", () => {
    ...
    const input = doc([
      ...
      },
      {
        type: "studio-card",
        cardType: "video",
        cardData: { src: "blob:http://localhost/xyz", secret },
        version: 1,
      },
      {
        type: "paragraph",
            ...
            children: [
              {
                type: "tablecell",
                headerState: 1,
                children: [{ type: "text", text: "Column 1", format: 1 }],
              },
              {
                type: "tablecell",
                headerState: 1,
                children: [{ type: "text", text: "Column 2", format: 1 }],
              },
            ],
          },
          {
            ...
            children: [
              {
                type: "tablecell",
                children: [{ type: "text", text: "Data 1", format: 0 }],
              },
              {
                type: "tablecell",
                children: [{ type: "text", text: "Data 2", format: 0 }],
              },
            ],
          },
        ],
        ...
        children: [
          {
            type: "listitem",
            checked: true,
            children: [{ type: "text", text: "Done task", format: 0 }],
          },
          {
            type: "listitem",
            checked: false,
  ...
  it("renders nothing for empty documents", () => {
    expect(
      renderStudioDocumentToHtml({ root: { type: "root", children: [] } }),
    ).toBe("");
  });
});
...
describe("renderStudioDocumentToPlainText", () => {
  it("includes card plain text and never leaks raw html", () => {
    const text = renderStudioDocumentToPlainText(
      doc([
        {
          type: "paragraph",
          children: [
  ...
  it("extracts table of contents hierarchy correctly", () => {
    const toc = extractTableOfContentsFromDocument(
      doc([
        {
          type: "heading",
          tag: "h2",
        ...
        { type: "paragraph", children: [{ type: "text", text: "Paragraph" }] },
        {
          type: "heading",
          tag: "h3",
          children: [{ type: "text", text: "Sub section" }],
        },
        {
          type: "heading",
          tag: "h2",
