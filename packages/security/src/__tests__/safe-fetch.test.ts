...
import { isPrivateIP, isSafeUrl, normalizeIP, safeFetch } from '../http/safe-fetch';
...
describe('SSRF Protection - IP & URL Validation', () => {
  ...
  describe('safeFetch integration', () => {
    it('blocks localhost (DNS resolves to loopback)', async () => {
      await expect(safeFetch('http://localhost:1/')).rejects.toThrow(/private|BLOCKED/i);
    });

    it('blocks 127.0.0.1 directly', async () => {
      await expect(safeFetch('http://127.0.0.1:1/')).rejects.toThrow(/private|BLOCKED/i);
    });

    it('blocks 169.254.169.254 (cloud metadata)', async () => {
      await expect(safeFetch('http://169.254.169.254/latest/meta-data')).rejects.toThrow(/private|BLOCKED/i);
    });
