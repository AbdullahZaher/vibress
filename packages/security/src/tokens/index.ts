...
export function signSiteAuthToken(secret: string): string {
  ...
  const hmac = crypto.createHmac('sha256', secret).update(`v1:${timestamp}`).digest('hex');
...
export function verifySiteAuthToken(
  ...
  const expectedHmac = crypto.createHmac('sha256', secret).update(`v1:${timestampStr}`).digest('hex');
