...
export class SecretEncryptionError extends Error {
  constructor(message: string, public code = 'ENCRYPTION_ERROR') {
    super(message);
 ...
 */
export function decryptSecret(encryptedPayloadJson: string, masterKey?: string): string {
  if (!encryptedPayloadJson || typeof encryptedPayloadJson !== 'string') {
    throw new SecretEncryptionError('Encrypted payload must be a non-empty string');
  }
  ...
  try {
    ...
    throw new SecretEncryptionError('Failed to parse encrypted secret payload', 'STORAGE_ENCRYPTION_INVALID');
  }

  if (payload.v !== 1 || !payload.iv || !payload.tag || !payload.data) {
    throw new SecretEncryptionError('Invalid encrypted secret payload structure', 'STORAGE_ENCRYPTION_INVALID');
  }
