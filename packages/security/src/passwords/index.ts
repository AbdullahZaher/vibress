...
export async function verifyPassword(hash: string, password: string): Promise<boolean> {
  if (!hash || typeof password !== 'string' || password.length > MAX_PASSWORD_LENGTH) {
    return false;
  }
  try {
...
const DUMMY_HASH = '$argon2id$v=19$m=65536,t=3,p=4$dummyhashforprotection$dummyhashforprotection';
