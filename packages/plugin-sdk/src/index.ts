...
export function validateManifest(manifest: unknown): PluginManifest {
  ...
  if (typeof m.entrypoint !== 'string' || !m.entrypoint.trim()) throw new Error('Manifest entrypoint is required');
  if (typeof m.vibressApiVersion !== 'string' || m.vibressApiVersion !== SDK_VERSION) {
    throw new Error(`Unsupported Vibress API version: ${String(m.vibressApiVersion)} (expected ${SDK_VERSION})`);
  }
  ...
  for (const cap of m.capabilities as unknown[]) {
    if (typeof cap !== 'string' || !(SUPPORTED_CAPABILITIES as readonly string[]).includes(cap)) {
      throw new Error(`Unsupported capability: ${String(cap)}`);
    }
  }
  if (m.hooks !== undefined && !Array.isArray(m.hooks)) throw new Error('Manifest hooks must be an array');
  ...
  return {
    ...
    settingsSchema: typeof m.settingsSchema === 'object' && m.settingsSchema !== null ? (m.settingsSchema as Record<string, unknown>) : undefined,
