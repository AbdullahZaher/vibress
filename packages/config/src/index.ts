...
const csvString = z.preprocess(
  ...
  z
    .string()
    .refine(
      (value) => value.split(",").every((origin) => isUrl(origin.trim())),
      "must be a comma-separated URL list",
    ),
);
...
const envSchema = z.object({
  ...
  VIBRESS_VERSION: nonEmptyString.default("1.1.0"),
...
export function loadConfig(env: EnvSource): AppConfig {
  ...
  if (!parsed.success) {
    const issues = parsed.error.errors.map(
  ...
  const adminOrigin = raw.ADMIN_ORIGIN
    ? stripTrailingSlash(raw.ADMIN_ORIGIN)
    : null;
  const portalOrigin = raw.PORTAL_ORIGIN
    ? stripTrailingSlash(raw.PORTAL_ORIGIN)
    : null;
  const productionCorsOrigins = uniqueStrings([
  ...
  const config: AppConfig = {
    ...
    cors: {
      ...
      staffAllowedOrigins: allowedOrigins(
        isProduction,
        [
          "http://localhost:7777",
          "http://localhost:7779",
      ...
      memberAllowedOrigins: allowedOrigins(
        isProduction,
        [
          "http://localhost:7777",
          "http://localhost:7781",
        ...
        [portalOrigin, ...configuredCorsOrigins],
      ),
    },
    cookies: {
    ...
    observability: {
      ...
      tracing: {
        otlpEndpoint:
          raw.OTEL_EXPORTER_OTLP_ENDPOINT || "http://127.0.0.1:4318",
...
function enforceProductionGuards(config: AppConfig): void {
  ...
  if (
    ...
    config.secrets.encryptionKey.includes("change-me")
  ) {
    issues.push("VIBRESS_ENCRYPTION_KEY must be set to a real production key");
  }
  if (
    !config.newsletters.unsubscribeSecret ||
    config.newsletters.unsubscribeSecret === "dev-unsub-secret"
  ) {
    issues.push("NEWSLETTER_UNSUBSCRIBE_SECRET must be set in production");
  }
  if (
    !config.billing.stripeSecretKey ||
    config.billing.stripeSecretKey === "sk_test_missing"
  ) {
  ...
  if (config.cors.origins.length === 0) {
    issues.push(
      "CORS_ORIGINS, ADMIN_ORIGIN, or PORTAL_ORIGIN must define at least one production origin",
    );
  }
  ...
  if (isSetupTokenInsecure(config.setup.token)) {
    issues.push(
      "VIBRESS_SETUP_TOKEN must be set to a secure value in production (at least 16 characters, not a placeholder)",
    );
  }
  if (issues.length > 0) {
    throw new ConfigError(
